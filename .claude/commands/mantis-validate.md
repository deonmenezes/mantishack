...
```bash
libexec/mantishack-validation-helper <A|B|C|D|E|F> "$OUTPUT_DIR" [--target "$TARGET_PATH"]
```
...
1. **Prep:** `libexec/mantishack-validation-helper A "$OUTPUT_DIR" --target "$TARGET_PATH"`
...
1. **Prep:** `libexec/mantishack-validation-helper B "$OUTPUT_DIR" --target "$TARGET_PATH"`
...
1. **Prep:** `libexec/mantishack-validation-helper C "$OUTPUT_DIR" --target "$TARGET_PATH"`
...
1. **Prep:** `libexec/mantishack-validation-helper D "$OUTPUT_DIR" --target "$TARGET_PATH"`
...
1. **Prep:** `libexec/mantishack-validation-helper E "$OUTPUT_DIR" --target "$TARGET_PATH"`
...
2. **Analysis:** For each binary group found by prep, run feasibility:
   ```bash
   libexec/mantishack-run-feasibility <binary_path> "$OUTPUT_DIR/findings.json" "$OUTPUT_DIR"
   ```
...
1. **Prep:** `libexec/mantishack-validation-helper F "$OUTPUT_DIR"`
...
Replace `<your-model-id>` with your exact model ID from your system prompt (e.g. `claude-opus-4-7`) — it records which model performed the validation, which only you (the harness) know (MANTISHACK's Python can't read `/model`). If you don't know your model ID, drop the `--model` flag; the run still completes, the model is just unrecorded.

```bash
libexec/mantishack-validation-helper 1 "$OUTPUT_DIR" --model <your-model-id>
```
