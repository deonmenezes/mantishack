...
3. **Run MANTISHACK fuzzing**: Execute the fuzzing command:
   ```bash
   python3 mantishack.py fuzz --binary <path> --duration <seconds>
   ```
...
```bash
python3 mantishack.py fuzz --binary /path/to/binary --duration 600 --max-crashes 5
```

With custom corpus:
```bash
python3 mantishack.py fuzz --binary /path/to/binary --corpus /path/to/seeds --duration 3600
```
...
- MANTISHACK automatically analyzes crashes and generates exploits
