...
2.5. **Run auth + logging audit** automatically (JWT misuse, cookie security, audit-log coverage) — rules tagged `mantis_capability: auth-audit`; see `/mantis-auth-audit` for the standalone command
...
Execute: `libexec/mantishack-agentic --repo <path>`
...
By default, the primary model is auto-detected from `~/.config/mantishack/models.json` or API key env vars (GEMINI_API_KEY, ANTHROPIC_API_KEY, OPENAI_API_KEY). Use `--model` to override.
...
and add a 1-2 sentence summary paragraph after the `# MANTISHACK Agentic Security Report`
