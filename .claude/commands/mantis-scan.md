...
3. **Run MANTISHACK scan**: Execute the appropriate command based on what they need:
   - For full autonomous scan (recommended): `python3 mantishack.py agentic --repo <path>`
   - For quick Semgrep scan: `python3 mantishack.py scan --repo <path>`
   - For CodeQL only: `python3 mantishack.py codeql --repo <path>`
