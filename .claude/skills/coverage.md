...
`mantishack-coverage-summary` accepts any of: no argument (active project), project name, target path, project output dir, or run dir. It resolves automatically.

**Summary and detail:**
```bash
libexec/mantishack-coverage-summary                          # active project
libexec/mantishack-coverage-summary --detailed               # per-file table
libexec/mantishack-coverage-summary /tmp/vulns               # resolves to project
libexec/mantishack-coverage-summary out/projects/vulns/validate-20260411/  # specific run
```

**Find gaps** (unreviewed functions):
```bash
libexec/mantishack-coverage-summary --gaps
```
...
```bash
libexec/mantishack-coverage-summary <run_dir> --mark src/auth.c:check_pw src/db.c:query
```

From file (many functions — preferred for `/understand` and `/validate`):
```bash
libexec/mantishack-coverage-summary <run_dir> --mark-file "$OUTPUT_DIR/reviewed-items.json"
```
...
```bash
libexec/mantishack-coverage-summary <run_dir> --unmark src/auth.c:check_pw
```

**Via project command** (summary and detail only):
```bash
libexec/mantishack-project-manager coverage
libexec/mantishack-project-manager coverage --detailed
```
...
Semgrep policy groups are compared against `MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK` to identify which vulnerability classes weren't scanned. Missing groups appear in the "Action needed" section.
...
See `/home/mantishack/design/coverage-layer.md` for the full design.
