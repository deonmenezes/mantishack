...
```yaml
output_dir: resolved by mantishack-run-lifecycle start understand
confidence_levels:
...
7. **libexec scripts:** Run `libexec/` scripts exactly as shown in the prompts — do not prepend `bash`, `export` commands, absolute paths, or additional shell logic. The permission system auto-approves `libexec/mantishack-*` commands only when run in this exact form.
