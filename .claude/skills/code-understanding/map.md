...
```bash
libexec/mantishack-normalize-context-map "$WORKDIR"
```
...
```bash
libexec/mantishack-enrich-context-map-callgraph "$WORKDIR"
```
...
```bash
libexec/mantishack-enrich-context-map-ast-view "$WORKDIR"
```
...
```bash
libexec/mantishack-enrich-context-map-sites "$WORKDIR"
```
...
```bash
libexec/mantishack-coverage-summary "$WORKDIR" --mark src/routes/query.py:handle_query src/db/query.py:run_query
```
...
```bash
mantishack-sandbox-observe --json --out "$WORKDIR/probe" -- \
