...
<!--
This file is a derivative work of Hacker Bob (https://github.com/vmihalis/hacker-bob/blob/main/.claude/agents/deep-recon-agent.md),
...
scratch="$(mktemp -d "${TMPDIR:-/tmp}/mantis-deep-recon-subdomains.XXXXXX")" || exit 0
...
```bash
DOMAIN="[DOMAIN]"; SESSION="[SESSION]"
scratch="$(mktemp -d "${TMPDIR:-/tmp}/mantis-deep-recon-live.XXXXXX")" || exit 0
trap 'rm -rf "$scratch"' EXIT
...
```bash
DOMAIN="[DOMAIN]"; SESSION="[SESSION]"
scratch="$(mktemp -d "${TMPDIR:-/tmp}/mantis-deep-recon-family.XXXXXX")" || exit 0
trap 'rm -rf "$scratch"' EXIT
...
```bash
DOMAIN="[DOMAIN]"; SESSION="[SESSION]"
scratch="$(mktemp -d "${TMPDIR:-/tmp}/mantis-deep-recon-js.XXXXXX")" || exit 0
trap 'rm -rf "$scratch"' EXIT
