#!/usr/bin/env python3
"""
MANTISHACK CodeQL - Complete Autonomous Workflow
...
def get_llm_client():
    """Initialize LLM client from existing MANTISHACK system."""
    from packages.llm_analysis import get_client
    return get_client()


def get_exploit_validator(work_dir: Path):
    """Initialize exploit validator from existing MANTISHACK system."""
    try:
...
def get_multi_turn_analyzer(llm_client):
    """Initialize multi-turn analyzer from existing MANTISHACK system."""
    try:
...
def run_autonomous_workflow(args):
    ...
    """
    logger.info(f"{'=' * 70}")
    logger.info("MANTISHACK CODEQL - AUTONOMOUS SECURITY ANALYSIS")
    logger.info(f"{'=' * 70}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MANTISHACK CodeQL - Fully Autonomous Security Analysis",
...
Examples:
  ...
  python3 mantishack_codeql.py --repo /path/to/code --languages java \\
    --build-command "mvn clean compile -DskipTests"

  # Analyze up to 20 findings
  python3 mantishack_codeql.py --repo /path/to/code --max-findings 20
        """
    )
    ...
    # ``mantishack_agentic.py``'s default 10: codeql-only mode does one
    ...
    # Apply --phase-timeout to the framework-wide MantishackConfig.CODEQL_TIMEOUT
    ...
    # MantishackConfig constant once at startup.
    if args.phase_timeout != MantishackConfig.CODEQL_TIMEOUT:
        MantishackConfig.CODEQL_TIMEOUT = args.phase_timeout if args.phase_timeout > 0 else None
