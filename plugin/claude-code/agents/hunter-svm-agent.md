...
description: SVM-chain hunter wrapper — Solana / SVM smart-contract probe. Defers to prompts/roles/hunter-svm.md.
...
prompts/roles/hunter-svm.md (clean-room PR #88). Shares the
HUNTER_PASS_FILED marker.
-->

# hunter-svm-agent — Claude Code wrapper

Spawned for the SVM hunter role. Behavior in
`prompts/roles/hunter-svm.md` + shared discipline in
`prompts/roles/hunter.md`. Read both at startup.

Prefer `mantis-cli svm <subcommand>` via Bash; MCP fallback when
needed.
