...
that — every libexec/mantishack-* file must contain the sentinel comment
...
# file, not $MANTISHACK_DIR, so the test inspects libexec/ in its own
# worktree (MANTISHACK_DIR may point at a different checkout).
REPO = Path(__file__).resolve().parents[2]
LIBEXEC = REPO / "libexec"

_SENTINEL = "# ─── trust-marker check"
_TRUST_VARS = ("CLAUDECODE", "_MANTISHACK_TRUSTED")


def _libexec_scripts() -> list[Path]:
    """All `libexec/mantishack-*` files (excluding test dir + caches)."""
    out = []
    for p in sorted(LIBEXEC.glob("mantishack-*")):
        if p.is_dir():
            continue
        out.append(p)
    return out
...
class LibexecMarkerCoverageTests(unittest.TestCase):
    """Every libexec/mantishack-* script must inline the trust-marker check."""
    ...
    def test_check_appears_near_top(self):
        ...
        That's loose enough to permit long module docstrings (mantishack-
