...
build_repo() {
    ...
    # Create mantishack-offset (copy real file for accurate box-width testing)
    mkdir -p "$REPO/core/startup/assets"
    cp "$SCRIPT_DIR/core/startup/assets/mantishack-offset" \
       "$REPO/core/startup/assets/mantishack-offset"
...
# mantishack-offset banner (lives under core/startup/assets/)
BANNER = 'core/startup/assets/mantishack-offset'
...
echo "side" >> mantishack.py && git add -A && git commit -m "side branch commit"
git tag v99.0.0
...
# Save originals
BANNER_PATH=core/startup/assets/mantishack-offset
...
for tag in v3.1.0 v10.20.30; do
    ...
    # mantishack-offset: check line contains tag and is correct width
    BANNER_LINE=$(grep "Based on Claude Code" "$BANNER_PATH")
    BANNER_LEN=${#BANNER_LINE}
    assert_eq      "mantishack-offset width for $tag" "77" "$BANNER_LEN"
    assert_contains "mantishack-offset contains $tag" "$BANNER_LINE" "$tag"
    assert_contains "mantishack-offset box intact"    "$BANNER_LINE" "║"
...
# Should be included
assert_file_exists "mantishack.py in archive"           "$ARCHIVE_DIR/mantishack.py"
assert_file_exists "mantishack-offset in archive"       "$ARCHIVE_DIR/core/startup/assets/mantishack-offset"
...
ARCHIVE_BANNER=$(grep "Based on Claude Code" "$ARCHIVE_DIR/core/startup/assets/mantishack-offset")
