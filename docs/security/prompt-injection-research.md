# Prompt Injection Defenses: State of the Art (2024–2026) for MANTISHACK

Scope: techniques **not** in MANTISHACK's planned PR-1 set (envelope/nonce, slot discipline, control-char sanitisation, base64 wrapping, output schema validation, capability isolation, system priming, trust-tag propagation). Verdicts target a security-research framework that must **read** untrusted source code, run on heterogeneous backends (Claude / Gemini / OpenAI / Ollama), and dispatch via subprocess (`claude -p`) and SDK calls.
...
**Applicability to MANTISHACK.** Vendor-side; we don't fine-tune Claude/Gemini/GPT. **However**, Meta SecAlign 70B is Ollama-pullable and is the only production-grade prompt-injection-hardened open model today. Worth a profile entry: if the user runs SecAlign-Llama, MANTISHACK can use the *real* `[MARK_INST]/[MARK_INPT]` delimiters instead of the generic envelope.
...
**Applicability to MANTISHACK.** Conceptually the closest match for our `/agentic` and `/crash-analysis` flows, but a full CaMeL implementation is a multi-month engineering project. The *transferable* idea: **the privileged LLM (orchestrator) should never see raw target source**. MANTISHACK's Stage A in `/validate` already approximates this — the orchestrator reads structured findings JSON, not the file. PR-3 (capability isolation) should make this rule explicit: "untrusted-readers never plan tool calls; planners never read raw target content."
...
**Applicability to MANTISHACK.** Cheap insurance for the local-model path. Add an optional post-envelope re-statement of the task, gated by `model_profile.requires_reiteration` (true for Ollama 7B-class models, false for frontier).
...
**Applicability to MANTISHACK.** Out of reach at the model level. **At the orchestration level, MANTISHACK already does this** via the `untrusted_blocks` parameter and trust-tag propagation across agent hops. Document it as "coarse-grained taint tracking at the prompt-construction boundary" and stop there.
...
**Applicability to MANTISHACK.** Requires forward-pass modification of the model. Not deployable for Claude/Gemini/GPT. *Could* be deployed for Ollama if we shipped a patched runtime, which we won't.
...
**Applicability to MANTISHACK.** **This is the technique you ruled out** ("classifier approaches → false positives on security-research corpora"). Worth re-examining: PromptArmor is not a classifier, it's an LLM doing the classification, and on AgentDojo's task suite (which includes *actual code* and tool outputs) its FPR is sub-1%. **However**, AgentDojo doesn't contain "files literally named `exploit.c` whose comments include `// ignore previous instructions and ...`" — which is exactly what MANTISHACK test corpora look like. The FPR claim does not transfer. The 2026 reconsideration question is real but the answer is still no for MANTISHACK.
...
**Applicability to MANTISHACK.** Direct fit. Map MANTISHACK's existing structure to these patterns:
...
**Verdict.** **Adopt (architectural framing).** Don't add code; *adopt the vocabulary* in PR-3's capability-matrix doc. This is the language Anthropic and Google use; aligning with it makes MANTISHACK's architecture legible.
...
**Applicability to MANTISHACK.** Excellent fit for `/agentic`, `/crash-analysis`, `/oss-forensics`. Audit each agent against (A/B/C):
...
**MANTISHACK alignment.** Our `<untrusted-${nonce}>` envelope is **explicitly endorsed** by Anthropic's XML-tag guidance. Adopt the `<document>` *outer* wrapping for content from named files (so Claude sees a familiar pattern):
```
<document index="1">
...
**MANTISHACK alignment.** Two concrete asks from OpenAI's docs we don't currently honour:
...
**MANTISHACK alignment.** When the user runs Ollama with a SecAlign or Prompt-Guard-fronted model, our envelope should defer to the model's native delimiters. Per-model profile.
...
|---|---|---|---|
| **Jatmo** (ESORICS 2024) | Task-specific fine-tune that has no instruction-following channel at all | https://arxiv.org/abs/2312.17673 | **Skip** — MANTISHACK needs general instruction-following |
...
| **AgentDojo** | 97 tasks × 629 attack cases benchmark | https://arxiv.org/abs/2406.13352 | **Adopt as eval** — closest to MANTISHACK's agentic shape |
