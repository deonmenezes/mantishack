...
MANTISHACK sandboxes any subprocess that handles untrusted content — LLM-generated
...
| `run_trusted()` | MANTISHACK chose the command AND its inputs; no untrusted content flows into it | open | off | off | yes |
...
`run_trusted()` only when the full command line is MANTISHACK-owned and no
...
doesn't support it — MANTISHACK logs a warning once per layer per process.
...
non-zero `result.returncode` plus a `MANTISHACK: Landlock …` line on the child's
...
CLI: `--sandbox <profile>` on any MANTISHACK command that honours it.
...
- File operations performed via `io_uring` SQEs (submission queue
  ...
  by the same process are untraceable. MANTISHACK's seccomp blocklist
...
none of which evade observation. MANTISHACK is not a malware-analysis
...
| `use_egress_proxy` | `False` | Route outbound HTTPS through the MANTISHACK proxy with a hostname allowlist. See [Egress proxy](#egress-proxy). |
...
> is forwarded verbatim to the child — `MantishackConfig.get_safe_env()` is NOT
...
The cache is per-process: a fresh MANTISHACK invocation re-probes (so
...
- `restrict_reads=True` — Landlock blocks reads outside the system-dir
  ...
  - `/proc` is wholesale allowlisted, but cross-process
    ...
    `ANTHROPIC_API_KEY` and other credentials out of the parent MANTISHACK
...
- `cat ~/.config/mantishack/models.json` → ENOENT
...
MANTISHACK proxy forwards its `CONNECT` tunnels through that upstream. `NO_PROXY` /
...
2. **Manual** — for cases the auto-recovery doesn't cover (different
   ...
   ```bash
   # Single run.
   libexec/mantishack-sandbox-summary <run_dir>

   # All stranded runs under a project dir at once.
   libexec/mantishack-sandbox-summary --sweep <project_dir>
   ```
...
The subprocess-path sandbox interposes `libexec/mantishack-pid1-shim` so
