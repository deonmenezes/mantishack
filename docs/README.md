...
MANTISHACK is an autonomous security testing framework that combines static analysis, dataflow validation, and binary fuzzing with LLM-powered vulnerability analysis. It aims to autonomously identify, validate, and exploit security vulnerabilities with minimal human intervention.
...
MANTISHACK leverages Large Language Models to provide intelligent analysis, distinguishing true vulnerabilities from false positives, and generating working exploits and secure patches.



## Core Concept

Traditional security tools generate thousands of findings but lack context and exploitability assessment. MANTISHACK addresses this by:
...
MANTISHACK follows a modular architecture with clear separation of concerns:

```
MANTISHACK-daniel-modular/
...
Instead of requiring manual seed inputs, MANTISHACK can:
...
**ASan Support**:
When binaries are compiled with ASan (`-fsanitize=address`), MANTISHACK automatically:
...
**Entry Point**: `mantishack_agentic.py`
...
**Entry Point**: `mantishack_fuzzing.py`
...
└── logs/
    └── mantishack_<timestamp>.jsonl        # Structured logs
```
...
└── logs/
    └── mantishack_fuzzing_<timestamp>.jsonl
```
...
MANTISHACK is open source and welcomes contributions. Areas where help is needed:
...
MANTISHACK leverages excellent open source tools:
