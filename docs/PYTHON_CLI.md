# MANTISHACK Python CLI Reference
...
```bash
python3 mantishack.py agentic --repo /path/to/code --max-findings 10
```
...
python3 mantishack.py agentic --repo /path/to/code --understand --validate

# Same, via the libexec wrapper (avoids per-invocation Bash permission prompt)
libexec/mantishack-agentic --repo /path/to/code --understand --validate
```
...
```bash
python3 mantishack.py fuzz --binary /path/to/binary --duration 3600 --parallel 4
```
...
```bash
python3 mantishack.py analyze --repo /path/to/code --sarif findings.sarif --max-findings 10
```
...
# Optional
export MANTISHACK_ROOT="/path/to/mantishack"
export MANTISHACK_OUT_DIR="/custom/output/path"
```
