# MANTISHACK Fuzzing Mode - Quick Start Guide

## What It Does

MANTISHACK Fuzzing Mode combines AFL++ fuzzing with LLM-powered crash analysis to:
...
MANTISHACK supports multiple LLM providers with different quality levels:
...
MANTISHACK automatically detects and uses **AddressSanitizer (ASan)** builds for dramatically better crash diagnostics:
...
**MANTISHACK Enhancement**: When ASan is detected, MANTISHACK:
...
**NEW**: MANTISHACK can automatically generate intelligent seed inputs by analysing your binary, eliminating the need for manual corpus creation.
...
**Real test results** (mantishack_testbench):
...
This is OK! MANTISHACK will use QEMU mode (slower but works).
...
export ANTHROPIC_API_KEY=your_key_here
python3 mantishack_fuzzing.py --binary ./target --duration 300
