# mantishack-sca — Software Composition Analysis
...
The user-facing entry point is `bin/mantishack-sca`. It strips dangerous
...
bin/mantishack-sca /path/to/project

# Show fix plan (safe default — no files modified)
bin/mantishack-sca fix /path/to/project

# Apply fixes in-place
bin/mantishack-sca fix /path/to/project --apply

# Upgrade all deps to latest safe version
bin/mantishack-sca fix /path/to/project --apply --harden

# CI gate: exit 1 if findings above threshold
bin/mantishack-sca /path/to/project --skip-review --skip-triage \
    --fail-on-severity high --fail-on-kev

# CI gate against an existing findings.json (no re-scan)
bin/mantishack-sca render /path/to/findings.json \
    --fail-on-severity high --fail-on-kev
```

> **Note** — `libexec/mantishack-sca-run` is the internal dispatch script.
> It refuses to run unless invoked via `bin/mantishack-sca`, the MANTISHACK
> launcher, or Claude Code. If you need to call it directly (e.g.,
> custom CI), set `_MANTISHACK_TRUSTED=1` in the environment and ensure
...
|---|---|---|
| `findings.json` | MANTISHACK findings schema | other MANTISHACK commands (`/validate`, `/patch`) |
...
Registries supported: PyPI, npm, crates.io, RubyGems, Go (proxy.golang.org), Maven Central, Packagist, NuGet, Debian Sources, Homebrew. Run `mantishack-sca health` to probe all ten in one shot.
...
```yaml
- run: bin/mantishack-sca $PROJECT --skip-review --skip-triage \
       --fail-on-severity high --fail-on-kev
...
```yaml
- run: |
    bin/mantishack-sca $PROJECT --out before-${{github.sha}}
    bin/mantishack-sca fix $PROJECT --apply
    bin/mantishack-sca $PROJECT --out after-${{github.sha}}
    bin/mantishack-sca diff before-*/findings.json after-*/findings.json
```

### Pre-flight: registries reachable

```yaml
- run: bin/mantishack-sca health
