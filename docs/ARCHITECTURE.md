# MANTISHACK Modular Architecture
...
MANTISHACK (Recursive Autonomous Penetration Testing and Observation Robot) is a security testing framework that uses LLMs to autonomously analyse code for vulnerabilities, generate exploits, and create patches. The framework operates in three distinct modes:

1. **Source Code Analysis Mode**: Static analysis of source code using Semgrep (`mantishack_agentic.py`)
2. **Deep CodeQL Analysis Mode**: Advanced static analysis with dataflow validation (`mantishack_codeql.py`)
3. **Binary Fuzzing Mode**: Coverage-guided fuzzing of compiled binaries using AFL++ (`mantishack_fuzzing.py`)

Additionally, an interactive mode is available via `mantishack.py` (Claude Code integration) that provides conversational access to all capabilities with progressive loading of expert personas.
...
├── mantishack.py              # Main launcher (Claude Code integration)
├── mantishack_agentic.py      # Source code analysis workflow
├── mantishack_codeql.py       # CodeQL workflow
└── mantishack_fuzzing.py      # Binary fuzzing workflow
```
...
│   ├── config/                     # MantishackConfig (paths, settings)
...
├── mantishack.py                       # Main launcher (Claude Code integration)
├── mantishack_agentic.py               # Source code analysis workflow
├── mantishack_codeql.py                # CodeQL workflow orchestrator
├── mantishack_fuzzing.py               # Binary fuzzing workflow
...
#### `core/config/` - MantishackConfig
**Responsibility**: Centralized configuration management

```python
class MantishackConfig:
    @staticmethod
    def get_mantishack_root() -> Path:
        """Get MANTISHACK installation root"""

    @staticmethod
    def get_out_dir() -> Path:
        """Get output directory (mantishack/out/)"""
...
- Environment variable support (MANTISHACK_ROOT)
...
```python
def get_logger(name: str = "mantishack") -> logging.Logger:
...
- Timestamped log files (mantishack_<timestamp>.jsonl)
...
```json
{
  ...
  "function": "info",
  "line": 111,
  "message": "MANTISHACK logging initialized - audit trail: /path/to/mantishack_1762658520.jsonl"
}
```
...
**Entry Point**: Also accessible via `mantishack_codeql.py` for full workflow
...
**Main Entry Point**: `cli.py` (invoked via the `libexec/mantishack-sca-run`
shim or `python3 mantishack.py sca`)

**CLI Interface**:
```bash
# Recommended — wraps the run-lifecycle helpers + project resolution.
libexec/mantishack-sca-run /path/to/code --out /path/to/output

# Equivalent direct invocation:
python3 -m packages.sca.cli /path/to/code --out /path/to/output

# Or via the unified launcher:
python3 mantishack.py sca --repo /path/to/code --out /path/to/output
```

Sub-commands (`fix`, `check`, `upgrade`, `diff`, `verify`, `health`,
`purl`, `render`, `clean-cache`) are documented in
`.claude/commands/mantishack-sca.md`. Threshold-based CI gating is exposed
...
**Usage**:
- Loaded on-demand by `mantishack.py` (Claude Code integration)
...
- Integration with all MANTISHACK packages
...
### `mantishack_agentic.py` - Full Workflow Orchestrator
...
**Outputs**:
- `mantishack_agentic_report.json` - End-to-end summary
...
**Mode Selection**:
MANTISHACK operates in two mutually exclusive modes:
...
MANTISHACK's exploit generation capabilities vary significantly based on the LLM provider used. Understanding these differences is critical for production deployments.
