# Extending the MANTISHACK Unified Launcher

This guide explains how to add new security scanning engines or tools to the unified `mantishack.py` launcher.

## Overview

The unified launcher (`mantishack.py`) provides a single entry point for all MANTISHACK capabilities. Adding a new engine is straightforward and requires minimal code changes.

## Benefits of the Unified Launcher

1. **Single Entry Point**: Users only need to remember `python3 mantishack.py <mode>`
...
# Use the canonical MANTISHACK_DIR env var (set by bin/mantishack and the
...
# existing entry-point script (mantishack.py, mantishack_agentic.py, etc.).
sys.path.insert(0, os.environ["MANTISHACK_DIR"])
...
### Step 3: Add Mode Handler to mantishack.py

Open `mantishack.py` and add a new mode handler function:
...
In the `main()` function of `mantishack.py`, add your mode to the `mode_handlers` dictionary:
...
Examples:
  ...
  # My custom scanner
  python3 mantishack.py myscan --target /path/to/target
```
...
```bash
# Test help
python3 mantishack.py myscan --help

# Test execution
python3 mantishack.py myscan --target /path/to/target

# Test mode-specific help
python3 mantishack.py help myscan
```
...
# Same MANTISHACK_DIR-from-env pattern as the example above —
...
def main():
    ...
    # Save results
    out_dir = Path(args.out) if args.out else MantishackConfig.get_out_dir() / "dependency-scan"
...
Import and use MANTISHACK's core utilities:

```python
from core.config import MantishackConfig  # For paths
...
- Save outputs to `MantishackConfig.get_out_dir() / "your-scanner-name/"`
- Use structured formats (JSON, SARIF)
- Include timestamps in output filenames
- Log to MANTISHACK's audit trail
...
python3 mantishack.py myscan

# Test actual execution
python3 mantishack.py myscan --target /tmp/test-target
```

### 3. Verify Banner Display

Ensure the ASCII mantishack banner displays correctly:

```bash
python3 mantishack.py myscan --help | head -20
```
...
2. Mode handler in `mantishack.py`
3. Updated help text in `mantishack.py`
...
2. Review `mantishack.py` to understand the routing pattern
3. Open an issue on GitHub with questions
4. Refer to `docs/ARCHITECTURE.md` for system design

## Summary

Adding a new scanner to MANTISHACK is simple:

1. Create `packages/my-scanner/agent.py`
2. Add `mode_my_scanner()` function to `mantishack.py`
3. Register in `mode_handlers` dictionary
4. Update help text
5. Test!

The unified launcher makes it easy to expand MANTISHACK's capabilities while maintaining a consistent user experience.
