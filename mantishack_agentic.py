#!/usr/bin/env python3
"""
MANTISHACK Truly Agentic Workflow
...
def run_command_streaming(
    ...
    def stream_output(pipe, storage, prefix=""):
        ...
        try:
            ...
            # of the call stack).
            sentinel = (
                f"[MANTISHACK stream_output reader aborted: "
    ...
    # Phase B credential-isolation: when mantishack_agentic.py was
    # itself spawned with a dispatcher session (MANTISHACK_LLM_SOCKET +
    # MANTISHACK_LLM_TOKEN_FD), relay the session to the grandchild so
    ...
    child_env = MantishackConfig.get_safe_env()
    child_pass_fds: list[int] = []
    if os.environ.get("MANTISHACK_LLM_SOCKET"):
        try:
            from core.llm.dispatcher.client import relay_for_grandchild
            socket_path, token_fd = relay_for_grandchild()
            child_env["MANTISHACK_LLM_SOCKET"] = socket_path
            child_env["MANTISHACK_LLM_TOKEN_FD"] = str(token_fd)
    ...
    try:
        ...
        # Operators saw MANTISHACK "complete" then HANG at exit
        ...
        # ``MantishackConfig.DEFAULT_TIMEOUT`` may itself be ``None``
...
def _crash_context_to_validate_finding(context, replay: list[dict] | None = None) -> dict:
    ...
    return {
        ...
        "ruling": {
            "status": "confirmed",
            "reason": "Crash reproduced during MANTISHACK fuzzing and analysed by CrashAnalyser.",
        },
        "crash": {
...
def _run_fuzz_validation_smoke(findings_path: Path, target: Path, out_dir: Path) -> dict:
    ...
    helper = Path(__file__).resolve().parent / "libexec" / "mantishack-validation-helper"
    ...
    if proc.returncode != 0 or not report_path.exists():
        ...
        return {
            "ran": False,
            "reason": f"mantishack-validation-helper exited {proc.returncode}",
...
def main():
    parser = argparse.ArgumentParser(
        description="MANTISHACK Agentic Security Testing - Scan, Analyse, Exploit, Patch",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Full autonomous workflow (Semgrep + CodeQL - default when called via unified launcher)
  python3 mantishack.py agentic --repo /path/to/code

  # Semgrep only
  python3 mantishack_agentic.py --repo /path/to/code --no-codeql --policy-groups crypto,secrets

  # CodeQL only (skip Semgrep)
  python3 mantishack_agentic.py --repo /path/to/code --codeql-only --languages java

  # With custom build command
  python3 mantishack_agentic.py --repo /path/to/code --codeql --languages java \\
    --build-command "mvn clean compile -DskipTests"

  # Limit number of findings processed
  python3 mantishack.py agentic --repo /path/to/code --max-findings 20

  # Skip exploit generation (analysis + patches only)
  python3 mantishack.py agentic --repo /path/to/code --no-exploits

  # Skip exploitability validation (faster, but may include false positives)
  python3 mantishack.py agentic --repo /path/to/code --skip-dedup

  # Focus validation on specific vulnerability type
  python3 mantishack.py agentic --repo /path/to/code --vuln-type sql_injection

  # Choose a specific analysis model (overrides models.json auto-detection)
  python3 mantishack.py agentic --repo /path/to/code --model gemini-2.5-pro
  ...
  python3 mantishack.py agentic --repo /path/to/code --model gemini-2.5-pro \\
    ...
    parser.add_argument(
        "--repo", default=os.environ.get("MANTISHACK_CALLER_DIR"),
        help=(
            "Path to repository to analyse (default: $MANTISHACK_CALLER_DIR "
            "— set by the bin/mantishack wrapper to the operator's cwd at "
            "launch time. When the script is invoked directly without "
            "the wrapper, MANTISHACK_CALLER_DIR is unset and --repo is "
            "required)."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=MantishackConfig.DEFAULT_TIMEOUT, metavar="SECONDS",
        help=(
            ...
            "MantishackConfig.DEFAULT_TIMEOUT). Set to 0 to disable the "
    ...
    parser.add_argument(
        ...
        help="Disable IRIS-style dataflow validation entirely. By default, "
             ...
             "and MANTISHACK-shipped LocalFlowSource queries against the project "
    ...
    # sentinel — set MantishackConfig.DEFAULT_TIMEOUT to None so
    # downstream subprocess calls that use the named constant
    # (or that read ``args.phase_timeout or None``) all see the
    # operator's choice. Same pattern as mantishack_codeql.py for
    # cross-command consistency.
    if args.phase_timeout != MantishackConfig.DEFAULT_TIMEOUT:
        MantishackConfig.DEFAULT_TIMEOUT = args.phase_timeout if args.phase_timeout > 0 else None
    ...
    if not args.repo:
        parser.error("--repo is required (or launch via `mantishack` from the target directory)")
    if not Path(args.repo).exists():
        parser.error(f"--repo path does not exist: {args.repo}")

    # Resolve paths
    script_root = Path(__file__).parent.resolve()  # MANTISHACK-daniel-modular directory
    ...
    if not git_dir.exists():
        ...
        try:
            ...
            import shutil
            import tempfile
            temp_dir = Path(tempfile.mkdtemp(prefix="mantishack_git_"))
            ...
            # paths in the except handlers below, leaking mantishack_git_*/ under
            ...
            git_safe = ["-c", "core.hooksPath=/dev/null",
                        ...
                        "-c", "user.name=mantishack",
                        "-c", "user.email=mantishack@local"]
            ...
            if result.returncode == 0:
                ...
                sandbox_run(
                    ["git"] + git_safe + ["commit", "-m", "MANTISHACK scan snapshot"],
                    block_network=True,
                    cwd=temp_repo, capture_output=True, timeout=60, env=env
                )
                repo_path = temp_repo
    ...
    # Parent (MANTISHACK_DIR/out/, project dir, or --out target's parent) is
    # mantishack-controlled — plain mkdir is fine. The leaf is the predictable
    ...
    logger.info("=" * 70)
    logger.info("MANTISHACK AGENTIC WORKFLOW STARTED")
    logger.info("=" * 70)
    ...
    # this, `python mantishack.py agentic --audit` would set audit mode in
    ...
    if run_semgrep:
        ...
        # ``semgrep_cmd`` is a list of MANTISHACK-constructed argv;
        # env inherits from MANTISHACK's own process (the operator's
        ...
        semgrep_proc = subprocess.Popen(
            ...
            env=MantishackConfig.get_safe_env(include_python_user_base=True),
    ...
    if run_codeql:
        ...
        # nosemgrep: python.lang.security.audit.dangerous-subprocess-use-tainted-env-args.dangerous-subprocess-use-tainted-env-args
        # Explicit ``env=MantishackConfig.get_safe_env()`` — strips
        ...
        try:
            ...
            try:
                ...
                _save_json(
                    ...
                    else MantishackConfig.get_out_dir() / ".semgrep_timeout",
    ...
    if sca_agent:
        ...
        if not source_scan_empty:
            logger.info("mantishack-sca not installed — skipping SCA phase")
    ...
    print("\n" + "=" * 70)
    print("🎉 MANTISHACK AGENTIC WORKFLOW COMPLETE")
    print("=" * 70)
    ...
    report_file = out_dir / "mantishack_agentic_report.json"
    ...
    print("\n" + "=" * 70)
    print("MANTISHACK has autonomously:")
    ...
    spec = build_findings_spec(
        analysed_results,
        title="MANTISHACK Agentic Security Report",
        metadata=metadata,
