...
- **`source_sink_scan`**: fast, dependency-free regex proxy for attacker-controlled-input sources (`req.query`, `request.args`, `os.Args`, ...) and dangerous sinks (`eval`, `exec`, `innerHTML`, `pickle.loads`, CRLF/HTTP response-header-injection sinks (CWE-113) like `res.setHeader(...)`, `response.headers[...] = ...`, `w.Header().Set(...)`, `response.addHeader(...)`, ...) across JS/TS, Python, Go, and Java. This is a **recall** tool, not proof -- it will surface sources and sinks in the same file or project without knowing if they're actually connected. Use it to cheaply widen your candidate list early in Recon/Detect, then manually trace whether a specific source really flows to a specific sink. Header-injection sinks fire on every header-set call site regardless of whether the value argument is attacker-controlled -- still trace it back to a source (and check for CRLF stripping) before treating it as reachable.
