...
- **`source_sink_scan`**: fast, dependency-free regex proxy for attacker-controlled-input sources (`req.query`, `request.args`, `os.Args`, ...) and dangerous sinks (`eval`, `exec`, `innerHTML`, `pickle.loads`, string-built SQL in `.query()`/`.execute()`/`db.Query()`, ...) across JS/TS, Python, Go, and Java. This is a **recall** tool, not proof -- it will surface sources and sinks in the same file or project without knowing if they're actually connected. The SQL sinks (CWE-89) only fire on string-concatenation/interpolation shapes, not on parameterized calls (`.query(sql, params)`), to keep noise down while still catching the classic injection pattern. Use it to cheaply widen your candidate list early in Recon/Detect, then manually trace whether a specific source really flows to a specific sink.
