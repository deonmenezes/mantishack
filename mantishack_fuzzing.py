...
""",
    ...
    ap.add_argument("--memory-file", help="Path to memory file for learning persistence (default resolves to ${HOME}/.mantishack/fuzzing_memory.json — note: under 'sudo -E' HOME expands to root, not the operator's home)")
    ...
    if args.prepare_corpus:
        ...
        if args.seed_out:
            ...
            seed_out = MantishackConfig.get_out_dir() / f"fuzz_seeds_{source_dir.name}"
        try:
    ...
    # Anchor default output_dir to MantishackConfig.get_out_dir().
    # Pre-fix `Path(f"out/fuzz_...")` was relative to the cwd at
    # script-launch time. Two failure modes:
    #   * Operator running MANTISHACK from `~/work/foo/` got fuzz
    ...
    if args.out:
        ...
        out_dir = MantishackConfig.get_out_dir() / f"fuzz_{binary_path.stem}_{unique_run_suffix()}"
    ...
    logger.info("=" * 70)
    logger.info("MANTISHACK FUZZING WORKFLOW STARTED")
    logger.info("=" * 70)
    ...
    if args.autonomous:
        ...
        # ``~/.mantishack/fuzzing_memory.json`` ends up in the wrong
    ...
    # ========================================================================
    print("\n" + "=" * 70)
    print("MANTISHACK FUZZING COMPLETE")
    print("=" * 70)
    print("\n Summary:")
