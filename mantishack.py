#!/usr/bin/env python3
"""
MANTISHACK - Unified Security Testing Launcher

Single entry point for all MANTISHACK capabilities:
...
Examples:
    ...
    python3 mantishack.py scan --repo /path/to/code --policy-groups secrets,owasp

    # Binary fuzzing
    python3 mantishack.py fuzz --binary /path/to/binary --duration 3600

    # Web scanning
    python3 mantishack.py web --url https://example.com

    # CodeQL analysis
    python3 mantishack.py codeql --repo /path/to/code --languages java
"""
...
def _unpack_archive_target(target: str, args: list, out_dir: Path):
    ...
    if canonical.exists():
        ...
        try:
            ...
            # gracefully — never crash mantishack with a traceback.
...
def _run_with_lifecycle(command: str, script_path: Path, args: list,
    ...
    # otherwise create .mantishack-run.json along an attacker symlink.
...
def _get_or_start_dispatcher():
    """Lazy single dispatcher per ``mantishack.py`` invocation.

    Phase B credential-isolation: when this is called, the spawned
    analysis script gets ``MANTISHACK_LLM_SOCKET`` + a per-spawn token
    ...
    try:
        ...
        # keys in ~/.config/mantishack/models.json (the documented UX the
        ...
        _active_dispatcher = LLMDispatcher(
            run_id=f"mantishack-{uuid.uuid4().hex[:8]}",
            creds=creds,
        )
        ...
        import sys as _sys
        msg = (
            f"mantishack.py: credential-isolation dispatcher failed to "
...
def _run_script(script_path: Path, args: list) -> int:
    """
    Run a MANTISHACK script with given arguments.
    ...
    try:
        ...
        # exists until Phase C); ``MANTISHACK_LLM_SOCKET`` and
        # ``MANTISHACK_LLM_TOKEN_FD`` direct the worker's SDK calls
        ...
        if dispatcher is not None:
            ...
            proc = spawn_worker(
                ...
                # ``mantishack_<mode>.py`` subprocess so its own opt-in
                # at ``get_safe_env(include_python_user_base=True)``
                # (e.g. ``mantishack_agentic.py:757`` semgrep spawn)
        ...
        # PYTHONUSERBASE for the spawned ``mantishack_<mode>.py``
...
def mode_sca(args: list) -> int:
    ...
    for i, arg in enumerate(args):
        ...
        if arg == "--repo" and i + 1 < len(args):
            if repo_seen:
                print("mantishack.py sca: --repo specified more than once; "
    ...
    try:
        from core.config import MantishackConfig
        # Trust marker — libexec/mantishack-sca-run refuses to run without
        # one of CLAUDECODE / _MANTISHACK_TRUSTED in env. ``get_safe_env``'s
        # allowlist (in this branch) doesn't include the markers, so we
        # set the trust marker explicitly here. ``mantishack.py`` is itself
        ...
        print(f"\n✗ Error running mantishack-sca: {e}")
        return 1
...
def mode_fuzz(args: list) -> int:
    ...
    script_root = Path(__file__).parent
    fuzzing_script = script_root / "mantishack_fuzzing.py"
...
def mode_agentic(args: list) -> int:
    ...
    script_root = Path(__file__).parent
    agentic_script = script_root / "mantishack_agentic.py"
...
def mode_codeql(args: list) -> int:
    ...
    script_root = Path(__file__).parent
    codeql_script = script_root / "mantishack_codeql.py"
...
def show_mode_help(mode: str) -> None:
    ...
    mode_scripts = {
        ...
        'fuzz': script_root / "mantishack_fuzzing.py",
        'web': script_root / "packages/web/scanner.py",
        'agentic': script_root / "mantishack_agentic.py",
        'codeql': script_root / "mantishack_codeql.py",
    ...
    # PYTHONPATH through to the spawned `python3 mantishack_<mode>.py
    # --help` — irrelevant for the help text itself but a
    # consistency hazard with the rest of mantishack.py's spawn paths
...
not before; the top-level parser does not declare them directly):
  ...
  Run ``python3 mantishack.py <mode> --help`` to see them in the mode's
  own argparse-generated list (they are added by
  ``core.sandbox.add_cli_args``, not the top-level parser).

  # Examples
  python3 mantishack.py agentic --repo /code --audit          # log + run
  python3 mantishack.py scan --repo /code --sandbox debug     # gdb-friendly
  python3 mantishack.py fuzz --binary /b --audit --audit-verbose  # full trace

  # Get help for a specific mode
  python3 mantishack.py help scan
...
def main():
    """Main entry point for unified MANTISHACK launcher."""
    # Pre-process --trust-repo at the top level so it works in any position
    # (`mantishack --trust-repo scan /x` or `mantishack scan /x --trust-repo`).
    ...
    if len(sys.argv) == 1:
        parser = argparse.ArgumentParser(
            description="MANTISHACK - Unified Security Testing Launcher",
    ...
    if mode == 'help':
        if remaining:
            ...
            print("Usage: mantishack.py help <mode>")
            print("Example: mantishack.py help scan")
        return 0
    ...
    if mode not in mode_handlers:
        ...
        print("\nRun 'python3 mantishack.py --help' for more information", file=sys.stderr)
        return 1
