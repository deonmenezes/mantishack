...
def probe_anthropic() -> HealthResult:
    ...
    or the dispatcher route (``MANTISHACK_LLM_SOCKET`` set) as a valid
    ...
    if not api_key and not via_dispatcher:
        ...
        return HealthResult(
            ...
            detail=(
                ...
                "MANTISHACK_LLM_SOCKET for credential-isolation dispatcher)"
            ),
        )
    if not api_key:
...
def probe_github() -> HealthResult:
    ...
    try:
        ...
        gh_env = MantishackConfig.get_safe_env()
