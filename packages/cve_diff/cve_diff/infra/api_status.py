...
# ``core.config.MantishackConfig.LLM_API_KEY_VARS`` so adding a new
...
def _llm_provider_env_vars() -> tuple[str, ...]:
    ...
    try:
        ...
        return tuple(MantishackConfig.LLM_API_KEY_VARS)
    except Exception:
        return ()
...
def llm_auth_status() -> tuple[bool, int, bool]:
    ...
    *count* before this function returns — the env-var names
    ...
    vars (the central :data:`core.config.MantishackConfig.LLM_API_KEY_VARS`
    stays the only source of truth).
    """
    via_dispatcher = bool(os.environ.get("MANTISHACK_LLM_SOCKET"))
    num_configured = sum(
...
def render_startup_banner() -> str:
    ...
    if via_dispatcher:
        lines.append(
            ...
            "(MANTISHACK_LLM_SOCKET set — provider keys handled by "
