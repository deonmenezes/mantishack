...
Covers:
  ...
  * MANTISHACK_LLM_SOCKET takes precedence (dispatcher route)
...
def _clean_llm_env(monkeypatch):
    """Strip every LLM-provider env var + MANTISHACK_LLM_SOCKET so each
...
def test_dispatcher_socket_overrides_other_paths(
    ...
    """``MANTISHACK_LLM_SOCKET`` set → dispatcher route wins over both
    env-direct and Claude Code fallback. The dispatcher's
    ``CredentialStore`` handles auth; cve-diff doesn't see keys."""
    monkeypatch.setenv("MANTISHACK_LLM_SOCKET", "./fake.sock")
...
def test_dispatcher_skips_claudecode_fallback_for_anthropic():
    ...
    os.environ["MANTISHACK_LLM_SOCKET"] = "./fake.sock"
    try:
...
def test_auth_module_does_not_enumerate_provider_env_vars():
    ...
    ``core.config.MantishackConfig.LLM_API_KEY_VARS`` (single source of
    truth). The resolver looks at exactly two env vars:
    ``ANTHROPIC_API_KEY`` (for the CC fallback decision) and
    ``MANTISHACK_LLM_SOCKET`` (for dispatcher routing). Pin this so a
    ...
    assert not leaked, (
        ...
        f"(MantishackConfig.LLM_API_KEY_VARS) should be the only place "
        f"that lists them."
    )
