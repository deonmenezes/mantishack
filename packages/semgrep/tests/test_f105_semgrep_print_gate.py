...
`mantishack_agentic.py` prints "✓ Scanned with Semgrep" in the final
...
This test parses `mantishack_agentic.py` with ``ast`` (avoiding the
...
# Anchor to this file, not $MANTISHACK_DIR, so the test always reads the
# mantishack_agentic.py in its own worktree (MANTISHACK_DIR may point elsewhere).
MODULE_PATH = Path(__file__).resolve().parents[3] / "mantishack_agentic.py"
