...
MANTISHACK_AGENTIC = REPO_ROOT / "mantishack_agentic.py"
...
class PhaseTimeoutArgparseTests(unittest.TestCase):
    ...
    def _run_help(self) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, str(MANTISHACK_AGENTIC), "--help"],
            capture_output=True, text=True, timeout=15,
            env={**os.environ, "MANTISHACK_DIR": str(REPO_ROOT)},
        )
...
class MantishackCodeqlPhaseTimeoutTests(unittest.TestCase):
    """`/codeql --phase-timeout` parses + mutates MantishackConfig.CODEQL_TIMEOUT.

    Different plumbing pattern from /agentic: /codeql's subprocess calls
    live inside ``packages/codeql/`` package code which reads
    ``MantishackConfig.CODEQL_TIMEOUT``. The CLI flag mutates that constant
    at startup so package-internal calls pick up the override without
    per-call argument plumbing.
    """

    MANTISHACK_CODEQL = REPO_ROOT / "mantishack_codeql.py"

    def _run_help(self) -> subprocess.CompletedProcess:
        return subprocess.run(
            [sys.executable, str(self.MANTISHACK_CODEQL), "--help"],
            capture_output=True, text=True, timeout=15,
            env={**os.environ, "MANTISHACK_DIR": str(REPO_ROOT)},
        )
    ...
    def test_help_text_mentions_MantishackConfig_default(self):
        """The default should reference the MantishackConfig constant so
...
class RunCommandStreamingTimeoutTests(unittest.TestCase):
    ...
    def _import_helper(self):
        ...
        if str(REPO_ROOT) not in sys.path:
            ...
            path_added = True
        try:
            from mantishack_agentic import run_command_streaming
