"""Tests for the libexec/mantishack-agentic bash wrapper."""
...
WRAPPER = REPO_ROOT / "libexec" / "mantishack-agentic"


class MantishackAgenticWrapperTests(unittest.TestCase):
    ...
    def test_wrapper_passes_through_help_to_argparse(self):
        # --help must be handled by mantishack_agentic.py argparse, not by the
    ...
    def test_wrapper_fails_loudly_when_mantishack_dir_invalid(self):
        # If the wrapper is somehow installed where ../mantishack.py doesn't
        ...
        with TemporaryDirectory() as tmp:
            broken = Path(tmp) / "fake-libexec" / "mantishack-agentic"
            broken.parent.mkdir()
            # Copy the wrapper but NOT mantishack.py — the parent dir is empty.
            ...
            self.assertNotEqual(proc.returncode, 0)
            self.assertIn("cannot find mantishack.py", proc.stderr,
                          msg=f"expected explanatory error, got: {proc.stderr[:200]}")

    def test_wrapper_works_via_symlink(self):
        # The wrapper walks $0 symlinks so MANTISHACK_DIR resolves correctly even
        # when invoked via a symlink in ~/bin or similar.
        with TemporaryDirectory() as tmp:
            link = Path(tmp) / "mantishack-agentic"
