...
Not an active issue in MANTISHACK right now. CrashAnalyser validates upstream
...
class GDBDebugger:
    ...
    def run_commands(self, commands: List[str], input_file: Optional[Path] = None, timeout: int = 30) -> str:
        ...
        # write (ENOSPC, I/O error, etc.) would leak /tmp/.mantishack_gdb_*.txt
        ...
        #
        # `dir=` to the binary's parent dir (MANTISHACK-controlled
        ...
        # to OTHER users via `ls /tmp/.mantishack_gdb_*` and (since
        ...
        fd, script_name = tempfile.mkstemp(
            prefix=".mantishack_gdb_", suffix=".txt", dir=str(binary_dir),
        )
