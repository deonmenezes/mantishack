...
class TestDebuggerTempFile:
    ...
    def test_no_predictable_path(self, debugger, tmp_path):
        """Script file should NOT be at /tmp/mantishack_gdb_script.txt."""
        script_paths = []
        ...
        assert script_paths
        assert script_paths[0] != "/tmp/mantishack_gdb_script.txt"
        assert ".mantishack_gdb_" in script_paths[0]
    ...
    def test_temp_file_cleaned_up_on_error(self, debugger, tmp_path):
        ...
        # with prefix `.mantishack_gdb_` (per debugger.py
        # tempfile.mkstemp call). After a failed run, the dir
        # should contain ZERO `.mantishack_gdb_*.txt` leftovers.
        #
        ...
        leftover_scripts = list(binary_dir.glob(".mantishack_gdb_*.txt"))
