...
MANTISHACK workflows, optionally decompiles high-value functions through
...
class BinaryUnderstand:
    ...
    def analyse(
        ...
        _wrapper = (
            ...
            / "libexec" / "mantishack-r2-sandboxed"
        )
        if not _wrapper.is_file():
            raise RuntimeError(
                f"r2 sandbox wrapper missing: {_wrapper}. "
                f"Reinstall MANTISHACK or check libexec/ is intact."
            )
...
def analyse_binary_context(
    ...
    *,
    ...
    This is the shared entry point other MANTISHACK commands should use instead
