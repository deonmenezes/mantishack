...
- The output of this could be consumed by MANTISHACK or other tools for further analysis for finding bugs/security issues
"""
import argparse
import json
import os
...
def run(cmd, cwd=None, timeout=MantishackConfig.DEFAULT_TIMEOUT, env=None,
    ...
    Network policy:
      ...
      - proxy_hosts=[...] set: route outbound via the MANTISHACK egress
    ...
    p = sandbox_run(
        ...
        output=output,
        cwd=cwd,
        env=env or MantishackConfig.get_safe_env(),
        text=True,
        capture_output=True,
        timeout=timeout,
...
def run_single_semgrep(
    ...
    def sanitize_name(name: str) -> str:
        ...
        # `MantishackConfig.BASELINE_SEMGREP_PACKS` /
    ...
    cmd = semgrep_pkg.build_cmd(
        ...
        rule_timeout=MantishackConfig.SEMGREP_RULE_TIMEOUT,
    ...
    clean_env = MantishackConfig.get_safe_env()
    ...
    effective_timeout = min(timeout, MantishackConfig.SEMGREP_PACK_TIMEOUT) if is_registry_pack else timeout

    try:
        # Engage Landlock via target + output. Writes pinned to out_dir
        # and /tmp. Reads Landlock-default-wide (semgrep is a
        # MANTISHACK-chosen trusted tool, not attacker-controlled code).
...
def semgrep_scan_parallel(
    ...
    rules_dirs: List[str],
    out_dir: Path,
    timeout: int = MantishackConfig.SEMGREP_TIMEOUT,
    progress_callback: Optional[Callable] = None
) -> List[str]:
    """
    ...
    for rd in rules_dirs:
        ...
        if rd_path.exists():
            ...
            if category_name in MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK:
                pack_name, pack_id = MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK[category_name]
                if pack_id not in added_packs:
                    resolved = MantishackConfig.get_semgrep_config(pack_id)
    ...
    for pack_name, pack_identifier in MantishackConfig.BASELINE_SEMGREP_PACKS:
        if pack_identifier not in added_packs:
            configs.append((pack_name, MantishackConfig.get_semgrep_config(pack_identifier)))
            added_packs.add(pack_identifier)

    logger.info(f"Starting {len(configs)} Semgrep scans in parallel (max {MantishackConfig.MAX_SEMGREP_WORKERS} workers)")
    ...
    with ThreadPoolExecutor(max_workers=MantishackConfig.MAX_SEMGREP_WORKERS) as executor:
        future_to_config = {
            executor.submit(
                run_single_semgrep,
                name,
...
def run_codeql(
    ...
    agent as a subprocess (mirroring how mantishack_agentic.py runs it
...
def _shipped_cocci_rules_dir() -> Optional[Path]:
    ...
    live at ``engine/coccinelle/rules/`` — distributed with MANTISHACK,
    not generated."""
...
def main():
    ap = argparse.ArgumentParser(description="MANTISHACK Automated Code Security Agent with parallel scanning")
    ...
    ap.add_argument(
        ...
        default=MantishackConfig.DEFAULT_POLICY_VERSION,
        dest="policy_version",
    )
    ap.add_argument(
        "--policy-groups", "--policy_groups",
        default=MantishackConfig.DEFAULT_POLICY_GROUPS,
        dest="policy_groups",
    ...
    tmp = Path(tempfile.mkdtemp(prefix="mantishack_auto_"))
    repo_path = None
    ...
    try:
        ...
        rules_base = MantishackConfig.SEMGREP_RULES_DIR
        ...
        if args.out:
            ...
            out_dir = MantishackConfig.get_out_dir() / f"scan_{repo_name}_{unique_run_suffix('_')}"
        ...
        # silently dropped. The lifecycle hook in mantishack.py wires
        ...
        # sandbox-summary.json. The lifecycle hook lives in mantishack.py
        # / mantishack_agentic.py for top-level invocations — neither
