...
Resolution layers:
  ...
  1. **Operator override** — ``~/.config/mantishack/semgrep-proxy-hosts.json``
     with a flat ``{"hosts": [...]}`` list. Required for shops on
     Semgrep self-hosted / a corporate registry mirror.
  2. **Calibrated profile** — ``mantishack-sandbox-calibrate --bin
...
_OVERRIDE_CONFIG_PATH = (
    Path.home() / ".config" / "mantishack" / "semgrep-proxy-hosts.json"
)
...
def proxy_hosts_for_semgrep() -> list[str]:
    ...
    (``~/.config/mantishack/semgrep-proxy-hosts.json`` ``{"hosts": [...]}``)
