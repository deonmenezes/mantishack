...
pack name. PR #196 added ``MantishackConfig.get_semgrep_config`` to
...
from core.config import MantishackConfig  # noqa: E402
...
class TestSemgrepScanParallel:
    ...
    def test_baseline_packs_always_included(self, mock_single, tmp_path):
        ...
        for pack_name, _ in MantishackConfig.BASELINE_SEMGREP_PACKS:
    ...
    def test_known_category_adds_standard_pack(self, mock_single, tmp_path):
        ...
        #  MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK for the current set.)
        auth_dir = tmp_path / "auth"
        auth_dir.mkdir()
        semgrep_scan_parallel(tmp_path, [str(auth_dir)], tmp_path, timeout=10)
        called_names = [c.args[0] for c in mock_single.call_args_list]
        auth_pack_name = MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK["auth"][0]
...
class TestSemgrepScanSequential:
    ...
    def test_scans_run_in_order(self, mock_single, tmp_path):
        ...
        assert mock_single.call_count == len(MantishackConfig.BASELINE_SEMGREP_PACKS)
