...
class CodeQLAgent:
    ...
    def __init__(
        ...
        if out_dir:
            ...
            self.out_dir = MantishackConfig.BASE_OUT_DIR / f"codeql_{repo_name}_{_unique_run_suffix('_')}"
        ...
        logger.info(f"{'=' * 70}")
        logger.info("MANTISHACK CODEQL AGENT")
        logger.info(f"{'=' * 70}")
    ...
    def _save_report(self, result: CodeQLWorkflowResult):
        ...
        try:
            minimal = {
                ...
                "error": "to_dict() raised mid-serialization; see mantishack.log",
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MANTISHACK CodeQL Agent - Autonomous CodeQL Security Analysis",
...
Examples:
  ...
  python3 packages/codeql/agent.py --repo /path/to/code --force
    ...
    try:
        ...
        # complete_run) lives in mantishack.py / mantishack_codeql.py for
        # top-level invocations and in mantishack_agentic.py for the
