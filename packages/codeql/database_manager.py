...
class DatabaseManager:
    ...
    def __init__(self, db_root: Optional[Path] = None, codeql_cli: Optional[str] = None):
        ...
        Args:
            db_root: Root directory for databases (defaults to MantishackConfig.CODEQL_DB_DIR)
            codeql_cli: Path to CodeQL CLI (auto-detected if None)
        """
        self.db_root = db_root or MantishackConfig.CODEQL_DB_DIR
    ...
    def get_codeql_version(self) -> Optional[str]:
        ...
        """
        try:
            # `env=MantishackConfig.get_safe_env()` so the version probe
    ...
    def _staging_path(self, repo_hash: str, language: str) -> Path:
        ...
        **Process-safe, NOT thread-safe.** Two threads in the same process
        ...
        within the staging dir would race. MANTISHACK's parallelism model uses
    ...
    def _evict_stale_canonical(
        ...
        a grace period (`MantishackConfig.CODEQL_DB_MISSING_METADATA_GRACE`)
        ...
        if metadata is None:
            ...
            if age >= MantishackConfig.CODEQL_DB_MISSING_METADATA_GRACE:
                evict = True
        else:
            try:
                created_at = datetime.fromisoformat(metadata.created_at)
    ...
    def create_database(
        ...
        if build_system and build_system.env_vars:
            ...
            blocked = set(MantishackConfig.DANGEROUS_ENV_VARS + MantishackConfig.PROXY_ENV_VARS)
        ...
        if build_system and build_system.command:
            ...
            if Path(build_cmd).is_file() or re.fullmatch(r'[a-zA-Z0-9._-]+', build_cmd):
                ...
                # `.mantishack_codeql_build_*.sh` files in their git checkout —
                ...
                fd, script_name = tempfile.mkstemp(
                    prefix=".mantishack_codeql_build_", suffix=".sh", dir=str(tmp_dir),
                )
                os.close(fd)
        ...
        logger.info(f"Timeout: {MantishackConfig.CODEQL_TIMEOUT}s")
        ...
        try:
            ...
            result = sandbox_run(
                ...
                capture_output=True,
                text=True,
                timeout=MantishackConfig.CODEQL_TIMEOUT,
            ...
            errors.append(f"Database creation timed out after {MantishackConfig.CODEQL_TIMEOUT}s")
    ...
    def create_databases_parallel(
        ...
        Args:
            ...
            max_workers: Max parallel workers (default: MantishackConfig.MAX_CODEQL_WORKERS)
