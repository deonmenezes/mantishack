...
def _find_script(tmp_path):
    """Find the generated .mantishack_build_*.py script in tmp_path."""
    scripts = list(tmp_path.glob(".mantishack_build_*.py"))
...
class TestSynthesiseCpp:
    ...
    def test_build_dir_is_temp(self, tmp_path):
        ...
        bd.synthesise_build_command("cpp")
        script = _find_script(tmp_path).read_text()
        assert ".mantishack_build_" in script
    ...
    def test_build_dir_created_for_codeql(self, tmp_path):
        ...
        build_dirs = [p for p in tmp_path.glob(".mantishack_build_*") if p.is_dir()]
        scripts = [p for p in tmp_path.glob(".mantishack_build_*.py")]
...
class TestMissingConfigHeaderDetection:
    ...
    def test_synthesised_compile_logs_missing_config_warning(
        ...
        # returns a wrapper around the ``"mantishack"`` stdlib logger
        # (see core/logging/__init__.py:MantishackLogger). Attach the
        ...
        target_logger = logging.getLogger("mantishack")
