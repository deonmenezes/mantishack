...
layers (priority high → low):
  ...
  1. ``~/.config/mantishack/codeql-proxy-hosts.json`` — operator
  ...
  2. Calibrated SandboxProfile — when ``mantishack-sandbox-calibrate``
...
and the operator updates the override config or upgrades MANTISHACK.
"""
...
_OVERRIDE_CONFIG_PATH = (
    Path.home() / ".config" / "mantishack" / "codeql-proxy-hosts.json"
)
