...
class BuildSystem:
    ...
    env_vars: Dict[str, str]  # Env vars we inject as-is (MANTISHACK-chosen constants)
...
class BuildDetector:
    ...
    def synthesise_build_command(self, language: str) -> Optional[BuildSystem]:
        ...
        import tempfile
        build_dir = Path(tempfile.mkdtemp(prefix=".mantishack_build_", dir=self.repo_path))
        fd, script_name = tempfile.mkstemp(
            prefix=".mantishack_build_", suffix=".py", dir=self.repo_path,
        )
        os.close(fd)
...
"""Synthesised by MANTISHACK for CodeQL database creation.
