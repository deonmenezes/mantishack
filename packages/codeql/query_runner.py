...
def _iris_pack_deps_already_resolved(pack_dir: Path) -> bool:
    ...
    every IRIS analysis. Common case in normal MANTISHACK setups (where
...
class QueryRunner:
    ...
    def run_suite(
        ...
        cmd = [
            ...
            "--format=sarif-latest",
            f"--output={sarif_path}",
            f"--threads={MantishackConfig.CODEQL_THREADS}",
            f"--ram={MantishackConfig.CODEQL_RAM_MB}",
        ...
        logger.info(f"Timeout: {MantishackConfig.CODEQL_ANALYZE_TIMEOUT}s")
        ...
        try:
            ...
            result = sandbox_run(
                ...
                capture_output=True,
                text=True,
                timeout=MantishackConfig.CODEQL_ANALYZE_TIMEOUT,
            )
            ...
            if not success and "cannot be found" in (result.stderr or "").lower():
                ...
                if pack_name:
                    ...
                    # Route codeql through the MANTISHACK egress proxy.
                    ...
                    for attempt in range(3):
                        dl = sandbox_run(
                            ...
                            # ~/.config/mantishack/codeql-proxy-hosts.json
                            # short-circuits both.
            ...
            error = f"Analysis timed out after {MantishackConfig.CODEQL_ANALYZE_TIMEOUT}s"
            errors.append(error)
    ...
    def analyze_all_databases(
        ...
        """
        max_workers = max_workers or MantishackConfig.MAX_CODEQL_WORKERS
        results = {}
    ...
    def analyze_iris_packs(
        ...
        """Run MANTISHACK's in-repo IRIS LocalFlowSource packs against each
        ...
        # globally via `MantishackConfig.IRIS_TIER1_ENABLED = False`. /codeql
        # CLI's `--no-iris-tier1` flag flips this for a single invocation.
        if not MantishackConfig.IRIS_TIER1_ENABLED:
            logger.info("IRIS pack analysis skipped: IRIS_TIER1_ENABLED is False")
            return {}

        extras = list(MantishackConfig.EXTRA_CODEQL_PACK_ROOTS or [])
        if not extras:
            return {}
        # First entry is the canonical MANTISHACK-shipped pack root.
        pack_root = extras[0]
            ...
            for future in as_completed(future_to_lang):
                ...
                try:
                    ...
                    results[lang] = QueryResult(
                        ...
                        suite_name="mantishack-iris-local",
                    )
        return results
    ...
    def _run_iris_pack(
        ...
        # cache. In normal MANTISHACK setups (where `~/.codeql/packages/
        ...
        try:
            ...
            return QueryResult(
                ...
                errors=[str(e)], suite_name="mantishack-iris-local",
            )
        ...
        if proc.returncode == 0 and sarif_path.exists():
            ...
            return QueryResult(
                ...
                findings_count=n,
                duration_seconds=time.time() - analysis_start,
                errors=[], suite_name="mantishack-iris-local",
            )
