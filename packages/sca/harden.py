...
def main(argv: Sequence[str]) -> int:
    ...
    if args.trust_repo:
        try:
            ...
            logger.debug("mantishack-sca fix: cc_trust unavailable; "
                          "--trust-repo had no effect")

    target = Path(args.target).resolve()
    if not target.exists():
        print(f"mantishack-sca fix --harden: target does not exist: {target}",
              file=sys.stderr)
        return 2
    if not target.is_dir():
        print(f"mantishack-sca fix --harden: target is not a directory: {target}",
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"mantishack-sca fix --harden: cannot create output dir {out_dir}: {e}",
              file=sys.stderr)
        return 2
    ...
    if args.check:
        ...
        if actionable:
            print(f"mantishack-sca fix --harden --check: {actionable} candidate(s) would be "
                  f"applied; rerun without --check to apply.")
            return 1
        print("mantishack-sca fix --harden --check: project is hardened (no actionable candidates).")
        return 0
    ...
    if args.apply:
        ...
        rc = apply_patch_to_target(target, patch_path,
                                    caller_label="mantishack-sca fix --harden")
        if rc != 0:
            return rc
...
def _plan_one(
    ...
    *,
    ...
    # ``mantishack-sca fix --harden`` doesn't promote to a version that
    ...
    # ``mantishack-sca fix --cve-only``; a fast-flag here would just be
...
def _run_self_test(
    *,
    ...
    if patch_path is None or not patch_path.exists():
        print("mantishack-sca fix --harden --self-test: no patch generated; nothing to test.")
        return 0
    if not (target / ".git").exists():
        print(f"mantishack-sca fix --harden --self-test: target {target} is not a git "
    ...
    tmp_root = Path(tempfile.mkdtemp(prefix="mantishack-sca-self-test-"))
    ...
    try:
        ...
        if stash.returncode not in (0, 1) or stash.stderr.strip():
            print(f"mantishack-sca fix --harden --self-test: `git stash create` "
        ...
        if proc.returncode != 0:
            print(f"mantishack-sca fix --harden --self-test: git worktree add failed: "
        ...
        if proc.returncode != 0:
            print(f"mantishack-sca fix --harden --self-test: patch application failed: "
                  f"{proc.stderr or proc.stdout}", file=sys.stderr)
            return 6
        ...
        print(f"mantishack-sca fix --harden --self-test: post-apply candidates → {post_path}")

        if post_actionable > 0:
            print(f"mantishack-sca fix --harden --self-test: REGRESSION — {post_actionable} "
        ...
        print("mantishack-sca fix --harden --self-test: PASS — applying the patch closes "
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="mantishack-sca fix --harden",
    ...
    p.add_argument("--cache-root",
                   help="override default ~/.mantishack/cache/sca cache root")
...
def _write_report(
    ...
    lines = ["# mantishack-sca fix --harden report", ""]
...
def _print_summary(
    ...
    print(f"mantishack-sca fix: {len(candidates)} deps analysed, "
    ...
    print(f"mantishack-sca fix: candidates.json   {out_dir / 'candidates.json'}")
    print(f"mantishack-sca fix: report.md         {out_dir / 'report.md'}")
