...
def test_repo_label_renders_in_header(tmp_path: Path) -> None:
    ...
    text = render_pr_comment(
        report, repo_label="mantishack · pr#42 · sha=abc1234",
    )
    assert "mantishack · pr#42 · sha=abc1234" in text
...
def test_pr_comment_cli_flag_via_main(tmp_path: Path, capsys) -> None:
    """``mantishack-sca bump <target> --pr-comment`` flag routes
    ...
    assert rc == 0
    # The PR-comment shape carries the "Generated by mantishack-sca
    # bump" footer — text-mode render doesn't.
    assert "Generated by mantishack-sca bump" in out
