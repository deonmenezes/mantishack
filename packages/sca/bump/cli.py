"""``mantishack-sca bump <target>`` subcommand entrypoint.

Operator-facing flow:

* ``mantishack-sca bump <target>`` — dry-run; print verdict table
  for each ARG bump candidate.
* ``mantishack-sca bump <target> --apply`` — apply Clean-verdict
...
* ``mantishack-sca bump <target> --json`` — machine-readable
...
def main(argv: Sequence[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="header label for ``--pr-comment`` (default: "
             "'mantishack-sca bump'). Operators add commit SHAs / "
    ...
    parser.add_argument(
        ...
        help="Set the process-wide ``cc_trust`` override. NO "
             "behaviour change in mantishack-sca bump itself — bump's "
    ...
    if args.trust_repo:
        # Same wiring shape as ``mantishack-sca fix --harden`` and the
        ...
        try:
            ...
            logger.debug("mantishack-sca bump: cc_trust unavailable; "
                          "--trust-repo had no effect")

    target = args.target.resolve()
    if not target.exists():
        print(f"mantishack-sca bump: target does not exist: {target}",
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        logger.exception("mantishack-sca bump: unrecoverable error")
        print(f"mantishack-sca bump: {e}", file=sys.stderr)
        return 3
