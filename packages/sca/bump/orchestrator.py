...
def run_bump(
    ...
    *,
    ...
    # Load operator policy from .mantishack-sca-bump.yml (or use
...
def _enumerate_candidates(
    ...
    *,
    ...
    # pinned support only; SHA-pinned refs (mantishack's convention)
...
def _enumerate_inline_install_candidates(
    *,
    ...
    mantishack's own devcontainer pins semgrep / claude-code / etc.
...
def _is_major_bump(current: str, target: str) -> bool:
    ...
    Two cases count as major-equivalent:
      ...
      2. **Different ``minor`` while both are still at major
         ...
         minor for ``0.y.z`` ranges. MANTISHACK mirrors that
...
def render_report(report: BumpReport) -> str:
    ...
    a future commit)."""
    lines: List[str] = []
    lines.append(f"mantishack-sca bump: target {report.target}")
    ...
    if report.candidates:
        ...
        # mantishack: 8 CODEQL_VERSION rows + 3 github/codeql-action
