"""``.mantishack-sca-bump.yml`` operator policy file.
...
``.mantishack-sca-bump.yml`` dotfile or, to keep CI/bot config out of
the project root, at ``.github/sca/mantishack-sca-bump-policy.yml``
(mirrors ``.github/codeql/codeql-config.yml``). Root wins if both
exist. Schema is identical wherever it lives:

    # .mantishack-sca-bump.yml
    skip:
...
#   1. ``<target>/.mantishack-sca-bump.yml`` — root dotfile; the simple default
#      for any project (a non-GitHub repo may have no ``.github/``).
#   2. ``<target>/.github/sca/mantishack-sca-bump-policy.yml`` — for repos that
#      keep CI/bot config in per-tool ``.github/`` subdirs (mirrors the
#      existing ``.github/codeql/codeql-config.yml`` convention).
_POLICY_FILE = ".mantishack-sca-bump.yml"
_POLICY_FILE_GITHUB = ".github/sca/mantishack-sca-bump-policy.yml"
...
class BumpPolicy:
    ...
    # ``.mantishack-sca-bump.yml`` (or the corresponding CLI flag). When
...
def load_policy(target: Path) -> BumpPolicy:
    ...
    ``.mantishack-sca-bump.yml`` dotfile, then
    ``.github/sca/mantishack-sca-bump-policy.yml``. The first that exists wins.
