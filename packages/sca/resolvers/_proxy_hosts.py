...
metadata-only resolves) plus two override layers:
  ...
  1. **Operator override** — ``~/.config/mantishack/sca-proxy-hosts.json``
...
_OVERRIDE_CONFIG_PATH = (
    Path.home() / ".config" / "mantishack" / "sca-proxy-hosts.json"
)
...
def proxy_hosts_for_gradle() -> list:
    ...
    ``gradlew`` which MANTISHACK resolves separately at the call site).
    """
    return _resolve(
