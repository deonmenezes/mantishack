...
— even ``--dry-run`` is blocked. mantishack-sca scans run on operator
...
``<project>/.mantishack-sca-venv-{pid}/`` and is removed after the run.
...
class PipResolver:
    ...
    ``/tmp/mantishack-sca-venv-<pid>-<hash>/`` and running pip-tools we
    ...
    def proxy_hosts(self) -> list:
        ...
        (``~/.config/mantishack/sca-proxy-hosts.json`` ``"pip"`` key) →
    ...
    def _venv_dir(self, project_dir: Path) -> Path:
        ...
        return Path("/tmp") / f"mantishack-sca-venv-{_os.getpid()}-{proj_hash}"
    ...
    def dry_run_batch(
        ...
        venv_dir = Path(
            f"/tmp/mantishack-sca-venv-batch-{os.getpid()}-{proj_hash}"
        )
    ...
    def _build_batch_script(
        ...
        for i in range(len(manifests)):
            parts.append(f"echo '===MANTISHACK_BATCH_OUT_{i}==='")
            parts.append(f"cat {results_dir}/{i}.out 2>/dev/null || true")
            parts.append(f"echo '===MANTISHACK_BATCH_RC_{i}==='")
            parts.append(f"cat {results_dir}/{i}.rc 2>/dev/null || echo 99")
            parts.append(f"echo '===MANTISHACK_BATCH_ERR_{i}==='")
            parts.append(f"cat {results_dir}/{i}.err 2>/dev/null || true")
        parts.append("echo '===MANTISHACK_BATCH_END==='")
        return "\n".join(parts)
    ...
    def _parse_batch_output(
        ...
        if "===MANTISHACK_BATCH_END===" not in stdout:
        ...
        for i in range(len(manifests)):
            out_marker = f"===MANTISHACK_BATCH_OUT_{i}==="
            rc_marker = f"===MANTISHACK_BATCH_RC_{i}==="
            err_marker = f"===MANTISHACK_BATCH_ERR_{i}==="
            next_marker = (
                f"===MANTISHACK_BATCH_OUT_{i + 1}==="
                if i + 1 < len(manifests)
                else "===MANTISHACK_BATCH_END==="
            )
