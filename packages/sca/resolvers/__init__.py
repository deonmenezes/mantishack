...
ecosystem's resolver in a uniform Protocol so ``mantishack-sca fix`` can
...
def _check_tool(cmd: list, *, timeout: int = 5) -> bool:
    ...
    Runs unsandboxed: the probe is a MANTISHACK-chosen command (``<tool>
...
def _run(
    ...
    Routes through :func:`core.sandbox.run` with:
      ...
      - ``restrict_reads=True`` + ``fake_home=True``: $HOME is hidden
        ...
        read ~/.ssh / ~/.aws / ~/.config/mantishack; the tool's own
    ...
    with tempfile.TemporaryDirectory(prefix="mantishack-sca-resolver-") as out_dir:
