"""End-to-end orchestration for ``mantishack-sca``.
...
class RunOptions:
    """Knobs controlling a ``mantishack-sca`` run.
...
@dataclass
class RunResult:
    """Summary of a completed ``mantishack-sca`` run."""
...
def run_sca(
    ...
    *,
    ...
    #     (or DEFAULT_POLICY when no .mantishack-sca-license-policy.yml
    #     ships in the target).
    #
    ...
    # We re-compute mantishack_risk_estimate for any finding whose
        ...
        if evidence_count:
            ...
            for f in vuln_findings:
                if (f.exploit_evidence is not None
                        ...
                        and not f.in_kev):
                    f.mantishack_risk_estimate, f.risk_components = _score(
                        f, f.dependency,
                    )
            logger.info(
    ...
    # 7a. Apply operator suppression overlay (`.mantishack-sca-suppress.yml`).
    suppressed_total = 0
