...
``mantishack_agentic.py`` and other MANTISHACK-side callers that want to launch
SCA as a sandboxed subprocess rather than in-process:
  ...
  - :func:`_find_sca_agent` — discover the SCA agent entry point.
    ...
    set via ``MANTISHACK_SCA_AGENT`` env). Pre-merge, this used to bridge
    to a separate ``mantishack-sca`` repo; post-merge SCA lives in-tree.
...
_REPO = Path(__file__).resolve().parents[2]  # mantishack-sca repo root
...
def _find_sca_agent() -> Optional[Path]:
    ...
    ``MANTISHACK_SCA_AGENT`` env var still allows pointing at an external
    agent (e.g. a vendored or pinned version) for CI / custom layouts.
    Returns ``None`` only when the override path is set but invalid.
    """
    env_path = os.environ.get("MANTISHACK_SCA_AGENT")
    if env_path:
        p = Path(env_path).resolve()
        if not p.is_file():
            logger.warning("MANTISHACK_SCA_AGENT=%s does not exist — ignoring",
                           env_path)
            return None
        ...
        # or downstream crashes blamed on mantishack-sca itself. The
        ...
        # the marker check — if MANTISHACK_SCA_AGENT picked up a giant
        ...
        try:
            ...
            logger.warning(
                "MANTISHACK_SCA_AGENT=%s could not be read — ignoring",
                env_path,
            )
            return None
        if "from packages.sca import SCA_ALLOWED_HOSTS" not in text:
            logger.warning(
                "MANTISHACK_SCA_AGENT=%s does not look like a mantishack-sca "
    ...
    *,
    ...
    try:
        result = sandbox_run(
            ...
            # MANTISHACK env when caller had specifically asked for nothing.
            env=env if env is not None else MantishackConfig.get_safe_env(),
            capture_output=True,
            text=True,
            timeout=timeout,
        )
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="MANTISHACK SCA agent")
    ...
    # by mantishack.py / ``/agentic --sca`` / future wrappers that pass
    # the standard MANTISHACK run-lifecycle flag set (``--max-cost``,
