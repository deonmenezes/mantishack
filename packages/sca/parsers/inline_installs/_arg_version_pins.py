...
These are real version pins for real tools. Pre-fix, mantishack-sca
...
map plus operator-supplied inline comment overrides:
    ...
    ARG SEMGREP_VERSION=1.117.0  # mantishack-sca: PyPI:semgrep
    ARG CUSTOM_TOOL_VERSION=2.0  # mantishack-sca: skip
    ARG VENDORED_LIB_VERSION=1.0 # mantishack-sca: npm:@vendor/lib
...
operators don't have to add ``# mantishack-sca: skip`` to every
boilerplate ARG.
...
_BUILTIN_ARG_MAP: Dict[str, Optional[Tuple[str, str]]] = {
    ...
    # explicitly so the operator doesn't need ``# mantishack-sca: skip``
...
def _resolve_mapping(
    ...
    """Inline ``# mantishack-sca: ...`` takes precedence over the built-in
    map. ``mantishack-sca: skip`` forces a skip even if the ARG is in
    the built-in map.
    """
    if comment:
