"""``mantishack-sca clean-cache`` — reclaim disk space from stale cache entries.

The OSV / KEV / EPSS / registry cache lives at ``~/.mantishack/cache/sca/``
...
Was previously the ``--clean-cache`` mode of ``libexec/mantishack-sca-gate``;
...
def main(argv: Sequence[str]) -> int:
    ...
    if max_age <= 0:
        print(
            f"mantishack-sca clean-cache: --max-age must be positive, got {max_age}",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        print(f"mantishack-sca clean-cache: cache eviction failed: {e}",
              file=sys.stderr)
        return 3
    ...
    print(
        f"mantishack-sca clean-cache: cleaned {res.files_removed}/"
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="mantishack-sca clean-cache",
    ...
    p.add_argument(
        "--cache-root",
        help="override default cache root (~/.mantishack/cache/sca)",
    )
