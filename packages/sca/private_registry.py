...
Recognised env vars:
  ...
  * ``MANTISHACK_SCA_PYPI_AUTH`` — Authorization header value
    (e.g. ``Bearer xyz`` or ``Basic <b64>``).
  * ``NPM_CONFIG_REGISTRY`` — npm mirror URL (matches the npm
    CLI's convention).
  * ``MANTISHACK_SCA_NPM_AUTH`` — Authorization header value.
  * ``MANTISHACK_SCA_MAVEN_REGISTRY`` — Maven mirror URL.
  * ``MANTISHACK_SCA_MAVEN_AUTH`` — Authorization header value.
...
# where one exists (PIP_INDEX_URL / NPM_CONFIG_REGISTRY); MANTISHACK_SCA_*
# for ecosystems where no canonical env var exists.
_ENV_URL = {
    "PyPI": "PIP_INDEX_URL",
    "npm": "NPM_CONFIG_REGISTRY",
    "Maven": "MANTISHACK_SCA_MAVEN_REGISTRY",
}

_ENV_AUTH = {
    "PyPI": "MANTISHACK_SCA_PYPI_AUTH",
    "npm": "MANTISHACK_SCA_NPM_AUTH",
    "Maven": "MANTISHACK_SCA_MAVEN_AUTH",
}
