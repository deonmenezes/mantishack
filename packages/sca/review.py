"""``mantishack-sca check`` — single-package pre-add evaluation.
...
Invocation::
    ...
    mantishack-sca check <ecosystem> <name> <version> [--out <path>] [--offline] ...

Examples::

    mantishack-sca check npm   lodash                          4.17.4
    mantishack-sca check PyPI  django                          2.0.0
    mantishack-sca check Maven org.apache.logging.log4j:log4j-core 2.14.1
...
def main(
    ...
    *,
    ...
    """``mantishack-sca check`` entry point."""
    ...
    if canonical_eco is None:
        print(
            f"mantishack-sca check: unknown ecosystem {args.ecosystem!r}; "
...
def _synthesise_dep(ecosystem: str, name: str, version: str) -> Dependency:
    ...
    return Dependency(
        ...
        name=name,
        version=version,
        declared_in=Path(f"<mantishack-sca check: {ecosystem}:{name}@{version}>"),
        scope="main",
        is_lockfile=False,
...
def _render_review_markdown(
    ...
    *,
    ...
    buf = StringIO()
    buf.write(f"# mantishack-sca check — {dep.purl}\n\n")
