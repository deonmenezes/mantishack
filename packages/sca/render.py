...
shape — re-run ``mantishack-sca`` if a fresh SBOM is needed.
"""
...
def main(argv: Sequence[str]) -> int:
    ...
    if not findings_path.exists():
        print(f"mantishack-sca render: findings file not found: {findings_path}",
              file=sys.stderr)
        return 2
    try:
        rows = json.loads(findings_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, UnicodeDecodeError) as e:
        print(f"mantishack-sca render: cannot read {findings_path}: {e}", file=sys.stderr)
        return 2
    if not isinstance(rows, list):
        print(f"mantishack-sca render: {findings_path} is not a finding list",
              file=sys.stderr)
        return 2
    ...
    if not wrote:
        print("mantishack-sca render: nothing to do (both --no-md and --no-sarif "
              "supplied)", file=sys.stderr)
        return 2

    sys.stdout.write("mantishack-sca render: wrote " + "; ".join(wrote) + "\n")
    sys.stdout.flush()
    ...
    if cfg.is_active:
        ...
        print_result(passed, fails, prog="mantishack-sca render")
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument("findings",
                   help="path to findings.json from a prior `mantishack-sca` run")
...
def _render_markdown(rows: List[Dict[str, Any]], *, target: Path) -> str:
    ...
    buf.write(f"_Generated by `mantishack-sca render` at "
