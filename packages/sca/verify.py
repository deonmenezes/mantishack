"""``mantishack-sca verify`` — confirm a ``proposed/`` patch from ``fix`` actually
clears the findings it claimed to fix.

Workflow:

    mantishack-sca <target> --out base
    mantishack-sca fix --findings base/findings.json --out fix
    # `fix/proposed/` contains rewritten manifests
    mantishack-sca verify <target> --proposed fix/proposed [--findings base/findings.json]
...
Outputs (under ``--out``):
    ...
    delta.json                    structured shape (same as `mantishack-sca diff --json`)
...
def main(
    ...
    *,
    ...
    if not target.is_dir():
        print(f"mantishack-sca verify: target not a directory: {target}", file=sys.stderr)
        return 2
    if not proposed.is_dir():
        print(f"mantishack-sca verify: --proposed dir not found: {proposed}",
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"mantishack-sca verify: cannot prepare overlay: {e}", file=sys.stderr)
        return 3
    if not applied:
        print(f"mantishack-sca verify: --proposed dir contains no files; nothing to "
              f"verify ({proposed})", file=sys.stderr)
        return 2
    logger.info("mantishack-sca verify: applied %d proposed file(s) to overlay",
                len(applied))
    ...
    try:
        ...
        print(f"mantishack-sca verify: analyse on overlay failed: {e}", file=sys.stderr)
        return 3

    if args.findings:
        before_findings = Path(args.findings).resolve()
        if not before_findings.exists():
            print(f"mantishack-sca verify: --findings file not found: {before_findings}",
                  file=sys.stderr)
            return 2
    else:
        ...
        try:
            ...
            print(f"mantishack-sca verify: analyse on target failed: {e}", file=sys.stderr)
            return 3
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument("--proposed", required=True,
                   help="proposed/ directory from `mantishack-sca fix`")
