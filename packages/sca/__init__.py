"""MANTISHACK SCA Package — Software Composition Analysis.
...
into otherwise-generic ``core.http`` / ``core.json.cache`` machinery:
  ...
  - :data:`SCA_CACHE_ROOT` — default disk-cache root under
    ``~/.mantishack/cache/sca/``. Callers thread this as the explicit
...
SCA_USER_AGENT = "mantishack-sca/0.1 (+https://github.com/gadievron/raptor)"
SCA_CACHE_ROOT = Path.home() / ".mantishack" / "cache" / "sca"
...
def default_client(
    ...
    *,
    ...
    The egress proxy itself is shared across mantishack subsystems (codeql,
...
def compose_proxy_hosts(target: "Optional[Path]" = None) -> list:
    ...
    # ``dependencies[*].repository`` — needed by ``mantishack-sca bump``
