...
Shared by ``mantishack-sca fix --harden --apply`` and ``mantishack-sca fix --cve-only --apply``. Both
    ...
    *,
    ...
    Args:
        ...
        patch_path: path to the unified diff. ``None`` is a graceful
            ...
            isn't an error).
        caller_label: subcommand prefix for log lines (``"mantishack-sca fix --harden"``
            or ``"mantishack-sca fix --cve-only"``); helps operators read CI output.
