...
Replaces the standalone ``mantishack-sca-gate`` binary. The same logic is now
exposed as flags on:

  - the main scan path (``bin/mantishack-sca <target> --fail-on-severity high``)
  - the render path (``bin/mantishack-sca render findings.json --fail-on-severity high``)
...
def add_threshold_args(parser) -> None:
    ...
    parser.add_argument(
        ...
        help="evaluate findings the operator marked suppressed in "
             ".mantishack-sca-suppress.yml (default: skip them)",
    )
    parser.add_argument(
