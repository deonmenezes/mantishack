"""``mantishack-sca upgrade`` — forward-looking upgrade-impact analysis.
...
Two modes:
    ...
    mantishack-sca upgrade <eco> <name> <from> <to>
        Pairwise comparison: advisories resolved by the upgrade vs new
        advisories introduced.

    mantishack-sca upgrade <eco> <name> <from> --candidate v1 --candidate v2 ...
    ...
    *,
    ...
    if args.ecosystem and not (args.add or args.remove or args.from_file):
        ...
        if canonical_eco is None:
            print(
                f"mantishack-sca upgrade: unknown ecosystem {args.ecosystem!r}; "
    ...
    *,
    ...
    if from_file:
        try:
            ...
            return f"mantishack-sca upgrade --from: cannot read {from_file}: {e}\n", 2
    ...
    lines: List[str] = ["# mantishack-sca upgrade — proposed change set", ""]
...
def _pairwise_report(
    ...
    try:
        ...
        return (
            f"# mantishack-sca upgrade — {ecosystem}:{name} "
    ...
    buf = StringIO()
    buf.write(f"# mantishack-sca upgrade — {ecosystem}:{name} "
              f"{from_version} → {to_version}\n\n")
...
def _candidates_report(
    ...
    if not base_ids:
        ...
        buf = StringIO()
        buf.write(f"# mantishack-sca upgrade — {ecosystem}:{name} from {from_version}\n\n")
...
def _synthesise(ecosystem: str, name: str, version: str) -> Dependency:
    return Dependency(
        ...
        name=name,
        version=version,
        declared_in=Path(f"<mantishack-sca upgrade: {ecosystem}:{name}@{version}>"),
        scope="main",
        is_lockfile=False,
