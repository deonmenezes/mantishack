"""``mantishack-sca diff`` — set-difference between two ``findings.json`` runs.

Common CI question: "did this PR introduce any new vulns?". ``mantishack-sca diff``
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument("--repo-label", default=None,
                   help="when ``--pr-comment`` is set, override the "
                        "header label (default: 'mantishack-sca'). Lets "
...
def _load_rows(path_str: str) -> Optional[List[Dict[str, Any]]]:
    ...
    if not path.exists():
        print(f"mantishack-sca diff: file not found: {path}", file=sys.stderr)
        return None
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError, UnicodeDecodeError) as e:
        print(f"mantishack-sca diff: cannot read {path}: {e}", file=sys.stderr)
        return None
    if not isinstance(data, list):
        print(f"mantishack-sca diff: {path} is not a finding list", file=sys.stderr)
        return None
    return data
...
def _render_markdown(
    ...
    buf = StringIO()
    buf.write(f"# mantishack-sca diff — `{a_path}` → `{b_path}`\n\n")
...
def render_pr_comment(
    ...
    """
    buf = StringIO()
    label = repo_label or "mantishack-sca"
    ...
    buf.write(
        "\n<sub>Generated by [mantishack-sca]"
        "(https://github.com/grok-org/mantishack) · "
        "suppress with `.mantishack-sca-suppress.yml`</sub>\n"
    )
    return buf.getvalue()
