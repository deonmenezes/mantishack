...
def _build_components(
    ...
    *,
    ...
    bucket-set + binary SHA + arch/bits/format as ``mantishack:cap_fp:*``
    ...
    for d in deps:
        ...
        comp["properties"] = [
            {"name": "mantishack:ecosystem", "value": d.ecosystem},
            {"name": "mantishack:direct", "value": "true" if d.direct else "false"},
            {"name": "mantishack:is_lockfile",
             "value": "true" if d.is_lockfile else "false"},
            {"name": "mantishack:pin_style", "value": d.pin_style.value},
            ...
            {"name": "mantishack:source_kind", "value": d.source_kind},
            {"name": "mantishack:declared_in",
             "value": str(d.declared_in)},
        ]
        if d.commented_out:
            comp["properties"].append({
                "name": "mantishack:commented_out", "value": "true"
            })
        # source_extra fields surfaced as mantishack:<key> properties.
...
def _fingerprint_properties(
    ...
    """Build the ``mantishack:cap_fp:*`` properties for one dep, or
    ``[]`` when:
    ...
    props: List[Dict[str, str]] = [
        {"name": "mantishack:cap_fp:schema_version",
         "value": str(getattr(fp, "schema_version", 1))},
        {"name": "mantishack:cap_fp:buckets",
         "value": ",".join(buckets)},
    ]
    sha = getattr(fp, "binary_sha256", None)
    if sha:
        props.append(
            {"name": "mantishack:cap_fp:binary_sha256", "value": str(sha)},
        )
    arch = getattr(fp, "arch", None)
    if arch:
        props.append({"name": "mantishack:cap_fp:arch", "value": str(arch)})
    bits = getattr(fp, "bits", None)
    if bits:
        props.append(
            {"name": "mantishack:cap_fp:bits", "value": str(bits)},
        )
    fmt = getattr(fp, "binary_format", None)
    if fmt:
        props.append(
            {"name": "mantishack:cap_fp:format", "value": str(fmt)},
        )
    return props
    ...
    for f in vuln_findings:
        ...
        if f.epss is not None:
            entry.setdefault("properties", []).append({
                "name": "mantishack:epss", "value": f"{f.epss:.5f}",
            })
        if f.in_kev:
            entry.setdefault("properties", []).append({
                "name": "mantishack:in_kev", "value": "true",
            })
        if analysis:
            entry["analysis"] = analysis
        if f.fixed_version:
            entry.setdefault("properties", []).append({
                "name": "mantishack:fixed_version", "value": f.fixed_version,
            })
        out.append(entry)
    return out
