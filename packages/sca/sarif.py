"""SARIF 2.1.0 emitter for ``mantishack-sca`` findings.
...
- Declares one ``run`` driven by ``mantishack-sca``.
...
def _rule_definition(rule_id: str) -> Dict[str, Any]:
    ...
    description = _RULE_DESCRIPTIONS.get(
        rule_id,
        f"MANTISHACK /sca finding of type `{rule_id}`.",
    )
    return {
        "id": rule_id,
        "name": name,
...
def _row_to_result(
    ...
    result: Dict[str, Any] = {
        ...
        "partialFingerprints": {
            "mantishackScaFingerprint": fingerprint,
        },
