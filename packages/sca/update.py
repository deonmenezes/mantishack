"""``mantishack-sca fix --cve-only`` — CVE-driven upgrade planner.
...
def main(argv: Sequence[str]) -> int:
    ...
    try:
        ...
        print(f"mantishack-sca fix --cve-only: cannot create output dir {out_dir}: {e}",
              file=sys.stderr)
        return 2
    ...
    if not targets:
        print("mantishack-sca fix: no actionable upgrades — every vulnerable dep is "
    ...
    if want_patch and applied:
        ...
        if patch_path is not None:
            extra = (
                f"\nmantishack-sca fix: upgrade.patch written to {patch_path}\n"
                f"          apply with: cd {repo_root} && "
                f"git apply {patch_path}"
            )
    print(f"mantishack-sca fix: {len(applied)} change(s) applied, "
    ...
    if args.apply:
        ...
        rc = apply_patch_to_target(
            ...
            patch_path,
            caller_label="mantishack-sca fix --cve-only",
        )
        if rc != 0:
            return rc
    ...
    if args.hash_pin:
        ...
        if target_dir is None:
            print("mantishack-sca fix: --hash-pin needs a target directory; "
            ...
            print(f"mantishack-sca fix --cve-only --hash-pin: {verb} {len(result.changes)} "
...
def _run_cascade_validation(
    ...
    print(f"mantishack-sca fix --cve-only --allow-cascade: {oks} ecosystem(s) resolve "
...
def _run_external_validation(manifest: Path, out_dir: Path) -> None:
    ...
    if eco is None:
        print(f"mantishack-sca fix --cve-only --validate-against: cannot detect ecosystem from "
    ...
    if resolver is None:
        print(f"mantishack-sca fix --cve-only --validate-against: no resolver wrapper for "
              f"{eco}", file=sys.stderr)
        return
    if not resolver.is_available():
        print(f"mantishack-sca fix --cve-only --validate-against: {eco} toolchain not in PATH",
              file=sys.stderr)
        return
    ...
    print(f"mantishack-sca fix --cve-only --validate-against: {manifest.name} {verb} via "
...
def _render_pr_comment(changes: List["UpgradeChange"]) -> str:
    ...
    skipped = [c for c in changes if c.skipped_reason is not None]
    out: List[str] = []
    out.append("## mantishack-sca fix --cve-only — proposed plan")
    out.append("")
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="mantishack-sca fix --cve-only",
        description="CVE-driven upgrade planner.",
    )
    g = p.add_mutually_exclusive_group(required=True)
    g.add_argument(
        "--findings",
        help="findings.json from a prior `mantishack-sca` run",
    )
    g.add_argument(
        "--target",
        help="run `mantishack-sca <target>` to produce findings before planning",
    )
...
def _load_findings(args: argparse.Namespace) -> Optional[List[Dict[str, Any]]]:
    if args.findings:
        ...
        if not path.exists():
            print(f"mantishack-sca fix: findings file not found: {path}",
                  file=sys.stderr)
            return None
        try:
            data = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError, UnicodeDecodeError) as e:
            print(f"mantishack-sca fix: cannot read {path}: {e}", file=sys.stderr)
            return None
        if not isinstance(data, list):
            print(f"mantishack-sca fix: {path} is not a finding list",
                  file=sys.stderr)
            return None
        return data
    ...
    if not target.exists():
        print(f"mantishack-sca fix --cve-only: target does not exist: {target}",
              file=sys.stderr)
        return None
    if not target.is_dir():
        print(f"mantishack-sca fix --cve-only: target is not a directory: {target}",
              file=sys.stderr)
        return None
...
def _render_changes_markdown(
    ...
    applied = [c for c in changes if c.skipped_reason is None]
    skipped = [c for c in changes if c.skipped_reason is not None]
    parts: List[str] = ["# mantishack-sca fix --cve-only — proposed changes", ""]
