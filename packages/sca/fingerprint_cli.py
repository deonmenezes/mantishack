...
def _cmd_print(fp_input, args) -> int:
    ...
    if fp is None:
        print(
            f"mantishack-sca fingerprint: could not fingerprint "
            f"{fp_input[1]}",
            file=sys.stderr,
        )
        return 3
...
def _cmd_save(fp_input, args, *, store_dir: Path) -> int:
    ...
    if written is None:
        print(
            f"mantishack-sca fingerprint: store write failed under "
            f"{store_dir}",
            file=sys.stderr,
        )
        return 3
...
def _cmd_check(fp_input, args, *, store_dir: Path) -> int:
    ...
    if baseline is None:
        print(
            f"mantishack-sca fingerprint: no baseline for {ref!r}; "
...
def _emit_json(payload, out_path: Optional[str]) -> int:
    ...
    try:
        ...
        print(
            f"mantishack-sca fingerprint: --out write failed "
            f"({out_path!r}): {e}",
            file=sys.stderr,
        )
        return 3
...
def _parse_args(argv: Optional[Sequence[str]]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="mantishack-sca fingerprint",
        description=(
    ...
    p.add_argument(
        ...
        help=(
            ...
            "cache root (``~/.mantishack/cache/sca``). Fingerprint "
