...
## CLI
  ...
  ``mantishack-sca dt-push --url https://dt.example.com \
                       --api-key $DT_API_KEY \
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not api_key:
        print(
            "mantishack-sca dt-push: --api-key not given and $DT_API_KEY "
    ...
    if result["status"] == "uploaded":
        print(
            f"mantishack-sca dt-push: SBOM uploaded to "
            f"{_redact_url(args.url)} for project "
            f"{args.project} {args.version}; token={result['token']}"
        )
        return 0
    print(
        f"mantishack-sca dt-push: {result['error']}", file=sys.stderr,
    )
    return 1
