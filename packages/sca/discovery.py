...
EXCLUDED_DIR_NAMES: Set[str] = {
    ...
    # directory in many monorepos (mantishack, rush, lerna, etc.). Skipping
...
def _should_skip_dir(name: str, excludes: Set[str]) -> bool:
    ...
    ``.mantishack-sca-venv-<pid>`` so the suffix varies per run.
    """
    if name.startswith(".mantishack-sca-venv-"):
        return True
