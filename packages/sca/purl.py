"""``mantishack-sca purl`` — tiny utility that prints a canonical purl.

Useful as glue for shell scripts: build cache keys, construct
CycloneDX ``bom-ref`` values, or hand the string to other tooling.

    mantishack-sca purl npm   lodash                                  4.17.21
    mantishack-sca purl PyPI  django                                  4.2.10
    mantishack-sca purl Maven org.apache.logging.log4j:log4j-core     2.17.1
"""
...
def main(argv: Sequence[str]) -> int:
    ...
    if eco_canonical is None:
        print(
            f"mantishack-sca purl: unknown ecosystem {args.ecosystem!r}; "
    ...
    if not _valid_name(args.name):
        print(
            f"mantishack-sca purl: invalid package name {args.name!r}",
            file=sys.stderr,
        )
        return 2
    if not _valid_version(args.version):
        print(
            f"mantishack-sca purl: invalid version {args.version!r}",
            file=sys.stderr,
        )
        return 2
