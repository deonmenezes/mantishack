...
These tests pin:
  1. Fields MANTISHACK uses are preserved.
  2. Fields MANTISHACK doesn't use are dropped.
...
def test_npm_strip_drops_devdependencies() -> None:
    ...
    # Runtime deps + license must be preserved — MANTISHACK reads these.
...
def test_npm_strip_drops_npm_internals_and_metadata() -> None:
    ...
    out = _strip_npm_metadata(raw)
    # Top-level survivors MANTISHACK uses.
...
def test_pypi_strip_drops_per_release_file_redundant() -> None:
    ...
    # Fields MANTISHACK uses (wheel_compat, yanked_versions, supply-chain).
...
def test_pypi_strip_drops_cosmetic_info_fields() -> None:
    ...
    out = _strip_pypi_metadata(raw)
    info = out["info"]
    # Fields MANTISHACK reads: license, license_expression, requires_dist,
