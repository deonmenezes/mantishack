...
# Top-level fields with no MANTISHACK consumer.
#   - ``ownership``: package-ownership history (security irrelevant).
#   - ``urls``: same content as ``releases[latest]`` (redundant).
#   - ``vulnerabilities``: PyPI's own advisory feed; MANTISHACK uses OSV +
...
# Per-release-file fields MANTISHACK doesn't read.
...
#     MANTISHACK's age checks use the ISO field.
...
# Cosmetic ``info`` fields. MANTISHACK reads ``license``,
