...
``mantishack_risk_estimate`` predicts real-world exploitation against
...
Two metrics, each with a clear pass/fail threshold:
  ...
  2. **Spearman rank correlation.** Across all findings, does the
     rank by ``mantishack_risk_estimate`` correlate with rank by
    ...
    *,
    ...
    for sample in samples:
        for f in sample["findings"]:
            score = f.get("mantishack_risk_estimate")
