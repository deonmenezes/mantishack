...
For each project in :data:`PROJECT_SAMPLES`, the collector:
  ...
  4. **Discards the source.** We never store the cloned tree —
     only OUR scan output, which is MANTISHACK-generated and ships
     under MIT.
...
(``mantishack_risk_estimate``, ``severity``, ``in_kev``, ``epss``,
``cve_id``) is preserved. Project source code is NEVER included.

License compliance:

  * We don't redistribute the cloned project — it's transient.
  * Our scan output is MANTISHACK-generated (MIT). Each output JSON
    carries a ``_source.license: "MIT (MANTISHACK-generated)"`` block.
...
def _collect_one(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mantishack-sca-sample-") as tmp:
    ...
    output = {
        "_source": {
            "name": f"MANTISHACK SCA scan of {sample.name}",
            "url": sample.repo_url,
            "license": "MIT (MANTISHACK-generated scan output)",
            "fetched_at": _utcnow(),
            "git_ref": sample.git_ref,
            "project_license": sample.license_spdx,
            "provenance": (
                f"Scan output produced by MANTISHACK's SCA pipeline "
...
def _sanitise_findings(
    ...
    for f in findings:
        ...
        out.append({
            ...
            "mantishack_risk_estimate": sca.get("mantishack_risk_estimate"),
