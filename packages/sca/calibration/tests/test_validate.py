...
def _build_synthetic_corpus(corpus_dir: Path, *, scored: int) -> None:
    ...
    for i in range(scored):
        ...
        findings.append({
            ...
            "in_kev": (i < half),
            "mantishack_risk_estimate": score,
        })
    sample = {
...
def test_validate_corpus_skips_findings_without_scores(
    ...
    """Findings missing ``mantishack_risk_estimate`` aren't counted
    ...
    (samples_dir / "x.json").write_text(json.dumps({
        "_source": {"license": "MIT", "url": "x"},
        "findings": [
            {"mantishack_risk_estimate": 0.7, "advisory": {"osv_id": "CVE-2024-X"}},
            {"mantishack_risk_estimate": None, "advisory": {"osv_id": "CVE-2024-Y"}},
