...
## Usage
    ...
    libexec/mantishack-sca-stress                # check current state vs baseline
    libexec/mantishack-sca-stress --update-baseline   # capture current state as new baseline
    libexec/mantishack-sca-stress --no-clone     # use existing project_samples/ output
...
## Baseline format
    ...
    {
      "_source": {
        "name": "MANTISHACK SCA stress-test baseline",
        "license": "MIT (MANTISHACK-generated)",
...
def run_stress_sweep(
    *,
    ...
    ``~/.mantishack/cache/sca/stress/clones/``. Stable so that the
    ...
    environment variable ``MANTISHACK_SCA_STRESS_EPHEMERAL=1`` — the
    ...
    if out_root is None:
        if os.environ.get("MANTISHACK_SCA_STRESS_EPHEMERAL"):
            cleanup_dir = Path(tempfile.mkdtemp(prefix="mantishack-sca-stress-"))
...
def write_baseline(
    ...
    *,
    ...
    output = {
        ...
        # Stress-baseline data is MANTISHACK-generated (no third-party
        # content embedded), MIT-licensed, regenerated locally.
        "_source": {
            "name": "MANTISHACK SCA stress-test baseline",
            "url": "internal — packages.sca.calibration.stress",
            "license": (
                "MIT (MANTISHACK-generated). Captured per-sample "
            ...
            "provenance": (
                ...
                "``mantishack-sca-stress --update-baseline`` after "
