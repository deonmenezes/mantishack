...
def _run_cli(
    ...
    """Invoke ``mantishack-sca`` CLI; mirrors test_cli_smoke pattern."""
...
def test_review_invalid_ecosystem_returns_error(tmp_path: Path) -> None:
    """``mantishack-sca review BOGUS_ECO pkg 1.0`` → graceful error,
    not crash."""
...
def test_whatif_invalid_version_string_returns_error(tmp_path: Path) -> None:
    """``mantishack-sca whatif pypi requests not-a-version 1.0``
