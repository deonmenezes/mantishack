...
2. SCA_ALLOWED_HOSTS on the mantishack side matches the mantishack-sca side.
3. _find_sca_agent discovers (or doesn't discover) the mantishack-sca agent.
...
class TestCrossRepoParity:
    ...
    @pytest.fixture()
    def mantishack_sca_init(self):
        """Path to mantishack-sca's packages/sca/__init__.py, if available."""
        candidate = (Path(__file__).resolve().parents[3]
                     / ".." / "mantishack-sca" / "packages" / "sca" / "__init__.py")
        if not candidate.resolve().is_file():
            pytest.skip("mantishack-sca not available at ../mantishack-sca")
        return candidate.resolve()

    def test_hosts_match_mantishack_sca(self, mantishack_sca_init):
        """Every host in mantishack-sca's SCA_ALLOWED_HOSTS must appear in
        the mantishack-side copy. Extra hosts on the mantishack side are OK
        (forward-compat), but missing hosts mean the sandbox will block
        SCA traffic."""
        text = mantishack_sca_init.read_text(encoding="utf-8")
        ...
        assert m, "could not find SCA_ALLOWED_HOSTS in mantishack-sca"
        block = m.group(1)
        hosts_in_sca = set(re.findall(r'"([a-z0-9][a-z0-9._-]*\.[a-z]{2,})"', block))
        mantishack_hosts = set(SCA_ALLOWED_HOSTS)
        missing = hosts_in_sca - mantishack_hosts
        assert not missing, (
            f"mantishack-sca declares hosts not in mantishack's SCA_ALLOWED_HOSTS: "
            f"{sorted(missing)}"
        )
...
class TestFindScaAgent:
    ...
    def test_returns_none_when_not_installed(self, tmp_path, monkeypatch):
        """When no mantishack-sca tree exists, returns None."""
        monkeypatch.delenv("MANTISHACK_SCA_AGENT", raising=False)
        # _find_sca_agent searches relative to packages/sca/agent.py's
        # parents; with no mantishack-sca worktree it should return None.
        result = _find_sca_agent()
        # Either None (no mantishack-sca) or a valid path (mantishack-sca exists).
        # Both are acceptable — the function must not crash.
        assert result is None or result.is_file()

    def test_env_override(self, tmp_path, monkeypatch):
        """MANTISHACK_SCA_AGENT env var overrides discovery."""
        agent = tmp_path / "agent.py"
        agent.write_text("from packages.sca import SCA_ALLOWED_HOSTS\n")
        monkeypatch.setenv("MANTISHACK_SCA_AGENT", str(agent))
        result = _find_sca_agent()
        assert result == agent

    def test_env_override_nonexistent(self, tmp_path, monkeypatch):
        """MANTISHACK_SCA_AGENT pointing to a missing file returns None."""
        monkeypatch.setenv("MANTISHACK_SCA_AGENT", str(tmp_path / "nope.py"))
        result = _find_sca_agent()
