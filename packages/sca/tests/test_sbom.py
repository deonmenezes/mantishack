...
def test_properties_include_mantishack_extension_keys() -> None:
    comp = build_bom(deps=[_dep()])["components"][0]
    keys = {p["name"]: p["value"] for p in comp["properties"]}
    assert keys["mantishack:ecosystem"] == "npm"
    assert keys["mantishack:direct"] == "true"
    assert keys["mantishack:is_lockfile"] == "false"
    assert keys["mantishack:pin_style"] == "exact"
    # Provenance properties — let SBOM consumers see where each dep came
    # from (manifest vs Dockerfile vs GHA workflow vs ...).
    assert keys["mantishack:source_kind"] in (
        "manifest", "lockfile", "dockerfile", "devcontainer",
        "shell_script", "gha_workflow",
    )
    assert keys["mantishack:declared_in"]
...
def test_properties_surface_inline_install_provenance() -> None:
    ...
    keys = {p["name"]: p["value"] for p in comp["properties"]}
    assert keys["mantishack:source_kind"] == "dockerfile"
    assert "Dockerfile" in keys["mantishack:declared_in"]
...
def test_properties_flag_commented_out_deps() -> None:
    ...
    comp = build_bom(deps=[d])["components"][0]
    keys = {p["name"]: p["value"] for p in comp["properties"]}
    assert keys["mantishack:commented_out"] == "true"
...
def test_properties_skip_commented_out_for_uncommented() -> None:
    ...
    comp = build_bom(deps=[_dep()])["components"][0]
    keys = {p["name"]: p["value"] for p in comp["properties"]}
    assert "mantishack:commented_out" not in keys
...
def test_vex_kev_property_emitted() -> None:
    ...
    assert props["mantishack:in_kev"] == "true"
    assert props["mantishack:epss"].startswith("0.97")
...
def test_container_component_gets_fingerprint_properties() -> None:
    ...
    assert props["mantishack:cap_fp:buckets"] == "alloc,exec"
    assert props["mantishack:cap_fp:binary_sha256"] == "abc"
    assert props["mantishack:cap_fp:arch"] == "x86"
    assert props["mantishack:cap_fp:bits"] == "64"
    assert props["mantishack:cap_fp:format"] == "elf"
    assert props["mantishack:cap_fp:schema_version"] == "1"
...
def test_no_fingerprint_for_non_container_dep() -> None:
    ...
    assert not any(n.startswith("mantishack:cap_fp:") for n in prop_names)
...
def test_fingerprint_lookup_falls_back_to_dep_name() -> None:
    ...
    comp = bom["components"][0]
    props = {p["name"]: p["value"] for p in comp["properties"]}
    assert props.get("mantishack:cap_fp:buckets") == "alloc"
...
def test_fingerprint_buckets_sorted_for_stable_diff() -> None:
    ...
    comp = bom["components"][0]
    props = {p["name"]: p["value"] for p in comp["properties"]}
    assert props["mantishack:cap_fp:buckets"] == "alpha,middle,zebra"
