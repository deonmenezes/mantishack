...
def test_minimal_document_shape() -> None:
    ...
    runs = bom["runs"]
    assert len(runs) == 1
    assert runs[0]["tool"]["driver"]["name"] == "mantishack-sca"
...
def test_fingerprint_is_stable_across_versions() -> None:
    ...
    assert fa["mantishackScaFingerprint"] == fb["mantishackScaFingerprint"]
...
def test_unknown_rule_id_falls_back_to_generic_description() -> None:
    ...
    rules = bom["runs"][0]["tool"]["driver"]["rules"]
    assert rules[0]["fullDescription"]["text"].startswith("MANTISHACK /sca")
