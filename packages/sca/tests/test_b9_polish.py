...
def test_sbom_surfaces_source_extra_as_properties(tmp_path):
    """source_extra fields appear as ``mantishack:<key>`` properties in
    ...
    assert props["mantishack:image"] == "debian:11"
    assert props["mantishack:digest"] == "sha256:" + "a" * 64
    assert props["mantishack:stage_name"] == "builder"
...
def test_sbom_skips_none_valued_source_extra_fields(tmp_path):
    ...
    assert "mantishack:image" in prop_names
    assert "mantishack:stage_name" not in prop_names
