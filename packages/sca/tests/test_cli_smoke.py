...
``mantishack-sca <subcommand>`` invocation breaks — argparse drift,
...
def test_scan_offline_smoke(tmp_path: Path) -> None:
    """``mantishack-sca <target> --offline --out <out>`` — default
...
def test_check_offline_smoke(tmp_path: Path) -> None:
    """``mantishack-sca check <eco> <name> <version> --offline`` —
...
def test_upgrade_offline_smoke(tmp_path: Path) -> None:
    """``mantishack-sca upgrade npm lodash 4.17.4 4.17.21 --offline``
...
def test_diff_offline_smoke(tmp_path: Path) -> None:
    """``mantishack-sca diff <a> <b>`` — compare two findings sets.
...
def test_bump_no_candidates_smoke(tmp_path: Path) -> None:
    """``mantishack-sca bump <empty-repo>`` — no bumpable surfaces
...
def test_fix_offline_smoke(tmp_path: Path) -> None:
    """``mantishack-sca fix <target> --offline`` — scan + propose
...
def test_help_smoke() -> None:
    """``mantishack-sca --help`` — argparse renders without error
    on every subcommand registered in SUBCOMMANDS."""
    proc = _run_cli(["--help"], timeout=10)
    assert proc.returncode == 0
    assert "mantishack-sca" in proc.stdout
