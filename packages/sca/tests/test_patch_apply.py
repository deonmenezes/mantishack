...
Both ``mantishack-sca fix --harden --apply`` and ``mantishack-sca fix --cve-only --apply``
...
def test_clean_apply_returns_zero(
    ...
    rc = apply_patch_to_target(tmp_path, patch, caller_label="mantishack-sca fix --cve-only")
    assert rc == 0
    out = capsys.readouterr().out
    assert "mantishack-sca fix --cve-only --apply" in out
...
def test_caller_label_threads_into_log_lines(
    ...
    apply_patch_to_target(tmp_path, patch, caller_label="mantishack-sca fix --harden")
    out = capsys.readouterr().out
    assert "mantishack-sca fix --harden --apply" in out
