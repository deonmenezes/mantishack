...
def test_maven_via_mantishack_var():
    overrides = load_overrides({
        "MANTISHACK_SCA_MAVEN_REGISTRY": "https://nexus.example.com/maven2/",
    })
...
def test_auth_header_companion_var():
    overrides = load_overrides({
        "PIP_INDEX_URL": "https://artifactory.example.com/pypi/simple/",
        "MANTISHACK_SCA_PYPI_AUTH": "Bearer xyz123",
    })
...
def test_auth_without_url_ignored():
    ...
    overrides = load_overrides({
        "MANTISHACK_SCA_PYPI_AUTH": "Bearer xyz",
    })
...
def test_multiple_ecosystems_concurrent():
    overrides = load_overrides({
        ...
        "MANTISHACK_SCA_MAVEN_REGISTRY": "https://art.example/maven/",
    })
...
def test_pypi_client_uses_override_url(monkeypatch):
    ...
    monkeypatch.setenv("MANTISHACK_SCA_PYPI_AUTH", "Bearer t1")
...
def test_pypi_client_default_when_no_override(monkeypatch):
    ...
    monkeypatch.delenv("MANTISHACK_SCA_PYPI_AUTH", raising=False)
...
def test_npm_client_uses_override_url(monkeypatch):
    ...
    monkeypatch.setenv("MANTISHACK_SCA_NPM_AUTH", "Bearer t2")
...
def test_maven_client_uses_override_url(monkeypatch):
    monkeypatch.setenv(
        "MANTISHACK_SCA_MAVEN_REGISTRY",
        "https://nexus.example.com/maven2/",
    )
    monkeypatch.delenv("MANTISHACK_SCA_MAVEN_AUTH", raising=False)
