"""Tier-3 E2E: subprocess-invoke ``mantishack-sca bump`` against a
...
def _run_bump(
    ...
    """Invoke ``mantishack-sca bump`` via the package's ``__main__``
...
def test_bump_whatif_emits_valid_json(tmp_path: Path) -> None:
    """``mantishack-sca bump --json --whatif`` runs to completion, emits
...
def test_bump_pr_comment_produces_markdown(tmp_path: Path) -> None:
    ...
    ``gh pr comment --body-file -``. Asserts:
      ...
      * Mentions ``mantishack-sca`` somewhere (attribution)"""
    repo = tmp_path / "repo"
    _build_bump_fixture(repo)
    ...
    # Attribution (some form of "mantishack-sca" string)
    assert "mantishack-sca" in out.lower(), (
