...
* Structural assertions read like documentation of the contract
  MANTISHACK holds with consumers
...
# ---------------------------------------------------------------------------
# findings.json — MANTISHACK's canonical finding schema
# ---------------------------------------------------------------------------
...
def test_findings_severity_is_canonical(
    ...
    """Severity is a fixed lowercase enum across all MANTISHACK
...
def test_sbom_top_level_invariants(
    ...
    are CycloneDX-OPTIONAL per spec but MANTISHACK always emits
    ...
    # serialNumber + version: MANTISHACK always emits them.
    assert "serialNumber" in sbom, (
        "MANTISHACK's SBOM emitter must set serialNumber for "
