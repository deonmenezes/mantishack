"""``mantishack-sca suppress`` — operator UX for the suppression overlay.

The substrate (``packages/sca/suppressions.py``) parses
``.mantishack-sca-suppress.yml`` and applies its entries to scan
...
def _cmd_list(target: Path, *, emit_json: bool) -> int:
    ...
    if not suppress_path.exists():
        print(f"mantishack-sca suppress: no {SUPPRESS_FILENAME} in "
    ...
    if emit_json:
        ...
        return 0
    if not entries:
        print(f"mantishack-sca suppress: {suppress_path} has no entries.")
        return 0
    today = date.today()
    print(f"mantishack-sca suppress: {len(entries)} entry(ies) in "
          f"{suppress_path}")
    for e in entries:
...
def _cmd_check(*, target: Path, findings_path: Path) -> int:
    ...
    if not findings_path.exists():
        print(f"mantishack-sca suppress: {findings_path} not found",
              file=sys.stderr)
        return 2
    entries = load(suppress_path)
    if not entries:
    ...
    if not isinstance(rows, list):
        print("mantishack-sca suppress: findings.json top-level is not a "
    ...
    print(f"mantishack-sca suppress: checked {len(entries)} entry(ies) "
