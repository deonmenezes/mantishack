...
Pre-fix the scan-flag surface was duplicated in two places:
  ...
  * ``libexec/mantishack-sca-run`` — the operator-facing entry (via
    ``bin/mantishack-sca``)

The two definitions drifted: ``cli.py`` had ``--no-progress`` /
``--pr-comment`` / ``--pr-comment-label`` (added in the UX-hardening
batch), ``mantishack-sca-run`` had ``--spdx`` / ``--no-llm`` (added in
...
— operators running ``bin/mantishack-sca`` got "unrecognized argument"
...
This module is the single source of truth:
  ...
  * :func:`options_from_args` builds a :class:`RunOptions` from the
    ...
    automatically reaches both ``cli.py`` and ``mantishack-sca-run``.
...
def add_scan_args(parser: argparse.ArgumentParser) -> None:
    ...
    ``$MANTISHACK_CALLER_DIR``). Add ``target`` separately in each
    ...
    (``cli.py`` + ``libexec/mantishack-sca-run``) get the same epilog
    ...
    parser.add_argument(
        ...
        help="route OSV lookups through a local sqlite-backed copy of the "
             ...
             "``~/.mantishack/cache/sca/osv.sqlite`` by default.",
    )
    parser.add_argument(
        "--offline-db-path",
    ...
    parser.add_argument(
        ...
        help="Set the process-wide ``cc_trust`` override. NO behaviour "
             "change in mantishack-sca itself — SCA's defenses (sandbox + "
             "egress proxy + atomic write + signal-checked bumps) "
             "are not trust-gated. Provided for cross-subcommand "
             "consistency so the same flag works on every MANTISHACK "
    ...
    parser.add_argument(
        ...
        help="header label for ``--pr-comment`` (default: 'mantishack-sca'). "
...
def options_from_args(args: argparse.Namespace) -> RunOptions:
    ...
    missing ``emit_spdx_sbom``, ``mantishack-sca-run`` was missing
