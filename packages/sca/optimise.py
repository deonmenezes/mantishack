"""``mantishack-sca fix`` — scan + fix CVEs + pin unpinned deps in one pass.
...
def main(argv: Sequence[str]) -> int:
    ...
    if args.apply and args.out:
        print("mantishack-sca fix: --apply and --out are mutually exclusive",
              file=sys.stderr)
        return 2

    target = Path(args.target).resolve()
    if not target.exists():
        print(f"mantishack-sca fix: target does not exist: {target}",
              file=sys.stderr)
        return 2
    if not target.is_dir():
        print(f"mantishack-sca fix: target is not a directory: {target}",
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"mantishack-sca fix: cannot create output dir {out_dir}: {e}",
              file=sys.stderr)
        return 2
    ...
    try:
        ...
        logger.exception("mantishack-sca fix: analyse prepass failed")
        return 3
    ...
    if (not vuln_plans and not hygiene_plans and not major_blocked
            and not do_hash_pin):
        print("mantishack-sca fix: nothing to do — all deps are pinned and "
    ...
    if major_blocked:
        n = len(major_blocked)
        if llm_verdicts:
            print(f"\nmantishack-sca fix: {n} CVE fix(es) need major-version bumps "
    ...
    if args.apply:
        print(f"mantishack-sca fix: {len(vuln_applied)} CVE fix(es), "
...
def _run_hash_pin(
    ...
    msg = (
        f"mantishack-sca fix: {verb} {len(hp_result.changes)} "
...
def _analyze_major_bumps(
    ...
    if client is None:
        print("mantishack-sca fix: no LLM provider configured; "
    ...
    n = len(major_blocked)
    print(f"mantishack-sca fix: analysing {n} major-bump CVE(s) with LLM...",
          file=sys.stderr)
    ...
    for key, plan in list(major_blocked.items()):
        ...
        try:
            ...
            logger.exception(
                "mantishack-sca fix: LLM impact analysis failed for %s:%s "
    ...
    if approved:
        print(f"mantishack-sca fix: {len(approved)} major-bump(s) LLM-approved as safe",
              file=sys.stderr)
    if verdicts:
        print(f"mantishack-sca fix: {len(verdicts)} major-bump(s) need manual review",
              file=sys.stderr)
...
def _print_dry_run(
    ...
    *,
    ...
    total = n_vuln + n_pin
    print(f"mantishack-sca fix: {total} change(s) planned "
...
def _parse_args(argv: Sequence[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        prog="mantishack-sca fix",
        description="Scan + fix CVEs + pin unpinned deps in one pass.",
        epilog="Modes: 'mantishack-sca fix' (default) pins all deps + fixes CVEs. "
