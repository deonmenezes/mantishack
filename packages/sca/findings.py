...
2. ``write_findings_json`` — serialises a mixed list of ``VulnFinding``
   ...
   shape consumed by the rest of MANTISHACK.
...
def _assemble_finding(
    *,
    ...
    f.mantishack_risk_estimate, f.risk_components = compute_risk_estimate(f, dep)
    return f
...
def _vuln_finding_to_row(f: VulnFinding) -> Dict[str, Any]:
    ...
    return {
        ...
        "sca": {
            ...
            "mantishack_risk_estimate": f.mantishack_risk_estimate,
...
def _vuln_title(f: VulnFinding, primary: Optional[Advisory]) -> str:
    ...
    renderers, ``mantishack-sca diff``) display the title; the
