"""CLI entrypoint for ``mantishack-sca``.

Commands:

    mantishack-sca <target>                          Scan a project
    mantishack-sca fix <target>                      Scan + fix CVEs + tighten pins
    mantishack-sca fix <target> --cve-only           Fix CVEs only (no hygiene)
    mantishack-sca fix <target> --harden             Upgrade to latest safe versions
    mantishack-sca fix --findings <path>             Reuse existing scan (CVE-only)
    mantishack-sca check <eco> <name> <version>      Pre-install safety verdict
    mantishack-sca upgrade <eco> <name> <from> <to>  Upgrade impact comparison
    mantishack-sca diff <baseline> <current>         Delta between two scan runs

Utilities:

    mantishack-sca verify <target> --proposed <dir>  Confirm proposed fixes are safe
    mantishack-sca health                            Registry reachability check
    mantishack-sca purl <eco> <name> <version>       Print canonical Package URL
    mantishack-sca render <findings.json>            Re-emit report.md / SARIF

The default (no subcommand) is ``scan`` — the full analyse pipeline.

Outputs (scan):

    <out>/findings.json    canonical schema, consumed by the rest of MANTISHACK
...
def _split_subcommand(argv: Sequence[str]) -> "tuple[str, List[str]]":
    ...
    bare ``mantishack-sca <target>`` invocation routes to the scan path.
    """
...
def _dispatch(subcommand: str, argv: List[str]) -> int:
    ...
    print(f"mantishack-sca: unknown subcommand {subcommand!r}", file=sys.stderr)
    return 2
...
def _dispatch_fix(argv: List[str]) -> int:
    ...
    if has_cve_only and has_harden:
        print("mantishack-sca fix: --cve-only and --harden are mutually exclusive",
              file=sys.stderr)
        return 2
...
def _run_analyse(argv: List[str]) -> int:
    ...
    if args.trust_repo:
        try:
            ...
            logger.debug("mantishack-sca: core.security.cc_trust unavailable; "
                          "--trust-repo had no effect")

    target = Path(args.target).resolve()
    if not target.exists():
        logger.error("mantishack-sca: target does not exist: %s", target)
        return 2
    if not target.is_dir():
        logger.error("mantishack-sca: target is not a directory: %s", target)
        return 2
    ...
    try:
        ...
        if stage:
            ...
            msg = (f"mantishack-sca: unrecoverable error during {stage} "
                    f"({ctx})" if ctx
                    else f"mantishack-sca: unrecoverable error during {stage}")
        else:
            msg = "mantishack-sca: unrecoverable error during run"
    ...
    if args.baseline:
        try:
            ...
            logger.exception("mantishack-sca: baseline delta computation failed")
    ...
    if cfg.is_active:
        ...
        try:
            ...
            logger.error("mantishack-sca: cannot read findings for "
                         "threshold check: %s", e)
            return 3
        if not isinstance(rows, list):
            logger.error("mantishack-sca: findings.json is not a list")
            return 3
...
def _emit_baseline_delta(
    *,
    ...
    semantics are consistent with the standalone ``mantishack-sca diff`` command.
    ...
    if not baseline_path.exists():
        logger.warning("mantishack-sca: baseline %s not found; skipping delta",
                       baseline_path)
        return
    ...
    if not isinstance(baseline_rows, list) or not isinstance(current_rows, list):
        logger.warning("mantishack-sca: baseline/current findings.json not a list; "
                       "skipping delta")
        return
    ...
    logger.info(
        "mantishack-sca: baseline delta — %d new, %d resolved, "
...
def _parse_analyse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    # so the libexec/mantishack-sca-run entrypoint gets the same flag
...
def _print_summary(result) -> None:
    ...
    lines: List[str] = [
        "",
        f"mantishack-sca: target            {result.target}",
        f"mantishack-sca: output            {result.output_dir}",
        f"mantishack-sca: dependencies      {result.deps_analysed}",
    ]
    transitive_line = _format_transitive_line(result)
    if transitive_line is not None:
        lines.append(transitive_line)
    lines.extend([
        f"mantishack-sca: vuln findings     {result.vuln_findings}",
        f"mantishack-sca: in-KEV            {result.in_kev}",
        f"mantishack-sca: supply-chain      {result.supply_chain_findings}",
        f"mantishack-sca: hygiene findings  {result.hygiene_findings}",
    ])
    if result.license_findings:
        lines.append(
            f"mantishack-sca: license findings  {result.license_findings}"
        )
    if result.llm_reviews_run or result.llm_reviews_failed:
        lines.append(
            f"mantishack-sca: LLM reviews       {result.llm_reviews_run} enriched"
            + (f", {result.llm_reviews_failed} failed"
               if result.llm_reviews_failed else ""),
        )
    if result.triage_run:
        lines.append("mantishack-sca: LLM triage        done")
    if result.llm_cost > 0:
        lines.append(f"mantishack-sca: LLM cost          ${result.llm_cost:.4f}")
    lines.extend([
        f"mantishack-sca: cache             {result.cache_hits} hits / "
        f"{result.cache_misses} misses",
        f"mantishack-sca: findings.json     {result.findings_path}",
        f"mantishack-sca: report.md         {result.report_path}",
        *(
            [f"mantishack-sca: report.html       "
             f"{result.report_path.with_suffix('.html')}"]
            if (result.report_path.with_suffix('.html')).exists()
            else []
        ),
        f"mantishack-sca: sbom.cdx.json     {result.sbom_path}",
        f"mantishack-sca: findings.sarif    {result.sarif_path}",
        "",
    ])
    sys.stdout.write("\n".join(lines))
    sys.stdout.flush()
...
def _format_transitive_line(result) -> Optional[str]:
    ...
    if result.transitive_added > 0:
        ...
        return (f"mantishack-sca: transitive        +{result.transitive_added} dep(s) "
    ...
    return (f"mantishack-sca: transitive        skipped — {collapsed} "
            f"({eco_list})")
