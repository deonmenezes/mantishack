...
The user-facing CLI is `bin/mantishack-sca`. This README covers the
...
mantishack-sca /path/to/project

# Scan with a CI gate (exit 1 if any high-severity or KEV-listed CVE)
mantishack-sca /path/to/project --fail-on-severity high --fail-on-kev

# Plan upgrades for vulnerable deps (read-only)
mantishack-sca fix /path/to/project

# Apply the upgrade plan in-place
mantishack-sca fix /path/to/project --apply --allow-major

# Pre-install safety verdict for a specific package
mantishack-sca check PyPI django 4.2.10

# Compare two findings.json files (CI baseline vs current)
mantishack-sca diff baseline.json current.json
```
...
|---|---|---|
| `findings.json` | JSON list of finding rows | Programmatic consumers (other MANTISHACK tools, downstream CI). Canonical schema. |
...
| `coverage-sca.json` | JSON | MANTISHACK coverage layer (which files were examined). |
...
| Hygiene | `sca:hygiene:<kind>` | MANTISHACK-internal heuristics (lockfile drift, pin too loose, unpinned, missing lockfile, dep declared in wrong scope, etc.) |
| Supply-chain | `sca:supply_chain:<kind>` | MANTISHACK-internal heuristics (typosquat, slopsquat, install-hook risky, sentinel package match, low-bus-factor, version_publish age, orphan-commit deps, payload-size spike, workflow-signing posture, branch-protection posture, etc.) |
...
- **Bumper rewriters** cover every read-side surface above —
  `mantishack-sca bump` writes `Directory.Packages.props`
...
`mantishack-sca fix <target>` reads a recent `findings.json` (or runs
...
```sh
mantishack-sca fix /path                  # Show the plan
mantishack-sca fix /path --apply          # Apply rewrites
mantishack-sca fix /path --apply --allow-major  # Allow major-version bumps
mantishack-sca fix /path --fix=GHSA-xxx-yyy --apply  # Restrict to one advisory
mantishack-sca fix /path --pin-only       # Skip wildcards / caret / range entries
mantishack-sca fix /path --validate-against=their-pr-manifest.txt  # Check Dependabot's plan
```
...
under `~/.mantishack/cache/sca/`. Default TTLs:
...
`mantishack-sca clean-cache --max-age 0`.
...
```sh
mantishack-sca <target> --fail-on-severity high --fail-on-kev
```

Exit codes: `0` = below threshold, `1` = above threshold (build
fail), `2` = invalid args, `3` = internal error.

For pre-commit / PR-comment workflows see `mantishack-sca fix --format=pr-comment`.
...
`~/.config/mantishack/sca-proxy-hosts.json`.
...
## What mantishack-sca does NOT do (by design)
