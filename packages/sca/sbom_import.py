...
can feed it directly to mantishack-sca instead of relying on manifest
discovery. Useful when:
  ...
  * The build system produces an SBOM during the build (cargo
    ...
    manifests.
  * The deps live in a build that mantishack-sca's parsers can't fully
    decode (Bazel, custom toolchains).
  * Cross-tool comparison — give mantishack-sca + Snyk the same SBOM
