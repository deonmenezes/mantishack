...
- **License:** MIT (MANTISHACK-generated). Per-sample scan diagnostics
...
- **Source:** `packages.sca.calibration.stress.write_baseline` —
  ...
  `mantishack-sca-stress --update-baseline` after intentional changes
...
## MANTISHACK-generated artefacts

`project_samples/` contains scan outputs MANTISHACK generated by running
`bin/mantishack-sca` against public OSS projects. The scan output is
MANTISHACK-generated and ships under MANTISHACK's MIT license. The projects
...
Populated by `packages/sca/scripts/mantishack-sca-collect-samples`. The curated
...
`packages/sca/scripts/mantishack-sca-validate-corpus`) carry top-N precision +
Spearman correlation metrics computed against the
ground-truth signals. These are MANTISHACK-generated and ship under
MIT. Each report cites the snapshot date of every ground-truth
source consulted, so reviewers can reproduce metrics.

`refit/<date>.json` files (created by `mantishack-sca-refit-calibration`)
carry the per-constant risk-multiplier deltas + joint-precision
metrics from a refit run. Also MANTISHACK-generated, MIT — no third-party
content.
...
exempt — they are MANTISHACK outputs, not attributed third-party sources.
