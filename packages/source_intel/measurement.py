...
Usage:
  PYTHONPATH=$(pwd) MANTISHACK_DIR=$(pwd) _MANTISHACK_TRUSTED=1 \\
...
Environment requirements:
  * Use the same python3 that runs the rest of MANTISHACK — it has the
...
(``mantishack-corpus-run`` / ``mantishack-corpus-metrics`` in ``libexec/``)
...
def _corpus_scan_target_resolver(dataflow, repo_path: Path) -> Path:
    ...
    the whole ``repo_path`` (mantishack repo root: 15k+ files, mostly
...
def main() -> int:
    ...
    # this, source_intel would scan ``repo_path`` (the mantishack
