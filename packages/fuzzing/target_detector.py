...
def _detect_directory(path: Path) -> TargetInfo:
    ...
    return TargetInfo(
        ...
        hints=["MANTISHACK currently fuzzes individual binaries, harness generation, or recognised package types."],
    )
    ...
    if kind == "pe-sys":
        ...
        if is_windows_host:
            info.hints.extend([
                ...
                "handle and fuzzes IOCTLs via DeviceIoControl. MANTISHACK can scaffold this with --harness-ioctl.",
            ])
            info.blockers.extend([
                "MANTISHACK does not orchestrate kernel-mode fuzzing in-process. "
        ...
        if is_windows_host:
            ...
            info.hints.append(
                ...
                "MANTISHACK does not orchestrate WSL/Windows handoff yet."
            )
            info.blockers.append(
...
def _detect_python_pkg(pkg_dir: Path) -> TargetInfo:
    ...
    info.hints.append(
        ...
        "MANTISHACK's harness generator can scaffold an atheris harness from a "
        "function name."
    )
    return info
