...
class CampaignPlan:
    ...
    def summary(self) -> str:
        lines = [
            "=" * 70,
            "MANTISHACK FUZZING CAMPAIGN PLAN",
            "=" * 70,
            "",
            "Target:",
    ...
    def plan(self, target_path: Path) -> CampaignPlan:
        ...
        if kind == "elf-linux":
            ...
            if not plan.fuzzer:
                plan.blockers.append(
                    ...
                    "Run MANTISHACK on a Windows host with WinAFL installed."
                )
            else:
                plan.blockers.append(
                    "WinAFL target detection is available, but MANTISHACK does "
            ...
            plan.blockers.append(
                ...
                "(kAFL or Snapchange) which MANTISHACK does not orchestrate yet. "
            ...
            if not plan.fuzzer:
                ...
                plan.blockers.append(
                    ...
                    "MANTISHACK can run them. Generate/build a harness first, then "
            ...
            if plan.fuzzer:
                plan.blockers.append(
                    "cargo-fuzz target detection is available, but MANTISHACK does "
            ...
            if plan.fuzzer:
                plan.blockers.append(
                    "Atheris target detection is available, but MANTISHACK does "
        ...
        if plan.fuzzer is None and not plan.blockers:
            if kind in ("elf-linux", "macho"):
                ...
                plan.blockers.append(
                    "Target was detected, but no supported MANTISHACK runner is "
        ...
        if caps.is_macos and caps.afl_shmem_ok is False:
            plan.hints.append(
                ...
                "Run 'sudo afl-system-config' to fix, or MANTISHACK will use libFuzzer "
                "where possible."
            )
