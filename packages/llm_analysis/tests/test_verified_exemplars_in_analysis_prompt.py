...
the finding, a "MANTISHACK-verified exemplars" block appears in the system
...
def test_no_corpus_leaves_prompt_unchanged():
    assert "MANTISHACK-verified exemplars" not in _all_text(_bundle())
...
def test_matching_outcome_renders_block_in_untrusted_envelope():
    ...
    assert "## MANTISHACK-verified exemplars" in user
    assert "F-1" in user and "CWE-787" in user and "`sandbox`" in user
    assert "MANTISHACK-verified exemplars" not in system
...
def test_non_matching_outcome_no_block():
    ...
    assert "MANTISHACK-verified exemplars" not in _all_text(
