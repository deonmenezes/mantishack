...
def _write_query(path: Path, *, kind: str, cwe_tag: str, qid: str = "mantishack/test") -> None:
...
class TestDiscovery:
    ...
    MantishackConfig.EXTRA_CODEQL_PACK_ROOTS so the in-repo
    mantishack-python-queries pack (a real production extras root) doesn't
    ...
    def _isolate_default_root(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(MantishackConfig, "EXTRA_CODEQL_PACK_ROOTS", [])
    ...
    def test_zero_padded_cwe_tags_are_normalised(self, tmp_path, monkeypatch):
        """Real CodeQL packs use zero-padded tags (`cwe-022`); MANTISHACK
...
class TestMultiRoot:
    """Discovery walks MantishackConfig.EXTRA_CODEQL_PACK_ROOTS before the
    default. Extras win on (lang, CWE) collisions so MANTISHACK-shipped
    ...
    def test_extras_override_default_on_collision(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(_dqb, "_resolved_pack_pointers", lambda: {})
        monkeypatch.setattr(MantishackConfig, "EXTRA_CODEQL_PACK_ROOTS", [extras_root])
    ...
    def test_missing_extras_root_tolerated(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(_dqb, "_resolved_pack_pointers", lambda: {})
        monkeypatch.setattr(
            MantishackConfig, "EXTRA_CODEQL_PACK_ROOTS",
    ...
    def test_flat_pack_layout_recognised(self, tmp_path, monkeypatch):
        ...
        # Flat layout — used by mantishack-python-queries
        extras_root = tmp_path / "extras"
    ...
    def test_in_repo_pack_discoverable_in_default_config(self):
        ...
        the MANTISHACK-shipped mantishack-python-queries pack via the default
        EXTRA_CODEQL_PACK_ROOTS without any monkeypatching."""
        # No monkeypatch — use real MantishackConfig defaults
        discover_prebuilt_queries()
        ...
        assert "mantishack-python-queries" in str(path) or \
    ...
    def test_shipped_query_files_present_on_disk(self):
        ...
        repo_packs = (MantishackConfig.EXTRA_CODEQL_PACK_ROOTS or [None])[0]
...
class TestResolvedPackPointers:
    ...
    def test_extras_still_win_over_resolved(self, tmp_path, monkeypatch):
        """Extras (MantishackConfig) take priority over both resolved
        pointers and the default — MANTISHACK-shipped LocalFlowSource packs
...
class TestBuildTemplateQuery:
    ...
    def test_query_id_in_metadata(self):
        q = build_template_query(
            ...
            source_predicate_body="x",
            sink_predicate_body="y",
            query_id="mantishack/iris/test",
        )
        assert "mantishack/iris/test" in q
...
class TestCweInference:
    ...
    def test_command_injection_patterns(self):
        for rule in (
            "mantishack.injection.command-shell",
    ...
    def test_sql_injection_patterns(self):
        for rule in (
            ...
            "mantishack.sql-injection.tainted",
        ):
    ...
    def test_path_traversal(self):
        for rule in (
            ...
            "mantishack.injection.directory-traversal",
        ):
    ...
    def test_hardcoded_credentials(self):
        for rule in (
            "mantishack.crypto.hardcoded-secret",
            "hardcoded-password",
            "hardcoded-token",
        ):
    ...
    def test_returns_none_for_unknown(self):
        assert infer_cwe_from_rule_id("mantishack.lint.style.indentation") is None
        assert infer_cwe_from_rule_id("mantishack.crypto.maybe-weak-thing") is None
