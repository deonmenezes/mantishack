...
subprocess, fork+exec). MANTISHACK runs the PoC under Landlock /
...
Inlining the trigger also lets MANTISHACK's sandbox observer surface
...
attempt to subprocess / execve / fork the target binary — MANTISHACK's
...
class CrashAnalysisAgent:
    ...
    def __init__(self, binary_path: Path, out_dir: Path,
        ...
        # ``--no-verify-exploits`` (plumbed from ``mantishack_fuzzing.py``)
        ...
        # ``mantishack_fuzzing.py``). Mirrors the
        ...
        # ``mantishack_fuzzing.py`` already records. Same data path,
        ...
        # ``mantishack_fuzzing.py``. Only meaningful with
        ...
        if availability.external_llm:
            ...
            logger.info("MANTISHACK Crash Analysis Agent initialized")
            ...
            logger.info("MANTISHACK Crash Analysis Agent initialized (prep-only mode)")
            logger.info(f"Binary: {binary_path}")
            logger.info(f"Output: {out_dir}")
...
{full_response}"""
                ...
                # ``mantishack_fuzzing.py``. Same store, same source=
