...
def _pack_roots() -> List[Path]:
    ...
    Returns extras (from MantishackConfig.EXTRA_CODEQL_PACK_ROOTS) first,
    then the default. First-seen-wins semantics in the walk mean
    extras override the default on (lang, CWE) collisions — MANTISHACK-
    shipped packs (e.g. mantishack-python-queries with LocalFlowSource
...
def discover_prebuilt_queries() -> Dict[Tuple[str, str], Path]:
    ...
    a MANTISHACK override and the stdlib version. First-seen wins, with
    extras (MantishackConfig.EXTRA_CODEQL_PACK_ROOTS) walked before the
    default root so MANTISHACK-shipped packs win on collisions. Within a
    single root, iteration is alphabetical for determinism.
    """
    out: Dict[Tuple[str, str], Path] = {}

    # Build the priority-ordered list of (language, pack_dir) pairs:
    #   1. Extras roots (MantishackConfig.EXTRA_CODEQL_PACK_ROOTS) — MANTISHACK-
...
// /home/mantishack/.local/codeql-queries/cpp/ql/src/Security/CWE/CWE-120/
...
def build_template_query(
    *,
    ...
    query_id: str = "mantishack/iris-validation",
) -> Optional[str]:
...
_RULE_ID_TO_CWE: list = [
    ...
    # "command-shell" (mantishack's variant), "exec-shell", etc.
    (re.compile(
...
def infer_cwe_from_rule_id(rule_id: str) -> Optional[str]:
    ...
    descriptive rule names like "mantishack.injection.command-shell" or
