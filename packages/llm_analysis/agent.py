#!/usr/bin/env python3
"""
MANTISHACK Truly Autonomous Security Agent
...
class AutonomousSecurityAgentV2:
    def __init__(self, repo_path: Path, out_dir: Path, llm_config: Optional[LLMConfig] = None,
        ...
        # Tier-3 retrieval: prime each analysis prompt with MANTISHACK's own
        ...
        if prep_only:
            ...
            logger.info("MANTISHACK Autonomous Security Agent initialised")
            ...
            logger.info("MANTISHACK Autonomous Security Agent initialised (prep-only mode)")
            logger.info(f"Repository: {repo_path}")
            logger.info(f"Output: {out_dir}")
    ...
    def _get_verified_outcomes(self):
        ...
        runs'. The substrate that primes analysis prompts with MANTISHACK's own
...
---
*Generated by MANTISHACK Autonomous Security Agent*
...
"""
    ...
    def process_findings(self, sarif_paths: List[str] = None, findings_path: str = None,
        ...
        # parent mantishack_agentic.py process — priming there doesn't
        ...
        # ``mantishack-coverage-summary`` picks them up as reviewed
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="MANTISHACK Autonomous Security Agent"
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        help="Don't prime analysis prompts with MANTISHACK's own prior "
    ...
    # IRIS Tier 2/3 deep-validate gate. Mirrors mantishack_agentic.py.
    ...
    if args.out:
        ...
        out_dir = MantishackConfig.get_out_dir() / f"autonomous_v2_{unique_run_suffix('_')}"
    ...
    if report.get('mode') != 'prep_only':
        ...
        # surfacing /agentic uses (mantishack_agentic.py). Renders only
