# MANTISHACK-shipped CodeQL packs (IRIS Tier 1 LocalFlowSource)
...
**Discovery convention:** MANTISHACK's `discover_prebuilt_queries`
(`packages/llm_analysis/dataflow_query_builder.py`) walks every pack
under `MantishackConfig.EXTRA_CODEQL_PACK_ROOTS` looking for `.ql` files
...
queries discovered from `~/.codeql/packages/codeql/`. MANTISHACK's
...
Each pack ships a `Mantishack/LocalFlowSource.qll` library defining a
...
(`python-queries/Mantishack/LocalFlowSource.qll`) is the authoritative
...
3. **Copy an existing companion query** as a template. Replace:
   - `@id` with `mantishack/iris/<lang>/<name>-local`
...
CLI sources, so absence isn't refutation). MANTISHACK's in-repo packs use
...
See [`memory/project_iris_deferred_followups.md`](https://github.com/grokjc/mantishack/tree/main/.claude/projects/-home-mantishack-mantishack/memory)
