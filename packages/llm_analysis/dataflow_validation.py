...
def _build_hypothesis(finding: Dict, analysis: Dict, repo_path: Path):
    ...
    # Trusted (MANTISHACK-controlled) bits go into context as-is. The
    ...
    # MANTISHACK's own prior verified outcomes for this finding (Tier-3
...
def _tier1_check_finding_inner(
    ...
    *,
    ...
    # `MantishackConfig.IRIS_TIER1_ENABLED = False` (or the per-consumer
...
def _validate_one_hypothesis(
    ...
    *,
    ...
    #
    #   - In-repo extras pack (MantishackConfig.EXTRA_CODEQL_PACK_ROOTS):
...
def _try_template_with_retry(
    ...
    for attempt in range(_MAX_COMPILE_RETRIES + 1):
        ...
        rule = build_template_query(
            ...
            query_id="mantishack/iris/template",
        )
        if rule is None:
...
def _verdict_from_prebuilt(
    ...
    (`MantishackConfig.EXTRA_CODEQL_PACK_ROOTS`) ship `LocalFlowSource`
...
def _db_indexed_files(db_path: Path) -> "frozenset[str]":
    ...
    `home/mantishack/repo/src/foo.py`). We surface the full set so callers
...
def _layer3_probe_path(language: str) -> Optional[Path]:
    ...
    try:
        from core.config import MantishackConfig
        roots = list(MantishackConfig.EXTRA_CODEQL_PACK_ROOTS or [])
    except ImportError:
        return None
    for root in roots:
        candidate = (
            Path(root) / f"{language}-queries" / "Mantishack" / "CallableInventory.ql"
        )
...
def _query_is_in_extras_pack(query_path: Path) -> bool:
    ...
    roots (i.e. an in-repo MANTISHACK pack with LocalFlowSource coverage).
