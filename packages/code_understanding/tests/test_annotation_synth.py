...
SHIM = REPO_ROOT / "libexec" / "mantishack-understand-annotate"
...
class TestHuntVariantsE2E:
    ...
    def test_variants_contribute_to_coverage_record(self, understand_run):
        ...
        functions so ``mantishack-coverage-summary`` picks them up as
...
class TestCoverageRecordWireIn:
    ...
    emitting annotations, so ``mantishack-coverage-summary`` can pick
...
class TestShim:
    ...
    def test_trust_marker_required(self, tmp_path):
        env = {k: v for k, v in os.environ.items()
               if k not in ("_MANTISHACK_TRUSTED", "CLAUDECODE")}
        r = subprocess.run(
