"""Tests for libexec/mantishack-understand.
...
# libexec/mantishack-understand binary as a subprocess (see _run() below).
pytestmark = pytest.mark.integration

REPO_ROOT = Path(__file__).resolve().parents[3]
LIBEXEC = REPO_ROOT / "libexec" / "mantishack-understand"
...
class TestArgparse:
    def test_help_runs(self, env):
        ...
        assert "mantishack-understand" in proc.stdout
...
class TestTrustMarker:
    def test_runs_without_trust_marker_rejected(self, tmp_path):
        e = os.environ.copy()
        e.pop("_MANTISHACK_TRUSTED", None)
...
class TestNulByteDefensiveHandling:
    ...
    def _load_module(self):
        # The root conftest.py sets _MANTISHACK_TRUSTED=1 for the whole
        ...
        loader = SourceFileLoader("mantishack_understand", str(LIBEXEC))
