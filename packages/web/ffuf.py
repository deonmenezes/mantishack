#!/usr/bin/env python3
"""Narrow ffuf integration for MANTISHACK web scans.

The runner is intentionally small and opt-in: operators must provide a
wordlist, and MANTISHACK constrains the ffuf URL template to the configured target
...
class FfufRunner:
    ...
    def build_url_template(self, path_template: str) -> str:
        ...
        the CLI without checking it would let a saved MANTISHACK config or copied
        ...
        probe_url = url_template.replace("FUZZ", "mantishack-scope-probe")
    ...
    def run(self, config: FfufConfig) -> dict[str, Any]:
        """Run ffuf in MANTISHACK's sandbox and return a compact result summary.

        ffuf exits non-zero for several operational conditions (no matches,
        interrupted run, config error). MANTISHACK keeps the raw JSON artifact when
