"""Tests for ``libexec/mantishack-zkpox`` — the operator CLI for
...
# packages/zkpox/tests/test_libexec_mantishack_zkpox.py → parents[3] = repo root
REPO = Path(__file__).resolve().parents[3]
sys.path.insert(0, str(REPO))

SCRIPT = REPO / "libexec" / "mantishack-zkpox"


def _clean_env() -> dict:
    env = {k: v for k, v in os.environ.items()
           if k not in ("CLAUDECODE", "_MANTISHACK_TRUSTED")}
    env["MANTISHACK_DIR"] = str(REPO)
    return env
