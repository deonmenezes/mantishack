#!/usr/bin/env python3
"""MANTISHACK Recon Agent (safe, read-only)
...
# `os.environ["MANTISHACK_DIR"]` (no fallback) raises KeyError if
...
def get_out_dir() -> Path:
    base = os.environ.get("MANTISHACK_OUT_DIR")
...
def main():
    ap = argparse.ArgumentParser(description='MANTISHACK Recon Agent - safe inventory')
    ...
    tmp = Path(tempfile.mkdtemp(prefix='mantishack_recon_'))
    repo_path = None
    try:
