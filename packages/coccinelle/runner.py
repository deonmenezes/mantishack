...
#     (a long-running MANTISHACK session that picks up a new env var
    ...
    *,
    ...
    if needs_harness:
        ...
        if injected != rule_text:
            ...
            # subprocess_runner.
            fd, tmp_name = tempfile.mkstemp(suffix=".cocci", prefix="mantishack-cocci-")
            try:
    ...
    run_env = dict(env) if env is not None else MantishackConfig.get_safe_env()
    ...
    # parent process's CWD (typically the MANTISHACK repo root,
    ...
    #     MANTISHACK-root not target-root).
