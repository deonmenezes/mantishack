"""End-to-end tests for the ``libexec/mantishack-annotate`` operator CLI.

Drives the CLI as a subprocess. Each test sets ``_MANTISHACK_TRUSTED=1``
...
def _run(*args, env=None, input_text=None):
    ...
    real_env["_MANTISHACK_TRUSTED"] = "1"
    if env:
        real_env.update(env)
