...
def get_safe_git_env() -> Dict[str, str]:
    ...
    return MantishackConfig.get_git_env()
...
def safe_git_command(*args: str) -> list:
    ...
    (MANTISHACK's own .git, test fixtures) don't need this — bare
...
def clone_repository(
    ...
    proc = run_untrusted(
        ...
        timeout=MantishackConfig.GIT_CLONE_TIMEOUT,
        capture_output=True,
        text=True,
    )
    ...
    *,
    ...
    Args:
        ...
        timeout: per-call wall-clock cap (seconds; default 20).
            Tighter than ``MantishackConfig.GIT_CLONE_TIMEOUT`` because
    ...
    with tempfile.TemporaryDirectory(prefix="mantishack-ls-remote-") as td:
        ...
        # any log destination — MANTISHACK's own log files,
