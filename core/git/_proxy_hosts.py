...
Resolution layers (priority high → low):
  ...
  1. **Operator override** — ``~/.config/mantishack/git-proxy-hosts.json``
  ...
  2. **Static default** — the documented set of public forge hosts
     MANTISHACK commonly clones from (github.com + gitlab.com + the
...
write to ``~/.config/mantishack/`` already controls the MANTISHACK install.
"""
...
_OVERRIDE_CONFIG_PATH = (
    Path.home() / ".config" / "mantishack" / "git-proxy-hosts.json"
)
