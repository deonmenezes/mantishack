"""Render MANTISHACK's own verified outcomes as prompt exemplars.
...
difference is these are MANTISHACK's *own ground truth* (oracle-confirmed), so the
set grows and sharpens as MANTISHACK runs.
...
def _safe(value: Any, *, cap: int = 120) -> str:
    ...
    ``finding_id`` — originates from outside MANTISHACK. The standard prompt
...
_HEADER = "## MANTISHACK-verified exemplars"
_INTRO = (
    "Findings like this one that MANTISHACK has *previously confirmed* by "
...
def _render_one(scored: ScoredOutcome) -> str:
    ...
    # oracle/status are enum values (closed set); reason is MANTISHACK-internal.
    return (
