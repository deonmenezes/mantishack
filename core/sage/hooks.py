"""
SAGE pipeline hooks for MANTISHACK.
...
def _get_client() -> Optional[SageClient]:
    ...
    long-lived MANTISHACK session (typical: forgot to start the SAGE
...
def _repo_key(repo_path: str) -> str:
    ...
    # `mantishack-findings-e3b0c44298fc` — cross-contaminating findings
...
def _findings_domain(repo_path: str) -> str:
    return f"mantishack-findings-{_repo_key(repo_path)}"


def _exploits_domain(repo_path: str) -> str:
    return f"mantishack-exploits-{_repo_key(repo_path)}"
...
def recall_context_for_scan(
    ...
    try:
        ...
        methodology = client.query(
            ...
            domain_tag="mantishack-methodology",
            top_k=3,
        )
...
def enrich_analysis_prompt(
    ...
    mantishack-findings-<repo_key>) and can't safely fall back to cross-repo
