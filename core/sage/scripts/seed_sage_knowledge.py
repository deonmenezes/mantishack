#!/usr/bin/env python3
"""
Seed MANTISHACK institutional knowledge into SAGE.

Extracts hardcoded expert knowledge from Mantishack's codebase and stores it
...
def extract_primitives() -> list[dict]:
    ...
    mitigation_content = (
        "MANTISHACK mitigation identifiers for exploit feasibility analysis: "
        + "; ".join(mitigations)
    )
    memories.append({
...
def extract_llm_prompts() -> list[dict]:
    ...
    for name, prompt in [
        ...
        content = f"MANTISHACK {name} system prompt: {prompt.strip()}"
        memories.append({
            "content": content[:2000],  # Truncate for embedding quality
            "domain": "mantishack-prompts",
            "memory_type": "fact",
            "confidence": 0.95,
            "label": f"prompt:{name}",
        })
...
def extract_methodology() -> list[dict]:
    ...
    for fname in guidance_files:
        ...
        for i, chunk in enumerate(chunks):
            memories.append({
                "content": f"MANTISHACK methodology — {doc_name} (part {i+1}/{len(chunks)}): {chunk}",
                "domain": "mantishack-methodology",
                "memory_type": "fact",
                "confidence": 0.90,
...
def extract_semgrep_config() -> list[dict]:
    ...
    # Baseline packs
    packs = ", ".join(f"{name} ({pack})" for name, pack in MantishackConfig.BASELINE_SEMGREP_PACKS)
    memories.append({
        "content": f"MANTISHACK baseline Semgrep packs (always included in scans): {packs}.",
        "domain": "mantishack-config",
        "memory_type": "fact",
        "confidence": 0.95,
    ...
    mappings = "; ".join(
        f"{group} → {name} ({pack})"
        for group, (name, pack) in MantishackConfig.POLICY_GROUP_TO_SEMGREP_PACK.items()
    )
    memories.append({
        "content": f"MANTISHACK Semgrep policy group mappings: {mappings}.",
        "domain": "mantishack-config",
        "memory_type": "fact",
        "confidence": 0.95,
...
async def seed(sage_url: str, dry_run: bool = False, force: bool = False):
    ...
    print("=" * 60)
    print("MANTISHACK Knowledge Seeder for SAGE")
    print("=" * 60)
    print()
    ...
    # Register as mantishack-seed agent
    try:
        # See register_agents.py for on_chain_height rationale — same
        # SAGE 6.6.0 type-mismatch fix.
        reg = await client.register_agent("mantishack-seed")
        height = getattr(reg, "on_chain_height", None)
        print(f"Registered as mantishack-seed (on-chain height {height})")
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        description="Seed MANTISHACK institutional knowledge into SAGE"
    )
    parser.add_argument(
        "--sage-url",
