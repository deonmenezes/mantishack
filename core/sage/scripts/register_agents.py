#!/usr/bin/env python3
"""
Register MANTISHACK agents on the SAGE network.

Each agent gets a registered identity and role definition stored
as consensus-validated fact memories in the mantishack-agents domain.
...
MANTISHACK_AGENTS = [
    {
        "name": "mantishack-crash-analysis",
        ...
        "domains": ["mantishack-crashes", "mantishack-findings"],
        "capabilities": [
    ...
    {
        "name": "mantishack-crash-analyzer",
    ...
    {
        "name": "mantishack-coverage-analyzer",
    ...
    {
        "name": "mantishack-function-tracer",
    ...
    {
        "name": "mantishack-exploitability-validator",
        ...
        "domains": ["mantishack-exploits", "mantishack-findings"],
        "capabilities": [
    ...
    {
        "name": "mantishack-offsec-specialist",
    ...
    {
        "name": "mantishack-oss-evidence-verifier",
    ...
    {
        "name": "mantishack-oss-hypothesis-checker",
    ...
    {
        "name": "mantishack-oss-hypothesis-former",
    ...
    {
        "name": "mantishack-oss-gh-archive",
    ...
    {
        "name": "mantishack-oss-ioc-extractor",
        "role": "IOC Extractor",
        "description": (
    ...
    {
        "name": "mantishack-oss-report-generator",
        ...
        "domains": ["mantishack-forensics", "mantishack-reports"],
        "capabilities": [
    ...
    async with sem:
        ...
        try:
            ...
            if force:
                ...
                role_exists = await async_memory_exists(client, "mantishack-agents", role_tag)
...
async def register_agents(sage_url: str, dry_run: bool = False, force: bool = False):
    """Register all MANTISHACK agents on the SAGE network."""

    print("=" * 60)
    print("MANTISHACK Agent Registration for SAGE")
    print(f"Agents: {len(MANTISHACK_AGENTS)}")
    if force:
    ...
    if dry_run:
        for i, agent in enumerate(MANTISHACK_AGENTS, 1):
            caps = ", ".join(agent["capabilities"])
            domains = ", ".join(agent["domains"])
            print(f"[{i}/{len(MANTISHACK_AGENTS)}] {agent['name']}")
    ...
    try:
        ...
        # Surface it so a grep for "mantishack-registrar" in debug logs
        # confirms the registration actually landed on-chain.
        reg = await client.register_agent("mantishack-registrar")
        height = getattr(reg, "on_chain_height", None)
        print(f"Registered as mantishack-registrar (on-chain height {height})\n")
    except Exception as e:
    ...
    raw_results = await asyncio.gather(
        *(_register_one(client, agent, force, sem) for agent in MANTISHACK_AGENTS),
        return_exceptions=True,
    )
    results = []
    for agent, r in zip(MANTISHACK_AGENTS, raw_results):
    ...
    # (agent name from MANTISHACK_AGENTS metadata, status text from
    ...
    print("=" * 60)
    print(
        f"Stored: {stored}/{len(MANTISHACK_AGENTS)}  "
...
def main():
    parser = argparse.ArgumentParser(
        description="Register MANTISHACK agents on the SAGE network"
    )
    parser.add_argument(
        "--sage-url",
