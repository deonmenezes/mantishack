...
class TestAsyncMemoryExists(unittest.TestCase):
    ...
    def test_returns_true_when_results_present(self):
        ...
        result = asyncio.run(async_memory_exists(client, "mantishack-agents", "agent:mantishack-scan"))

        self.assertTrue(result)
        client.list_memories.assert_awaited_once_with(
            domain="mantishack-agents",
            tag="agent:mantishack-scan",
            limit=1,
        )
    ...
    def test_returns_false_when_empty(self):
        ...
        result = asyncio.run(async_memory_exists(client, "mantishack-agents", "agent:unknown"))
    ...
    def test_returns_false_on_query_error(self):
        ...
        result = asyncio.run(async_memory_exists(client, "mantishack-agents", "agent:anything"))
