# SAGE Integration for MANTISHACK

MANTISHACK integrates with [SAGE](https://github.com/l33tdawg/sage) (Sovereign Agent Governed Experience) — a consensus-validated persistent memory system — to enable cross-session learning across all analysis workflows.

## Architecture

MANTISHACK uses a **hybrid integration** approach:
...
connects to port 8090, and MANTISHACK runs exactly as before with zero SAGE
...
- Sets `SAGE_ENABLED=true` in `.claude/settings.local.json` so Claude Code
  propagates the flag into MANTISHACK subprocesses (Python-pipeline opt-in —
...
- Registers all 16 MANTISHACK agents on the SAGE network.
...
`libexec/mantishack-sage-setup` is safe to re-run at any time. The seed and
register steps query SAGE for each item's tag (`primitive:rop-chain`,
`agent:mantishack-scan`, etc.) before proposing, so re-runs skip entries
...
partial: mantishack-scan (filled in missing half from a prior partial run)
```
...
```bash
libexec/mantishack-sage-setup --uninstall
```
...
| `mantishack-personas` | Expert persona definitions |
| `mantishack-config` | Configuration knowledge |
...
`.mcp.json` is `.gitignore`d and managed by `libexec/mantishack-sage-setup`.
...
loaded by MANTISHACK's root `CLAUDE.md` only when the `sage_inception` tool is
present (i.e. when `.mcp.json` registers SAGE, i.e. only when a user has
actually run `libexec/mantishack-sage-setup`). The tools exposed via MCP:
