#!/usr/bin/env bash
# Shared dangerous-env-var strip list for `bin/mantishack` and `bin/cve-diff`.
#
# Lives in `core/security/` next to its Python siblings:
#   * core/config.MantishackConfig.DANGEROUS_ENV_VARS — canonical Python list
...
# lists:
#   * bin/mantishack stripped 30+ vars (the canonical set + a handful of
...
# LD_PROFILE_OUTPUT to bin/mantishack, bin/cve-diff was NOT updated —
# operators running cve-diff had a wider attack surface than
# operators running mantishack. The shared fragment closes that gap and
