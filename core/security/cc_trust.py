...
Called by every entry point that runs Claude Code against an untrusted repo:
    - bin/mantishack (via libexec/mantishack-cc-trust-check)
...
is parsed. `bin/mantishack` passes the override via argv to the libexec wrapper;
mantishack_agentic.py calls `set_trust_override(True)` after argparse.
...
subprocesses, including later MANTISHACK invocations), which could forge trust
...
Dangerous fields (block):
    settings:  apiKeyHelper, awsAuthHelper, awsAuthRefresh, gcpAuthRefresh
               ...
               env.MANTISHACK_* (attempts to forge our own control env vars)
...
_COMPREHENSIVE_DANGEROUS_ENV_VARS = frozenset({
    ...
    # MantishackConfig.DANGEROUS_ENV_VARS list does, but the fallback was
...
try:
    ...
    _DANGEROUS_ENV_VARS = (
        ...
        | frozenset(MantishackConfig.DANGEROUS_ENV_VARS)
    )
except ImportError:
...
# MANTISHACK repo root = core/security/cc_trust.py -> ../../
    ...
    try:
        ...
        if isinstance(env_cfg, dict):
            ...
            for env_key, env_val in env_cfg.items():
                ...
                # MANTISHACK_* and SAGE_* in a target repo's env dict are suspicious
                # regardless of the specific var — targets have no business
                # setting MANTISHACK's own control env vars (MANTISHACK_OUT_DIR, etc.)
                ...
                if (key_upper in dangerous_upper
                        or key_upper.startswith("MANTISHACK_")
...
def _render_scan_report(target: Path, scans, any_blocking: bool,
    ...
    if any_blocking:
        if trust_override:
            print(f"mantishack: {safe_target} has dangerous Claude Code config "
                  f"(trust override active):")
        else:
            print(f"mantishack: {safe_target} has dangerous Claude Code config:")
    else:
        print(f"mantishack: {safe_target} has Claude Code config:")
