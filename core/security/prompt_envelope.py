...
LLM output) is segregated from MANTISHACK's own instructions through layered
...
def wrap_tool_result(content: str, tool_name: str) -> str:
    ...
    The ``tool_name`` is MANTISHACK-controlled (set by the consumer's
...
# MANTISHACK's anti-BiDi defense: ``_BYPASS_CHAR_RE`` IS the
