...
def _scan_cached(resolved_path: str) -> Tuple[Tuple[FileScan, ...], bool]:
    ...
    # Skip MANTISHACK's own repo — MANTISHACK ships codeql packs under
    # packages/llm_analysis/codeql_packs/ that would always flag
    # if scanned. Operator running MANTISHACK against itself is
    # implicitly trusted.
...
def _render_scan_report(
    ...
    if any_blocking:
        if trust_override:
            print(f"mantishack: {safe_target} has dangerous CodeQL pack config "
                  f"(trust override active):")
        else:
            print(f"mantishack: {safe_target} has dangerous CodeQL pack config:")
    else:
        print(f"mantishack: {safe_target} has CodeQL pack config:")
