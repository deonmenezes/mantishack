...
# `_check_repo_claude_settings` in mantishack_agentic.py was superseded by
...
class TestRepoDefault:
    """--repo defaults to MANTISHACK_CALLER_DIR."""

    def test_env_var_used_as_default(self, tmp_path):
        """argparse picks up MANTISHACK_CALLER_DIR when --repo not specified."""
        with patch.dict(os.environ, {"MANTISHACK_CALLER_DIR": str(tmp_path)}):
            default = os.environ.get("MANTISHACK_CALLER_DIR")
    ...
    def test_env_var_not_set_gives_none(self):
        ...
        with patch.dict(os.environ, env, clear=True):
            assert os.environ.get("MANTISHACK_CALLER_DIR") is None
