"""
Subprocess tests for libexec/mantishack-cc-trust-check.
...
for ancestor in Path(__file__).resolve().parents:
    candidate = ancestor / "libexec" / "mantishack-cc-trust-check"
    if candidate.exists():
        WRAPPER = candidate
        break
if WRAPPER is None:
    raise RuntimeError(
        "test_wrapper.py: could not locate libexec/mantishack-cc-trust-check "
        "above this file"
    )
...
class TestShadowAttack:
    """The wrapper imports cc_trust from MANTISHACK_DIR, not from cwd.
