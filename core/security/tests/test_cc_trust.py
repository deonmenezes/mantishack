...
Coverage:
  ...
  - dangerous fields (credential helpers, hooks, env injection, stdio MCP,
    unknown MCP transport shapes, env.MANTISHACK_* self-trust attempts)
  ...
  - MANTISHACK self-scan short-circuit
"""
...
class TestEnvInjection:
    ...
    def test_mantishack_star_env_blocks(self, tmp_path):
        """Target repos setting env.MANTISHACK_* are trying to manipulate MANTISHACK's
        control vars (MANTISHACK_OUT_DIR, etc.). Treated as dangerous."""
        claude = tmp_path / ".claude"
        claude.mkdir()
        (claude / "settings.json").write_text(json.dumps({
            "env": {"MANTISHACK_OUT_DIR": str(tmp_path / "evil-redirect")},
        }))
        assert _check(str(tmp_path)) is True
    ...
    def test_sage_star_env_blocks(self, tmp_path, key):
        """env.SAGE_* — targets manipulating MANTISHACK's SAGE config (e.g.
...
class TestMantishackSelfScan:

    def test_self_scan_short_circuits(self, tmp_path, monkeypatch):
        """target == _MANTISHACK_DIR → return False even if dangerous content
        is planted. Prevents self-flagging when MANTISHACK scans itself."""
        import core.security.cc_trust as mod
        claude = tmp_path / ".claude"
        claude.mkdir()
        (claude / "settings.json").write_text(json.dumps({"apiKeyHelper": "x"}))
        monkeypatch.setattr(mod, "_MANTISHACK_DIR", tmp_path.resolve())
        assert _check(str(tmp_path)) is False


class TestEnvListSync:

    def test_superset_of_mantishack_config(self):
        """cc_trust's env-var list must cover MantishackConfig's list."""
        from core.security.cc_trust import _DANGEROUS_ENV_VARS
        try:
            from core.config import MantishackConfig
        except ImportError:
            pytest.skip("MantishackConfig not importable in this harness")
        missing = set(MantishackConfig.DANGEROUS_ENV_VARS) - _DANGEROUS_ENV_VARS
        assert not missing, f"cc_trust missing MantishackConfig entries: {missing}"
