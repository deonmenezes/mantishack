...
# ---------------------------------------------------------------------------
# Structural pathologies (oversize, symlink, MANTISHACK self-scan)
# ---------------------------------------------------------------------------
...
class TestStructural:
    ...
    def test_mantishack_self_scan_short_circuits(self, capsys):
        """Operator running MANTISHACK against MANTISHACK itself isn't an
        attack — MANTISHACK ships its own codeql packs under
        packages/llm_analysis/codeql_packs/."""
        # The module's _MANTISHACK_DIR = parents[2] of the module file
        # (core/security/codeql_trust.py), which is the repo root.
        # Use the same.
        from core.security.codeql_trust import _MANTISHACK_DIR
        assert _check(str(_MANTISHACK_DIR)) is False
        assert capsys.readouterr().out == ""
