"""Run provenance — point-in-time facts sealed into .mantishack-run.json.
...
#   parents[1] = core
#   parents[2] = <repo root> (the MANTISHACK checkout)
# Derived from __file__ rather than $MANTISHACK_DIR so the snapshot always
# reflects the checkout the running code actually lives in. MANTISHACK_DIR can
# be unset for direct ``python3 mantishack.py`` invocations (see the startup
# doctor's "MANTISHACK_DIR not set" warning), whereas __file__ is always right.
...
def _git(repo_dir: Path, *args: str, untrusted: bool = False) -> Optional[str]:
    ...
    overrides that neutralise those vectors. MANTISHACK's own checkout is trusted,
...
def source_control_snapshot(repo_dir: Optional[Path] = None) -> Dict[str, Any]:
    """Snapshot MANTISHACK's own source-control state. Sealed at run START.
...
def environment_snapshot() -> Dict[str, Any]:
    ...
    in MANTISHACK reads operationally — its only consumers would be debug
...
def build_start_manifest(repo_dir: Optional[Path] = None,
    ...
    the enclosing .mantishack-run.json ``version``. complete_run later merges
    ...
    # ~/.mantishack/identity.json. Omitted entirely when unset (no default); the
...
def format_manifest_block(manifest: Optional[Dict[str, Any]],
    ...
    if sc.get("base_sha"):
        ...
        lines.append(f"{indent}MANTISHACK: {sc['base_sha'][:12]}{suffix}")
...
def public_view(run_metadata: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """Publish-safe projection of a run's ``.mantishack-run.json`` for citation.
    ...
    field added to ``.mantishack-run.json`` later cannot silently leak. ``extra``
...
def aggregate_provenance(
    ...
    """Roll up per-run ``.mantishack-run.json`` manifests into a project summary.
...
def format_provenance_rollup(summary: Dict[str, Any]) -> str:
    ...
    if shas:
        ...
        if len(shas) > 1:
            lines.append(f"    ⚠ {len(shas)} distinct MANTISHACK versions across runs")
...
# the raw ``.mantishack-run.json`` dict, so the manifest's internal shape can evolve
...
def run_framework_sha(run_metadata: Optional[Dict[str, Any]]) -> Optional[str]:
    """The MANTISHACK framework ``base_sha`` that produced the run, or None."""
...
def harness_model_entry(model: Optional[str]) -> Optional[Dict[str, Any]]:
    ...
    only value only the harness (the Claude session) knows, since MANTISHACK's
