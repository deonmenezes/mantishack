"""Run metadata — .mantishack-run.json lifecycle helpers.

Every run directory gets a .mantishack-run.json file tracking what command
...
RUN_METADATA_FILE = ".mantishack-run.json"
...
# Includes both legacy prefixes (mantishack_, autonomous, exploitability-validation)
# and project-mode prefixes (agentic, validate, understand, fuzz, web).
_PREFIX_MAP = {
    # Scanning
    "scan": "scan",
    "codeql": "codeql",
    # Agentic (legacy: mantishack_, autonomous)
    "agentic": "agentic",
    "mantishack_": "agentic",
    "autonomous": "agentic",
...
def start_run(output_dir: Path, command: str, extra: Dict[str, Any] = None,
    ...
    """Write initial .mantishack-run.json with status=running.
...
def complete_run(output_dir: Path, extra: Dict[str, Any] = None,
    ...
    """Update .mantishack-run.json to status=completed.
...
def fail_run(output_dir: Path, error: str = None, extra: Dict[str, Any] = None,
    ...
    """Update .mantishack-run.json to status=failed."""
    extra = extra or {}
    if error:
        extra["error"] = error
    _finalize_sandbox_summary(output_dir)
...
def cancel_run(output_dir: Path, extra: Dict[str, Any] = None) -> None:
    """Update .mantishack-run.json to status=cancelled."""
    _finalize_sandbox_summary(output_dir)
...
def tracked_run(output_dir: Path, command: str, extra: Dict[str, Any] = None,
    ...
    Usage:
        with tracked_run(out_dir, "agentic", target="/repo") as run_dir:
            # do work...
        # .mantishack-run.json: completed on success, failed on exception, cancelled on Ctrl-C
    ...
    `.mantishack-run.json`). Now mirrored from start_run's signature.
    """
...
def load_run_metadata(run_dir: Path) -> Optional[Dict[str, Any]]:
    """Load .mantishack-run.json from a run directory. Returns None if missing."""
    return load_json(run_dir / RUN_METADATA_FILE)


def is_run_directory(path: Path, *, strict: bool = True) -> bool:
    """Check if a directory looks like a MANTISHACK run output.

    Default (``strict=True``): requires the canonical
    ``.mantishack-run.json`` marker file. This is the only signal
    ...
    `.mantishack-run.json` was a thing). Caller passes the flag
...
def infer_command_type(run_dir: Path) -> str:
    ...
    Checks .mantishack-run.json first, falls back to directory name prefix.
    """
    # Check metadata file
...
def generate_run_metadata(run_dir: Path) -> None:
    """Generate .mantishack-run.json for a directory that doesn't have one.
...
def _update_status(output_dir: Path, status: str, extra: Dict[str, Any] = None,
                   ...
                   manifest: Dict[str, Any] = None) -> None:
    """Update the status field in .mantishack-run.json.
