...
explicit:
  ...
  * **Per-uid, local config.** Read from ``~/.mantishack/identity.json`` — $HOME-
  ...
  * **No default.** Absent / empty / nameless file ⇒ ``None`` — *no* identity
    asserted (no placeholder "Mantishack User"). The manifest then omits ``who``
...
IDENTITY_PATH = Path.home() / ".mantishack" / "identity.json"
