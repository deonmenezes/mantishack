...
class TestRewriteTargetArg(unittest.TestCase):
    ...
    def test_space_form(self):
        import mantishack
        self.assertEqual(
            mantishack._rewrite_target_arg(["--repo", "/a.zip", "-x"], "/a.zip", "/ex"),
            ["--repo", "/ex", "-x"])

    def test_eq_form(self):
        import mantishack
        self.assertEqual(
            mantishack._rewrite_target_arg(["--repo=/a.zip"], "/a.zip", "/ex"),
            ["--repo=/ex"])

    def test_binary_flag_and_passthrough(self):
        import mantishack
        self.assertEqual(
            mantishack._rewrite_target_arg(["--binary", "/a.zip"], "/a.zip", "/ex"),
            ["--binary", "/ex"])
        self.assertEqual(
            mantishack._rewrite_target_arg(["--foo", "bar"], "/a.zip", "/ex"),
            ["--foo", "bar"])
...
class TestUnpackArchiveTarget(unittest.TestCase):
    ...
    def test_content_addressed_cache_and_reuse(self):
        ...
        with TemporaryDirectory() as d:
            ...
            res = mantishack._unpack_archive_target(
            ...
            new_args2, identity2 = mantishack._unpack_archive_target(
    ...
    def test_corrupt_archive_returns_none(self):
        ...
        with TemporaryDirectory() as d:
            ...
            self.assertIsNone(
                mantishack._unpack_archive_target(str(bad), ["--repo", str(bad)], out_dir))
