...
def _full_run_metadata() -> dict:
    """A realistic .mantishack-run.json with both safe and sensitive fields."""
    return {
        "version": 2,
        "command": "agentic",
...
class TestTargetSnapshot(unittest.TestCase):
    ...
    def test_source_control_uses_bare_git(self):
        # MANTISHACK's own checkout is trusted — no per-invocation overrides needed
...
class TestLifecycleE2E(unittest.TestCase):
    ...
    def test_scan_run_seals_and_enriches_full_manifest(self):
        ...
        with TemporaryDirectory() as d:
            ...
            self.assertIn("source_control", sealed)       # MANTISHACK's own repo
