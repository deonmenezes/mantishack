"""Tests for libexec/mantishack-run-lifecycle."""
...
LIFECYCLE = str(REPO_ROOT / "libexec" / "mantishack-run-lifecycle")


def _run(*args, tmp_home=None):
    """Run libexec/mantishack-run-lifecycle with given args."""
    env = os.environ.copy()
    if tmp_home:
        env["HOME"] = tmp_home
...
def _setup_project_symlink(home_dir, project_dir):
    ...
    projects_dir = Path(home_dir) / ".mantishack" / "projects"
