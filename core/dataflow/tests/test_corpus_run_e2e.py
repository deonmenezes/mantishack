...
# subprocesses against the corpus shims (mantishack-corpus-run +
# mantishack-corpus-metrics). Skipped from default fast-tier; opt in
...
def test_corpus_run_shim_produces_csv_with_one_row_per_finding(tmp_path: Path):
    ...
    rc = subprocess.call(
        [sys.executable, str(_shim("mantishack-corpus-run")), "--output", str(out)]
    )
    assert rc == 0
    assert out.exists()
...
def test_corpus_run_shim_then_metrics_shim_succeeds(tmp_path: Path):
    ...
    rc = subprocess.call(
        [sys.executable, str(_shim("mantishack-corpus-run")), "--output", str(csv_path)]
    )
    assert rc == 0

    proc = subprocess.run(
        [sys.executable, str(_shim("mantishack-corpus-metrics")), str(csv_path)],
        capture_output=True,
        text=True,
        check=False,
    )
...
def test_metrics_shim_pivot_gate_passes_on_seed_corpus(tmp_path: Path):
    ...
    rc = subprocess.call(
        [sys.executable, str(_shim("mantishack-corpus-metrics")),
