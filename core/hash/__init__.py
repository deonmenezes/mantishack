...
def sha256_tree(
    ...
    Args:
        ...
        max_file_size: Skip files larger than this. None = config default
            (MantishackConfig.MAX_FILE_SIZE_FOR_HASH). Pass 10**12 to disable.
        chunk_size: Read chunk size. None = config default
            (MantishackConfig.HASH_CHUNK_SIZE). Affects only read efficiency,
            not the digest.
    ...
    if max_file_size is None:
        max_file_size = MantishackConfig.MAX_FILE_SIZE_FOR_HASH
    if chunk_size is None:
        chunk_size = MantishackConfig.HASH_CHUNK_SIZE
    ...
    cumulative_cap = getattr(
        MantishackConfig, "MAX_TREE_HASH_BYTES",
    ...
    for p in all_files:
        ...
        try:
            ...
            # `MantishackConfig.MAX_FILE_SIZE_FOR_HASH` (a small finite
