...
Threat model: cache files live at ``~/.cache/mantishack/reachability/``
...
_CACHE_DIR = Path.home() / ".cache" / "mantishack" / "reachability"
...
# the same name. Also doubles as a cheap "is this a mantishack cache
# file" check before handing bytes to ``pickle.load``. The numeric
# suffix tracks ``_CACHE_VERSION``.
_HEADER_MAGIC = b"MANTISHACK-REACHABILITY-CACHE-V5\n"
    ...
    try:
        ...
        try:
            with os.fdopen(fd, "wb") as f:
                ...
                # mantishack targets and gives reasonable size/speed.
