...
# alongside the other MANTISHACK pool sizes; default "auto" resolves to
...
# supply an explicit ``output_dir``. Lives under ``~/.mantishack/cache/
# inventory/<target-hash>/`` — the SHA-256-prefix-of-target-path
# keys distinct projects so two scans of unrelated trees don't
# share state. Operator-purge: ``rm -rf ~/.mantishack/cache/inventory/``
# or ``mantishack-sca clean-cache``.
_DEFAULT_INVENTORY_CACHE_ROOT = (
    Path.home() / ".mantishack" / "cache" / "inventory"
)
...
def build_inventory(
    ...
    Args:
        ...
        output_dir: Directory to save checklist.json. When ``None``
            ...
            ``~/.mantishack/cache/inventory/<target-hash>/``. Persistence
...
def _process_single_file(
    ...
    try:
        ...
        if language == 'python':
            ...
            # was absent for every C scan. Closes MANTISHACK's largest
