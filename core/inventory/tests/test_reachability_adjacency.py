...
def test_reexport_absolute_import():
    """``core/__init__.py`` does ``from core.config import MantishackConfig``
    (absolute import, not ``.config``). MANTISHACK's actual
    ...
    inv = _inv(
        ...
        _file("core/config.py",
            "def MantishackConfig():\n"
            "    pass\n"
        ),
        _file("app.py",
            "from core import MantishackConfig\n"
            "def main():\n"
            "    MantishackConfig()\n"
        ),
    )
    target = InternalFunction("core/config.py", "MantishackConfig", 1)
    r = callers_of(inv, target)
    assert InternalFunction("app.py", "main", 2) in r.definitive
