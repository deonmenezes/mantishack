...
# A legitimate MANTISHACK project zip holds at most a few hundred output
...
def _enforce_zip_entry_cap(zip_path: Path) -> None:
    ...
    if count is not None and count > _MAX_ENTRIES:
        raise _ZipBombShapeError(
            ...
            f"zip-bomb shape (legitimate MANTISHACK project exports have "
            f"<< 1000 entries)"
        )
    ...
    for i, info in enumerate(zf.infolist()):
        if i >= _MAX_ENTRIES:
            raise _ZipBombShapeError(
                ...
                f"MANTISHACK project exports have << 1000 entries)"
            )
        entries.append(info)
    return entries
...
def _is_transient_artefact(path: Path) -> bool:
    ...
    ``.mantishack-run.json`` is preserved.
    """
    name = path.name
...
def import_project(zip_path: Path, projects_dir: Path,
    ...
    Args:
        ...
        projects_dir: Directory for project JSON files (~/.mantishack/projects/).
    ...
    Raises:
        ValueError: If zip is unsafe, not a MANTISHACK archive, or project
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # Require .project.json — reject non-MANTISHACK archives early
            meta_path = f"{first_part}/.project.json" if has_common_root else ".project.json"
            if meta_path not in names:
                raise ValueError(
                    "Not a MANTISHACK project archive (missing .project.json). "
                    "Use `mantishack project export` to create importable archives."
                )
