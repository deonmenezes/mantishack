...
def _build_tool_gaps(
    ...
    if scanned_not_validated:
        ...
        suggested.append(
            f"mantishack validate  # {n} unvalidated scan finding"
            f"{'s' if n != 1 else ''}"
        )
    if validated_not_scanned:
        suggested.append(
            f"mantishack scan  # {len(validated_not_scanned)} file"
            f"{'s' if len(validated_not_scanned) != 1 else ''}"
            f" with LLM findings but no static analysis"
        )
    for cmd in missing:
        suggested.append(f"mantishack {cmd}  # no {cmd} runs found")
