...
directory. Project files live in ~/.mantishack/projects/<name>.json.
...
# Default locations
PROJECTS_DIR = Path.home() / ".mantishack" / "projects"
DEFAULT_OUTPUT_BASE = Path("out/projects")


@dataclass
class Project:
    """A MANTISHACK project."""
    name: str
    target: str
    output_dir: str
    created: str = ""
    ...
    def get_run_dirs_by_type(self) -> Dict[str, List[Path]]:
        ...
        Generates .mantishack-run.json for any run directory that's missing it
...
class ProjectManager:
    ...
    def add_directory(self, name: str, directory: str, target: str = None,
        ...
        # `.mantishack-run.json` yet (legacy runs, manually-copied
