"""CLI entry point for `mantishack project` subcommands.

Called directly from bin/mantishack when `project` is the first argument.
...
def main():
    parser = argparse.ArgumentParser(
        prog="mantishack project",
        usage="mantishack project <command> [args]",
        description="Manage MANTISHACK projects. Run 'mantishack project help <command>' for details.",
    ...
    p_create = sub.add_parser("create", help="Create a new project",
                              usage="mantishack project create <name> --target <path> [-d <desc>] [--output-dir <dir>]", **_F)
    ...
    p_use = sub.add_parser("use", help="Set the active project (no arg = show current)",
                           usage="mantishack project use [<name>]", **_F)
    ...
    sub.add_parser("list", help="Show all projects",
                   usage="mantishack project list", **_F)

    # status
    p_status = sub.add_parser("status", help="Show project summary",
                              usage="mantishack project status [<name>]", **_F)
    ...
    p_cov = sub.add_parser(
        ...
        usage="mantishack project coverage [<name>] [--detailed] [--fail-under <pct>]",
        **_F,
    )
    ...
    p_prov = sub.add_parser(
        ...
        usage="mantishack project provenance [<name>]",
        **_F,
    )
    ...
    p_show = sub.add_parser(
        ...
        usage="mantishack project show <run> [<name>]",
        **_F,
    )
    ...
    p_findings = sub.add_parser("findings", help="Show merged findings across all runs",
                                usage="mantishack project findings [<name>] [--detailed]", **_F)
    ...
    p_anns = sub.add_parser(
        ...
        usage="mantishack project annotations [<name>] [--status S] "
              "[--source S] [--file PATH]",
        **_F,
    )
    ...
    p_delete = sub.add_parser("delete", help="Delete a project",
                              usage="mantishack project delete <name> [--purge] [--yes]", **_F)
    ...
    p_rename = sub.add_parser("rename", help="Rename a project",
                              usage="mantishack project rename <old> <new>", **_F)
    ...
    p_notes = sub.add_parser("notes", help="View or update project notes",
                             usage="mantishack project notes <name> [<text>] [--file <path>] [--edit]", **_F)
    ...
    p_desc = sub.add_parser("description", help="View or update project description",
                            usage="mantishack project description <name> [<text>]", **_F)
    ...
    p_add = sub.add_parser("add", help="Add existing runs to a project",
                           usage="mantishack project add <name> <directory> [--target <path>] [--output-dir <dir>]", **_F)
    ...
    p_remove = sub.add_parser("remove", help="Move a run out of the project",
                              usage="mantishack project remove <name> <run> --to <path>", **_F)
    ...
    p_report = sub.add_parser("report", help="Generate merged report across all runs",
                              usage="mantishack project report [<name>]", **_F)
    ...
    p_anndiff = sub.add_parser(
        ...
        usage="mantishack project annotations-diff <run-a> <run-b> "
              "[--name <project>]",
        **_F,
    )
    ...
    p_diff = sub.add_parser("diff", help="Compare findings between two runs",
                            usage="mantishack project diff <name> <run1> <run2>", **_F)
    ...
    p_merge = sub.add_parser("merge", help="Merge runs per command type (destructive)",
                             usage="mantishack project merge [<name>] [--type <type>] [--yes]", **_F)
    ...
    p_clean = sub.add_parser("clean", help="Delete old runs, keep latest n",
                             usage="mantishack project clean [<name>] [--keep <n>] [--dedup] [--dry-run] [--yes]", **_F)
    ...
    p_correlate = sub.add_parser("correlate", help="Cross-run finding correlation",
                                 usage="mantishack project correlate [<name>] [--json]", **_F)
    ...
    p_export = sub.add_parser("export", help="Export project as zip",
                              usage="mantishack project export <name> <path> [--force]", **_F)
    ...
    p_import = sub.add_parser("import", help="Import project from zip",
                              usage="mantishack project import <path> [--force] [--sha256 <hash>]", **_F)
    ...
    p_help = sub.add_parser("help", help="Show help",
                            usage="mantishack project help [<subcommand>]", **_F)
    ...
    try:
        if args.subcommand == "help":
            ...
            if not name:
                print("No project specified. Use: mantishack project status <name>")
                return
            p = mgr.load(name)
            if not p:
            ...
            if args.file:
                ...
                try:
                    ...
                    # problems than MANTISHACK launching it.
...
def _get_output_summary(run_dir, meta):
    ...
    back to .mantishack-run.json so subsequent calls are instant.
    """
...
def _print_annotations(
    ...
    if since:
        ...
        if cutoff is None:
            print(f"mantishack: bad --since value {since!r}; expected "
                  f"e.g. ``7d`` / ``24h`` / ``30m``")
            return
