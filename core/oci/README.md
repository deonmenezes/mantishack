...
Shared infrastructure for inspecting container images. Several mantishack consumers want this:
...
1. **Per-registry env vars** — `MANTISHACK_OCI_<HOST_UPPER>_USER` / `MANTISHACK_OCI_<HOST_UPPER>_PASSWORD`. Host with `.` and `-` becomes `_`. Example: `ghcr.io` → `MANTISHACK_OCI_GHCR_IO_USER`. Designed for CI / ad-hoc credentials.
...
- **`credsStore`** — would require shelling out to `docker-credential-<name>` (osxkeychain, secretservice, wincred, …). Expanding mantishack's trust surface to exec another binary on every registry call is too much for marginal convenience. Operators using credsStore fall back to the env-var path.
...
| `gcr.io` / `*.pkg.dev` (GCP Artifact Registry) | Bearer tokens via service-account exchange | Requires GCP-side auth setup; mantishack reads the resulting JWT via env vars |
