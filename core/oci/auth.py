...
Three sources, tried in order:
  ...
  3. **Per-registry env vars** — ``MANTISHACK_OCI_<HOST_UPPER>_USER`` and
     ``MANTISHACK_OCI_<HOST_UPPER>_PASSWORD``, with ``.`` replaced by
     ``_`` in the host. So ``ghcr.io`` → ``MANTISHACK_OCI_GHCR_IO_USER`` /
     ``MANTISHACK_OCI_GHCR_IO_PASSWORD``. Catches CI / ad-hoc cases
...
def _from_env(registry: str) -> Optional[BasicCredentials]:
    ...
    replaced with ``_`` so ``ghcr.io`` → ``MANTISHACK_OCI_GHCR_IO_*``,
    ``registry-1.docker.io`` → ``MANTISHACK_OCI_REGISTRY_1_DOCKER_IO_*``.
    The ``-`` → ``_`` substitution covers hosts with hyphens
    (``registry-1`` etc.)."""
    safe = registry.upper().replace(".", "_").replace("-", "_")
    user = os.environ.get(f"MANTISHACK_OCI_{safe}_USER")
    password = os.environ.get(f"MANTISHACK_OCI_{safe}_PASSWORD")
...
def _from_docker_config(registry: str) -> Optional[BasicCredentials]:
    ...
    if data.get("credsStore"):
        logger.debug(
            ...
            "shell out; fall back to MANTISHACK_OCI_<HOST>_USER/_PASSWORD",
