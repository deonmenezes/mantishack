...
def test_env_vars_picked_up(monkeypatch):
    monkeypatch.setenv("MANTISHACK_OCI_GHCR_IO_USER", "alice")
    monkeypatch.setenv("MANTISHACK_OCI_GHCR_IO_PASSWORD", "secret")
...
def test_env_vars_with_hyphen_in_host(monkeypatch):
    ...
    monkeypatch.setenv("MANTISHACK_OCI_REGISTRY_1_DOCKER_IO_USER", "u")
    monkeypatch.setenv("MANTISHACK_OCI_REGISTRY_1_DOCKER_IO_PASSWORD", "p")
...
def test_env_vars_partial_returns_none(monkeypatch):
    ...
    monkeypatch.setenv("MANTISHACK_OCI_GHCR_IO_USER", "alice")
    monkeypatch.delenv("MANTISHACK_OCI_GHCR_IO_PASSWORD", raising=False)
...
def test_docker_config_inline_auth_picked_up(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MANTISHACK_OCI_GHCR_IO_USER", raising=False)
    monkeypatch.delenv("MANTISHACK_OCI_GHCR_IO_PASSWORD", raising=False)
    creds = lookup_credentials("ghcr.io")
    assert creds == BasicCredentials("alice", "secret")
...
def test_docker_config_malformed_auth_field(tmp_path, monkeypatch):
    ...
    fallthrough; the failure mode is "no creds found" not "mantishack
    crashed"."""
    cfg_dir = _write_config(tmp_path, {
...
def test_env_vars_beat_docker_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("DOCKER_CONFIG", str(cfg_dir))
    monkeypatch.setenv("MANTISHACK_OCI_GHCR_IO_USER", "envuser")
    monkeypatch.setenv("MANTISHACK_OCI_GHCR_IO_PASSWORD", "envpw")
    creds = lookup_credentials("ghcr.io")
