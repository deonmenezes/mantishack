...
The output ecosystem strings match mantishack's existing OSV
...
class InstalledPackage:
    ...
    ``ecosystem`` matches mantishack's OSV ecosystem strings ("Debian",
...
def parse_rpm_sqlite(content: bytes) -> List[InstalledPackage]:
    ...
    with tempfile.NamedTemporaryFile(
        prefix="mantishack-rpm-", suffix=".sqlite", delete=False,
    ) as fp:
        fp.write(content)
