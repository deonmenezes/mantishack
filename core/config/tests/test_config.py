"""Tests for core.config.MantishackConfig."""
...
#
# in order to make the bare `from core.config import MantishackConfig`
# import below work when pytest is run from a deep cwd.
#
# Two problems with that:
#
#   1. Project rule (CLAUDE.md "Python path safety"): NEVER add
#      anything to sys.path except `os.environ["MANTISHACK_DIR"]`. The
...
class TestGetSafeEnv:
    ...
    def test_strips_proxy_env_vars(self):
        ...
        injected = {var: "http://proxy.evil.com" for var in MantishackConfig.PROXY_ENV_VARS}
        with patch.dict(os.environ, injected):
            env = MantishackConfig.get_safe_env()
            for var in MantishackConfig.PROXY_ENV_VARS:
                assert var not in env, f"{var} should be stripped from safe env"
    ...
    def test_missing_dangerous_vars_handled_gracefully(self):
        ...
        cleaned = {var: None for var in MantishackConfig.DANGEROUS_ENV_VARS}
        env_without = {k: v for k, v in os.environ.items() if k not in cleaned}
        with patch.dict(os.environ, env_without, clear=True):
            env = MantishackConfig.get_safe_env()  # must not raise
    ...
    def test_mutations_do_not_leak_to_os_environ(self):
        ...
        """
        env = MantishackConfig.get_safe_env()
        env["MANTISHACK_TEST_SENTINEL"] = "should_not_leak"
        assert "MANTISHACK_TEST_SENTINEL" not in os.environ


class TestGetGitEnv:
    """Tests for MantishackConfig.get_git_env()."""

    def test_disables_terminal_prompt(self):
        env = MantishackConfig.get_git_env()
    ...
    def test_also_strips_dangerous_vars(self):
        injected = {var: "bad" for var in MantishackConfig.DANGEROUS_ENV_VARS}
        with patch.dict(os.environ, injected):
            env = MantishackConfig.get_git_env()
            ...
            git_overrides = set(MantishackConfig.GIT_ENV_VARS)
            for var in MantishackConfig.DANGEROUS_ENV_VARS:
                if var in git_overrides:
                    assert env[var] == MantishackConfig.GIT_ENV_VARS[var]
                else:
...
class TestGetOutDir:
    """Tests for MantishackConfig.get_out_dir()."""

    def test_uses_mantishack_out_dir_env(self, tmp_path):
        with patch.dict(os.environ, {"MANTISHACK_OUT_DIR": str(tmp_path)}):
            result = MantishackConfig.get_out_dir()
            assert result == tmp_path.resolve()

    def test_falls_back_to_base_out_dir(self):
        env_without = {k: v for k, v in os.environ.items() if k != "MANTISHACK_OUT_DIR"}
        with patch.dict(os.environ, env_without, clear=True):
            result = MantishackConfig.get_out_dir()
            assert result == MantishackConfig.BASE_OUT_DIR

    def test_empty_mantishack_out_dir_falls_back(self):
        """Empty string for MANTISHACK_OUT_DIR should fall back to base.
        ...
        ``MANTISHACK_OUT_DIR=`` (e.g. shell-expansion of an
        unset var with ``$MANTISHACK_OUT_DIR``) used to surface as
        ...
        """
        with patch.dict(os.environ, {"MANTISHACK_OUT_DIR": ""}):
            result = MantishackConfig.get_out_dir()
            assert result == MantishackConfig.BASE_OUT_DIR
    ...
    def test_rejects_system_paths(self, system_path):
        """MANTISHACK_OUT_DIR pointing at a system prefix raises ValueError.
        ...
        """
        with patch.dict(os.environ, {"MANTISHACK_OUT_DIR": system_path}):
            with pytest.raises(ValueError, match="resolves under system path"):
                MantishackConfig.get_out_dir()
    ...
    def test_accepts_usr_local_lookalike(self):
        ...
        """
        with patch.dict(os.environ, {"MANTISHACK_OUT_DIR": "./usr-local-foo"}):
...
class TestEnsureDirectories:
    """Tests for MantishackConfig.ensure_directories()."""

    def test_creates_required_directories(self, tmp_path):
        """Patch REPO_ROOT so dirs are created under tmp_path."""
        with patch.object(MantishackConfig, "BASE_OUT_DIR", tmp_path / "out"), \
             patch.object(MantishackConfig, "MCP_JOB_DIR", tmp_path / "out" / "jobs"), \
             patch.object(MantishackConfig, "LOG_DIR", tmp_path / "out" / "logs"), \
             patch.object(MantishackConfig, "SCHEMAS_DIR", tmp_path / "schemas"), \
             patch.object(MantishackConfig, "CODEQL_DB_DIR", tmp_path / "codeql_dbs"), \
             patch.object(MantishackConfig, "CODEQL_SUITES_DIR", tmp_path / "codeql" / "suites"):
            MantishackConfig.ensure_directories()
    ...
    def test_idempotent(self, tmp_path):
        ...
        with patch.object(MantishackConfig, "BASE_OUT_DIR", tmp_path / "out"), \
             ...
             patch.object(MantishackConfig, "CODEQL_SUITES_DIR", tmp_path / "codeql" / "suites"):
            MantishackConfig.ensure_directories()
            MantishackConfig.ensure_directories()  # must not raise
...
class TestGetSafeEnvIncludePythonUserBase:
    ...
    when MANTISHACK spawns the scanner subprocess — verified in
    W6/W9 PoC.
    """
...
class TestGetLlmEnvIncludePythonUserBase:
    ...
    Without this forwarding, ``python mantishack.py agentic`` (the
    canonical operator entry point at ``mantishack.py:313,317``) calls
    ``get_llm_env()`` which strips PYTHONUSERBASE before the
    ``mantishack_agentic.py:757`` opt-in can restore it for the semgrep
    ...
    def test_default_strips_pythonuserbase(self):
        ...
        with patch.dict(os.environ, {"PYTHONUSERBASE": "/home/user/.local"}):
            env = MantishackConfig.get_llm_env()
            assert "PYTHONUSERBASE" not in env
    ...
    def test_include_python_user_base_keeps_pythonuserbase(self):
        ...
        (``mantishack.py:313,317``) can preserve it for the agentic
        subprocess that wires it into the semgrep spawn."""
        with patch.dict(os.environ, {"PYTHONUSERBASE": "/home/user/.local"}):
            env = MantishackConfig.get_llm_env(include_python_user_base=True)
            assert env.get("PYTHONUSERBASE") == "/home/user/.local"
