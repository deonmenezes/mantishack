#!/usr/bin/env python3
"""
MANTISHACK Centralized Configuration Module

This module provides centralized configuration management for the MANTISHACK framework,
...
class MantishackConfig:
    """Centralized configuration for MANTISHACK framework.

    This class is a NAMESPACE — every public member is a class
    attribute or @classmethod. Pre-fix MantishackConfig had no
    `__init__` guard, so ``MantishackConfig()`` silently returned
    a usable instance. Two surprises that fell out of that:

      * ``cfg = MantishackConfig(); cfg.SOMETHING = 42`` set the
        attribute on the INSTANCE, while every internal site
        reads `MantishackConfig.SOMETHING` (the class attribute) —
        ...
        instance.
      * Test fixtures that did ``cfg = MantishackConfig()`` to scope
    ...
    def __init__(self) -> None:
        raise TypeError(
            "MantishackConfig is a class-level configuration namespace; "
            "do NOT instantiate. Access members as MantishackConfig.X (or "
            "patch via patch.object(MantishackConfig, ...) in tests)."
        )
    ...
    TOOL_DEPS = {
        "afl++":        {"binary": "afl-fuzz",  "severity": "required", "affects": "/mantis-fuzz"},
        "codeql":       {"binary": "codeql",    "group": "scanner",     "affects": "/mantis-codeql, /mantis-agentic"},
        ...
        "gdb":          {"binary": "gdb",       "severity": "required", "affects": "/mantis-crash-analysis, /mantis-fuzz"},
        "rr":           {"binary": "rr",        "severity": "degrades", "affects": "/mantis-crash-analysis"},
        "semgrep":      {"binary": "semgrep",   "group": "scanner",     "affects": "/mantis-scan, /mantis-agentic"},
    }

    TOOL_GROUPS = {
        "scanner": {"min_required": 1, "affects": "/mantis-scan, /mantis-agentic"},
    }
    ...
    # so MANTISHACK-shipped packs can override stdlib queries.
    #
    # Default includes the in-repo mantishack-python-queries pack so
    ...
    ENV_OUT_DIR = "MANTISHACK_OUT_DIR"
    ENV_JOB_ID = "MANTISHACK_JOB_ID"
    ENV_LLM_CMD = "MANTISHACK_LLM_CMD"

    # LLM Provider Configuration.
    #
    # OLLAMA_HOST reads the env var on every access so a runtime
    # change (test setup that sets OLLAMA_HOST after importing a
    # consumer; an operator sourcing a shell rc after MANTISHACK is
    ...
    # diagnostic.
    #
    # Implement as a descriptor so both `MantishackConfig.OLLAMA_HOST`
    # (class access; the existing call pattern across `core/llm/`)
    # and `MantishackConfig().OLLAMA_HOST` (instance access; rare but
    ...
    SAFE_ENV_ALLOWLIST = frozenset({
        ...
        # etc. PATH hijack of MANTISHACK-owned setup binaries (unshare,
        ...
        "TZ",
        # Display — X11. Rarely set in headless MANTISHACK runs but harmless.
        "DISPLAY",
        ...
        # were invoked from a trusted parent (bin/mantishack, bin/cve-diff,
        # or Claude Code). Pure boolean flags; not shell-interpreted.
        # Must propagate through get_safe_env() because the sandbox
        # spawns its own libexec scripts (mantishack-pid1-shim,
        # mantishack-run-sandboxed) using this env.
        "_MANTISHACK_TRUSTED", "CLAUDECODE",
        # MANTISHACK runtime config that downstream subprocesses must
        # honour for the operator's intent to take effect:
        #   MANTISHACK_OUT_DIR  output dir override (without this in the
        #                    allowlist, libexec/mantishack-run-lifecycle
        #                    re-resolves out dir without the override
        #                    and writes to BASE_OUT_DIR — operator's
        #                    setting is silently lost across the
        #                    subprocess boundary).
        #   MANTISHACK_DIR      installation root; libexec scripts derive
        #                    paths from it.
        # Both are validated by get_out_dir() (refuses system paths)
        # so an attacker setting them gains nothing beyond what they
        # already had with same-UID write access to ~/mantishack-out.
    ...
    DANGEROUS_ENV_VARS = [
        ...
        # exploit, but zero legitimate use for the tools MANTISHACK runs.
    ...
    def get_out_dir() -> Path:
        """
        Resolve the output directory, honoring MANTISHACK_OUT_DIR environment variable.
        ...
        MANTISHACK_OUT_DIR before any filesystem damage.
        ...
        Raises:
            ValueError: when MANTISHACK_OUT_DIR points at a system prefix.
        """
        base = os.environ.get(MantishackConfig.ENV_OUT_DIR)
        if not base:
            return MantishackConfig.BASE_OUT_DIR
        ...
        for prefix in forbidden:
            ...
            if resolved_str == prefix or resolved_str.startswith(prefix + "/"):
                raise ValueError(
                    f"MANTISHACK_OUT_DIR={resolved!r} resolves under system "
                    f"path {prefix!r}. Refusing to create output there. "
                    f"Set MANTISHACK_OUT_DIR to a path under your home or a "
        ...
        # typo (`MANTISHACK_OUT_DIR=/home/raptr/out` — note the missing
        # `o` in `mantishack`), leaving orphaned directories scattered
        ...
        if not resolved.exists() and not resolved.parent.exists():
            raise ValueError(
                f"MANTISHACK_OUT_DIR={resolved!r} parent directory "
    ...
    def get_job_out_dir(job_id: str) -> Path:
        ...
        """
        return MantishackConfig.MCP_JOB_DIR / job_id
    ...
    def get_safe_env(
        *,
        ...
        allowlist = MantishackConfig.SAFE_ENV_ALLOWLIST
        prefixes = MantishackConfig.SAFE_ENV_PREFIXES
        env = {}
        ...
        if not preserve_proxy:
            env = strip_env_vars(env, MantishackConfig.PROXY_ENV_VARS)
        env = strip_env_vars(env, MantishackConfig.DANGEROUS_ENV_VARS)
    ...
    LLM_API_KEY_VARS = (
        ...
        "GEMINI_API_KEY",
        "MISTRAL_API_KEY",
        # Below: providers MANTISHACK doesn't call directly today, but
    ...
    def get_llm_env(
        *,
        ...
        Use this for spawning MANTISHACK's own analysis scripts that may call
    ...
    def get_git_env() -> dict:
        ...
        """
        env = MantishackConfig.get_safe_env()
        env.update(MantishackConfig.GIT_ENV_VARS)
        return env
    ...
    def ensure_directories() -> None:
        ...
        directories = [
            MantishackConfig.BASE_OUT_DIR,
            MantishackConfig.MCP_JOB_DIR,
            MantishackConfig.LOG_DIR,
            MantishackConfig.SCHEMAS_DIR,
            MantishackConfig.CODEQL_DB_DIR,
            MantishackConfig.CODEQL_SUITES_DIR,
        ]
...
def get_out_dir() -> Path:
    ...
    return MantishackConfig.get_out_dir()
