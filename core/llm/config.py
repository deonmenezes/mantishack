...
Config file: ~/.config/mantishack/models.json
...
def _get_configured_models() -> List[Dict]:
    """
    Get all models from MANTISHACK config file.
    ...
    1. MANTISHACK_CONFIG environment variable
    2. ~/.config/mantishack/models.json
...
def _get_best_thinking_model() -> Optional['ModelConfig']:
    ...
    # probed once before the operator created `~/.config/mantishack/models.json`
            ...
            for pattern_provider, pattern_model, base_score in thinking_model_patterns:
                if entry_provider == pattern_provider and entry_model == pattern_model:
                    ...
                    if effective_score > best_score:
                        ...
                        # configured ``MantishackConfig.OLLAMA_HOST`` over
...
def _build_ollama_config() -> Optional['ModelConfig']:
    ...
    for pref in preferred:
        ...
        if selected_model != ollama_models[0]:
            break
    ollama_base = _validate_ollama_url(MantishackConfig.OLLAMA_HOST)
...
def _get_default_primary_model(
    ...
    2. **Operator's thinking-model config** (``~/.config/mantishack/models.json``).
    ...
    # Gemini in ``~/.config/mantishack/models.json``, respect that even
...
@dataclass
class LLMConfig:
    """Main LLM configuration for MANTISHACK."""
    ...
    def to_file(self, config_path: Path) -> None:
        ...
        **What this does NOT write:**
          ...
          * fallback_models / specialized_models — multi-model setups
            should be authored in `~/.config/mantishack/models.json`
        ...
        `~/.config/mantishack/models.json` rather than via this method.
