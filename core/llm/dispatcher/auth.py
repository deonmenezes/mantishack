...
Only providers MANTISHACK actively dispatches to are supported here. If
...
class CredentialStore:
    ...
    didn't supply, from ``~/.config/mantishack/models.json``. Env-set
...
def seed_from_config(store: CredentialStore) -> None:
    """Fill empty slots in *store* from ``~/.config/mantishack/models.json``.
    ...
    ``$MANTISHACK_CONFIG`` if set, else ``~/.config/mantishack/models.json``.
    ...
    config_path_str = os.environ.get("MANTISHACK_CONFIG")
    if config_path_str:
