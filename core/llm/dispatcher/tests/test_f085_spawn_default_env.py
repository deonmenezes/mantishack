...
HOME, no LANG, no anything except the two MANTISHACK_LLM_* vars added a
...
is None, default to `MantishackConfig.get_safe_env()` (so credentials are
...
def test_spawn_worker_env_none_defaults_to_safe_env(fake_dispatcher):
    ...
    `MantishackConfig.get_safe_env()`. Mirrors the sandbox context.py
    ...
    # The MANTISHACK_LLM_* vars are always set by spawn_worker. The bug:
    # env=None used to make base_env={} so PATH/HOME/etc were missing.
    # The fix: env=None should fall back to MantishackConfig.get_safe_env()
    # which always contains PATH (it's in SAFE_ENV_VARS).
    assert "PATH" in captured_env, (
        "spawn_worker(env=None) failed to inherit PATH via "
        f"MantishackConfig.get_safe_env(). Captured env keys: "
        f"{sorted(captured_env.keys())}"
    )
    # And the dispatcher vars must still be set.
    assert captured_env.get("MANTISHACK_LLM_SOCKET") == "./fake.sock"
    assert captured_env.get("MANTISHACK_LLM_TOKEN_FD") == "99"
