...
from ``~/.config/mantishack/models.json`` so operators who keep their
...
def test_seed_fills_empty_slots(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MANTISHACK_CONFIG", str(config))
...
def test_silent_on_missing_file(tmp_path, monkeypatch):
    monkeypatch.setenv("MANTISHACK_CONFIG", str(tmp_path / "does-not-exist.json"))
