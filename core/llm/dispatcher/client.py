...
A worker spawned via :func:`spawn_worker` inherits two pieces of state:
  ...
  * ``MANTISHACK_LLM_SOCKET`` env var — UDS path of the dispatcher.
  * ``MANTISHACK_LLM_TOKEN_FD`` env var — read-end of a pipe with the
...
_TOKEN_HEADER = "X-Mantishack-Token"
...
def read_token(fd: Optional[int] = None) -> str:
    ...
    ``MANTISHACK_LLM_TOKEN_FD`` from the environment. The FD is closed
    after a successful read so the token doesn't survive the call.
    """
    if fd is None:
        env = os.environ.get("MANTISHACK_LLM_TOKEN_FD")
        if env is None:
            raise RuntimeError(
                "MANTISHACK_LLM_TOKEN_FD not set — worker must be spawned via "
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"MANTISHACK_LLM_TOKEN_FD payload was not ASCII "
        ...
        os.close(fd)
    if not token:
        raise RuntimeError("MANTISHACK_LLM_TOKEN_FD pipe was empty")
    return token


# Token cache: ``read_token()`` consumes the FD on first read. When
# multiple ``Provider`` instances share the same worker process — every
# MANTISHACK analysis script does this when the operator has multiple
...
def _make_httpx_client(
    ...
    ``X-Mantishack-Token`` header is attached to every request via the
...
def _resolve_socket_and_token(
    ...
    if socket_path is None:
        env = os.environ.get("MANTISHACK_LLM_SOCKET")
        if env is None:
            raise RuntimeError(
                "MANTISHACK_LLM_SOCKET not set — worker must be spawned via "
                "core.llm.dispatcher.spawn_worker"
            )
        socket_path = env
    if token is None:
...
def make_anthropic_client(
    *,
    ...
    Defaults read socket path from ``MANTISHACK_LLM_SOCKET`` and the
    token from ``MANTISHACK_LLM_TOKEN_FD``. Pass arguments explicitly
    only in tests.
...
def relay_for_grandchild() -> tuple[str, int]:
    ...
    dispatcher (env has ``MANTISHACK_LLM_SOCKET`` + ``MANTISHACK_LLM_TOKEN_FD``)
    needs to spawn its own subprocess that should share the same
    LLM session — typical example is ``mantishack_agentic.py`` spawning
    ...
    is fine: both processes are part of the same MANTISHACK run, both
