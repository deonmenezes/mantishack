...
The child receives:
  ...
  * ``MANTISHACK_LLM_SOCKET`` — UDS path of the dispatcher.
  * ``MANTISHACK_LLM_TOKEN_FD`` — file descriptor number to read the
    token from.
...
def spawn_worker(
    ...
    **popen_kwargs,
    ...
    ``MantishackConfig.get_safe_env()`` so no API keys leak in). The
    helper adds ``MANTISHACK_LLM_SOCKET`` and ``MANTISHACK_LLM_TOKEN_FD`` on
    ...
    # When env=None, fall back to MantishackConfig.get_safe_env() rather
    ...
    if env is not None:
        ...
        base_env = MantishackConfig.get_safe_env()
    base_env["MANTISHACK_LLM_SOCKET"] = socket_path
    base_env["MANTISHACK_LLM_TOKEN_FD"] = str(token_fd)
