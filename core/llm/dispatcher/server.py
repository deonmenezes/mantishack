...
Five security layers, in the order an attacker must defeat them:
  ...
  L3. **Per-worker capability token, FD-passed.** Each spawned
      ...
      on Linux). Worker presents the token in the ``X-Mantishack-Token``
...
class LLMDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        # ``MANTISHACK_LLM_DISPATCHER_TOKEN_TTL_S`` / ``..._BUDGET`` env →
        ...
        self._token_ttl_s = (
            ...
            if token_ttl_s is not None
                ...
                "MANTISHACK_LLM_DISPATCHER_TOKEN_TTL_S",
        ...
        self._token_budget = (
            ...
            if token_budget is not None
                ...
                "MANTISHACK_LLM_DISPATCHER_TOKEN_BUDGET",
        ...
        self._sock_dir = Path(tempfile.mkdtemp(prefix=f"mantishack-llm-{run_id}-"))
        ...
        try:
            ...
            # Best-effort cleanup so /tmp/mantishack-llm-* doesn't
    ...
    def _init_server(self, run_id: str) -> None:
        ...
        self._thread = threading.Thread(
            ...
            name=f"mantishack-llm-dispatcher-{run_id}",
            daemon=True,
        )
        self._thread.start()
    ...
    def allocate_worker(self, label: str) -> tuple[str, int]:
        ...
        sets ``MANTISHACK_LLM_TOKEN_FD=<n>`` in the worker's env. The
