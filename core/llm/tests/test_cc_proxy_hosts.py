...
(priority high → low):
  1. ~/.config/mantishack/cc-dispatch-proxy-hosts.json override
     (proxy_hosts only)
...
def no_override_config(monkeypatch, tmp_path):
    ...
    operator's real ~/.config/mantishack isn't read during tests."""
...
class TestDefault:
    ...
    def test_returns_documented_anthropic_set(
        ...
        # calibration probe (see MANTISHACK_CC_CALIBRATE_NETWORK_PROBE).
        ...
        # Datadog telemetry stays absent by MANTISHACK policy.
...
class TestReadablePathsForCCDispatch:
    ...
    def test_calibrated_paths_used_when_present(
        ...
        prof = _fake_profile(
            ...
            paths_stat=["/etc/mantishack/claude.conf"],
        )
        monkeypatch.setattr(mod, "_calibrated_profile", lambda claude_bin=None: prof)
        paths = readable_paths_for_cc_dispatch()
        ...
        assert "/etc/mantishack/claude.conf" in paths
...
class TestNetworkProbeOptIn:
    ...
    def test_opt_in_value_other_than_1_treated_as_off(
        ...
        # avoids the "is `MANTISHACK_CC_CALIBRATE_NETWORK_PROBE=` set
