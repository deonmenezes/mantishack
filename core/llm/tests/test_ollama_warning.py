...
def _attach_caplog_to_mantishack_logger(caplog):
    """MantishackLogger sets propagate=False, so caplog (attached to root) misses
    its records. Attach caplog's handler directly to the 'mantishack' logger for
    the duration of each test in this module."""
    mantishack_logger = logging.getLogger("mantishack")
    mantishack_logger.addHandler(caplog.handler)
    try:
        yield
    finally:
        mantishack_logger.removeHandler(caplog.handler)
...
class TestOllamaWarning:
    ...
    def test_ollama_warning_on_init(self, caplog):
        ...
        import requests
        try:
            response = requests.get(f"{MantishackConfig.OLLAMA_HOST}/api/tags", timeout=2)
        ...
        config.primary_model = ModelConfig(
            provider="ollama",
            model_name="mistral",
            api_base=MantishackConfig.OLLAMA_HOST,
        )
    ...
    def test_warning_format(self, caplog):
        ...
        if ollama_warnings:
            ...
            assert "llm" in warning.name.lower() or "mantishack" in warning.name.lower(), \
