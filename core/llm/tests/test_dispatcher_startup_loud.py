"""Pin that ``mantishack._get_or_start_dispatcher`` surfaces failures
...
``MantishackConfig.get_llm_env``, the env-direct fallback that
...
@pytest.fixture
def fresh_mantishack_module():
    """Re-import ``mantishack`` so the module-level ``_active_dispatcher``
    ...
    sys.modules.pop("mantishack", None)
    mantishack = importlib.import_module("mantishack")
    yield mantishack
    # Reset for cleanliness — clear the module-level cache.
    mantishack._active_dispatcher = None
    sys.modules.pop("mantishack", None)


def test_dispatcher_startup_failure_writes_loud_stderr_line(
    fresh_mantishack_module,
):
    ...
    when a worker dies."""
    mantishack = fresh_mantishack_module
    ...
    with mock.patch(
        ...
        result = mantishack._get_or_start_dispatcher()
...
def test_dispatcher_startup_success_is_quiet(fresh_mantishack_module):
...
def test_loud_message_includes_phase_c_migration_hint(
    ...
    with mock.patch(
        ...
        mantishack._get_or_start_dispatcher()
...
def test_dispatcher_failure_is_idempotent_within_one_process(
    ...
    with mock.patch(
        ...
        first = mantishack._get_or_start_dispatcher()
    ...
    with mock.patch(
        ...
        second = mantishack._get_or_start_dispatcher()
