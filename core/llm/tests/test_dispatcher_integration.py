...
``MANTISHACK_LLM_SOCKET`` is set in the worker's env, and the env-direct
...
class TestProviderRouteSelection:
    ...
    when ``MANTISHACK_LLM_SOCKET`` is set, and the direct-SDK path
    ...
    def test_anthropic_provider_uses_dispatcher_when_socket_set(
        ...
        try:
            ...
            monkeypatch.setenv("MANTISHACK_LLM_SOCKET", str(d.socket_path))
            monkeypatch.setenv("MANTISHACK_LLM_TOKEN_FD", str(fd))
    ...
    def test_anthropic_provider_uses_direct_sdk_when_no_socket(
        ...
        pytest.importorskip("anthropic")
        monkeypatch.delenv("MANTISHACK_LLM_SOCKET", raising=False)
        from core.llm.providers import AnthropicProvider
        cfg = ModelConfig(provider="anthropic", model_name="claude-3-haiku-20240307")
    ...
    def test_openai_compat_does_NOT_use_dispatcher_for_ollama(
        ...
        ``MANTISHACK_LLM_SOCKET`` set, the dispatcher path must NOT be
...
class TestGrandchildRelay:
    """``mantishack_agentic.py --sequential`` spawns
    ...
    def test_relay_for_grandchild_returns_inheritable_fd_with_token(
        ...
        try:
            ...
            monkeypatch.setenv("MANTISHACK_LLM_SOCKET", str(d.socket_path))
            monkeypatch.setenv("MANTISHACK_LLM_TOKEN_FD", str(parent_fd))
            from core.llm.dispatcher import client as _client
            _client._cached_token = None
    ...
    def test_grandchild_subprocess_uses_relayed_session(
        ...
        try:
            ...
            # mantishack_agentic.py does when it spawns agent.py with
            # the relayed session.
            ...
            parent_src.write_text(
                ...
                "    'MANTISHACK_LLM_SOCKET': sock,\n"
                "    'MANTISHACK_LLM_TOKEN_FD': str(child_fd),\n"
                "}\n"
                "rc = subprocess.call(\n"
            ...
            assert "x-mantishack-token" not in sent
        finally:
            upstream.shutdown()
            d.shutdown()
...
class TestPhaseBChainE2E:
    """Adversarial-grade end-to-end: simulate the full mantishack.py →
    mantishack_agentic.py → packages/llm_analysis/agent.py chain that
    ...
    Three processes:
      1. **outer-parent** (this test) — owns the dispatcher and the
         ...
         child's environment.
      2. **child** — like mantishack_agentic.py: builds a real
...
class TestDetectLLMAvailabilityRecognizesDispatcher:
    ...
    mantishack_agentic.py and the prep-only decision in agent.py. After
    Phase C, env has no API keys but ``MANTISHACK_LLM_SOCKET`` is set;
    ...
    def test_external_llm_true_when_dispatcher_socket_set(
        ...
        monkeypatch.setenv("MANTISHACK_LLM_SOCKET", "./whatever-not-used.sock")
    ...
    def test_external_llm_false_without_dispatcher_or_keys(
        ...
        for k in (
            "ANTHROPIC_API_KEY", "OPENAI_API_KEY",
            "GEMINI_API_KEY", "MISTRAL_API_KEY",
            "MANTISHACK_LLM_SOCKET",
        ):
            monkeypatch.delenv(k, raising=False)
        # Stub out ollama / config-file checks so this test is
        # hermetic (no /etc/mantishack/llm-models.toml dependency, no
...
class TestEndToEndCredentialIsolationThroughProviders:
    ...
    in ``core/llm/providers.py``) — i.e. the actual code MANTISHACK analysis
    scripts run."""
