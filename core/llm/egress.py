...
through MANTISHACK's existing in-process egress proxy.
...
----------------------
``MantishackConfig.get_safe_env()`` strips ``HTTPS_PROXY`` from subprocess
...
------------------------
If the operator has ``HTTPS_PROXY=http://corp:8080`` set when MANTISHACK
...
intend to gate. Audit performed 2026-05-07:
  ...
  * ``core.git.clone`` — git runs as a sandboxed subprocess with
    its own sanitised env (``MantishackConfig.get_git_env()``); parent
