...
class CCDispatchConfig:
    ...
    # Default-True: sub-agents spawned by mantishack's dispatch paths
    ...
    # under mantishack's sandbox egress-allowlist produces a DENY for
...
def build_cc_command(config: CCDispatchConfig) -> list[str]:
    ...
    if config.strict_mcp:
        ...
        # servers — the right posture for mantishack's per-finding
        # analysis dispatches.
        #
