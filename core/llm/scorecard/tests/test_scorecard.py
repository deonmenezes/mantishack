...
class TestAutoGc:
    ...
    def test_logs_summary_on_drop(self, tmp_path, caplog):
        ...
        # MantishackLogger uses a "mantishack" logger with propagate=False,
        # so caplog (which hooks root by default) doesn't see its
        # records. Attach a captor handler directly to the "mantishack"
        ...
        mantishack_logger.addHandler(captor)
        try:
            sc.record_event(
                "x:y", "fresh", EventType.CHEAP_SHORT_CIRCUIT, "correct",
            )
        finally:
            mantishack_logger.removeHandler(captor)
    ...
    def test_no_log_when_nothing_dropped(self, tmp_path):
        ...
        # — caplog can't see records on the propagate=False mantishack
        # logger.
        import logging
        captured: list[logging.LogRecord] = []
