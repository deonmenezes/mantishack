...
(``mantishack-sandbox-calibrate --bin claude --force``), or by waiting
for a MANTISHACK release that adds the host to the hardcoded fallback.
...
_OVERRIDE_CONFIG_PATH = Path.home() / ".config" / "mantishack" / "cc-dispatch-proxy-hosts.json"
...
# ``--max-budget-usd 0.01`` cap below, an operator running MANTISHACK
...
_NETWORK_PROBE_OPT_IN_ENV = "MANTISHACK_CC_CALIBRATE_NETWORK_PROBE"
...
# patterns where cc_dispatch is called many times in a single MANTISHACK
...
# Claude Code 2.1.138 (see MANTISHACK_CC_CALIBRATE_NETWORK_PROBE):
...
# Datadog telemetry is denied by MANTISHACK policy. The proxy logs the
...
# in production) flip ``MANTISHACK_CC_CALIBRATE_NETWORK_PROBE=1``.
