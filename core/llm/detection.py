...
def _get_available_ollama_models() -> List[str]:
    ...
    ollama_url = _validate_ollama_url(MantishackConfig.OLLAMA_HOST)

    _ollama_checked = True
    try:
        # nosemgrep: sinks.mantishack.web.ssrf.dynamic-url
        ...
        ollama_display = MantishackConfig.OLLAMA_HOST if 'localhost' in MantishackConfig.OLLAMA_HOST or '127.0.0.1' in MantishackConfig.OLLAMA_HOST else '[REMOTE-OLLAMA]'
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                new_config = Path.home() / ".config/mantishack/models.json"
            ...
            if installed in ("1.82.7", "1.82.8"):
                msg = (
                    ...
                    f"  MANTISHACK no longer uses litellm, but the package can still harm your system.\n"
                    f"\n"
                )
                if installed == "1.82.8":
                ...
                print(msg)
                raise SystemExit(
                    f"MANTISHACK cannot run with litellm {installed} installed. "
...
def _try_auto_migrate(old_config: Path, new_config: Path) -> bool:
    """Attempt to auto-migrate LiteLLM YAML config to MANTISHACK JSON.
    ...
    # Allowlist of providers MANTISHACK's downstream code can handle.
    ...
    try:
        ...
        if skipped_unknown:
            logger.info(
                ...
                "MANTISHACK supports: %s. Add a manual entry to %s if needed.",
                len(skipped_unknown),
        ...
        print(
            f"\n  [mantishack] Auto-migrated LiteLLM config → {new_config}\n"
...
def _check_litellm_migration():
    ...
    if old_config.exists() and not new_config.exists():
        ...
        print(
            "\n  [mantishack] LiteLLM is no longer used. Your config needs migrating.\n"
            "\n"
            "  Found:    ~/.config/litellm/config.yaml\n"
            "  Expected: ~/.config/mantishack/models.json\n"
            "\n"
            "  Create the new config:\n"
            "\n"
            "    mkdir -p ~/.config/mantishack\n"
            "    cat > ~/.config/mantishack/models.json << 'EOF'\n"
            f"{sample_indented}\n"
            "    EOF\n"
            "    chmod 600 ~/.config/mantishack/models.json\n"
            "\n"
...
def _read_config_models() -> list:
    """Read model entries from MANTISHACK config file.
    ...
    try:
        ...
        config_path_str = os.getenv('MANTISHACK_CONFIG')
        if config_path_str:
...
def _config_has_keyed_models() -> bool:
    """Check if the MANTISHACK config file has any usable model.
...
def detect_llm_availability() -> LLMAvailability:
    ...
    # dispatcher has ``MANTISHACK_LLM_SOCKET`` set but no API keys in
    ...
    # (Phase 4 orchestration in mantishack_agentic.py, the ``--prep-only``
    ...
    has_dispatcher_route = bool(os.getenv("MANTISHACK_LLM_SOCKET"))
