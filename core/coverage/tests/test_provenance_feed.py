...
who/what-version/when from a run's .mantishack-run.json manifest."""
...
def _run(tmp_path, name, tool, files, manifest=None):
    ...
    if manifest is not None:
        (d / ".mantishack-run.json").write_text(json.dumps(manifest))
    return d
...
def test_no_manifest_degrades_gracefully(tmp_path):
    run = _run(tmp_path, "scan-1", "semgrep", ["a.c"])   # no .mantishack-run.json
