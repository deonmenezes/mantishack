"""Tests for the libexec/mantishack-enrich-context-map-ast-view wrapper.
...
# libexec/mantishack-enrich-context-map-ast-view wrapper as a subprocess.
# Top tests at 11s; opt-in via ``pytest -m integration``.
pytestmark = pytest.mark.integration


# parents[3] = core/ast/tests → core/ast → core → repo root. Anchor to
# this file, not $MANTISHACK_DIR, so the wrapper resolves within this
# worktree (MANTISHACK_DIR may point at a different checkout).
REPO_ROOT = Path(__file__).resolve().parents[3]
WRAPPER = REPO_ROOT / "libexec" / "mantishack-enrich-context-map-ast-view"


def _run(*args, **kwargs):
    """Invoke the wrapper as a real subprocess (matches how skills call it)."""
    env = {**os.environ, "_MANTISHACK_TRUSTED": "1"}
...
class MantishackEnrichContextMapAstViewTests(unittest.TestCase):
    ...
    def test_trust_marker_required(self):
        """No CLAUDECODE and no _MANTISHACK_TRUSTED → refuse with exit 2."""
        with TemporaryDirectory() as tmp:
            tmp = Path(tmp)
            (tmp / "context-map.json").write_text("{}")
            # Strip both trust markers — must refuse.
            env = {k: v for k, v in os.environ.items()
                   if k not in ("CLAUDECODE", "_MANTISHACK_TRUSTED")}
            proc = subprocess.run(
                [str(WRAPPER), str(tmp)],
