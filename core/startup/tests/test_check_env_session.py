...
``core.startup.init.check_env``:
  ...
  * MANTISHACK_DIR defensive check.
...
# ---------------------------------------------------------------------------
# MANTISHACK_DIR check
# ---------------------------------------------------------------------------


class TestMantishackDirCheck:
    def test_unset_warns_with_computed_value(
        self, fake_repo, monkeypatch,
    ):
        monkeypatch.delenv("MANTISHACK_DIR", raising=False)
        _, warnings = _run_check_env(monkeypatch)
        match = [w for w in warnings if "MANTISHACK_DIR not set" in w]
        assert match, f"no MANTISHACK_DIR warning in: {warnings}"
    ...
    def test_set_correctly_no_warning(self, fake_repo, monkeypatch):
        monkeypatch.setenv("MANTISHACK_DIR", str(fake_repo))
        _, warnings = _run_check_env(monkeypatch)
        assert not any(
            "MANTISHACK_DIR" in w and "not set" in w for w in warnings
        )


# ---------------------------------------------------------------------------
# No .claude/mantishack.env or .claude/settings.json checks
# ---------------------------------------------------------------------------
...
class TestNoClaudeFileChecks:
    """Pin that ``check_env`` does NOT warn about ``.claude/mantishack.env``
    ...
    advice (operator damage / MANTISHACK ship-side bug / claude
    misconfig)."""

    def test_no_mantishack_env_warning_inside_claude_session(
        self, fake_repo, monkeypatch,
    ):
        monkeypatch.setenv("CLAUDECODE", "1")
        _, warnings = _run_check_env(monkeypatch)
        assert not any("mantishack.env" in w for w in warnings), warnings

    def test_no_mantishack_env_warning_outside_claude_session(
        self, fake_repo, monkeypatch,
    ):
