"""CLI smoke tests for mantishack.py — argparse acceptance and entry-point behavior.

Ported from the now-retired ``test/real_tests_fast.sh``. These tests verify
that the top-level ``mantishack.py`` dispatcher and per-mode subparsers reject
...
MANTISHACK_PY = REPO_ROOT / "mantishack.py"


def _run_mantishack(*args: str, timeout: float = 10.0) -> subprocess.CompletedProcess:
    """Invoke ``python mantishack.py <args>`` with stdout+stderr captured."""
    return subprocess.run(
        [sys.executable, str(MANTISHACK_PY), *args],
        capture_output=True,
        text=True,
        timeout=timeout,
    )
...
def _argparse_accepted(*args: str, timeout: float = 3.0) -> bool:
    ...
    """
    try:
        r = _run_mantishack(*args, timeout=timeout)
...
def test_no_args_exits_zero():
    """``mantishack.py`` with no arguments prints help and exits 0."""
    r = _run_mantishack()
...
def test_invalid_mode_errors():
    ...
    r = _run_mantishack("invalid_mode")
    assert r.returncode != 0
...
def test_missing_required_arg_errors(mode: str, required_keyword: str):
    ...
    try:
        r = _run_mantishack(mode, timeout=20)
...
@pytest.mark.parametrize(
    "module",
    ["mantishack", "mantishack_agentic", "mantishack_fuzzing", "mantishack_codeql"],
)
