...
class TestRenderClassification:
    ...
    def test_env_warnings_are_warnings(self):
        text, n_fail, n_warn = _render(
            *_gather_stub(env_warnings=["MANTISHACK_DIR not set"]),
            verbose=False,
        )
        assert n_warn == 1
...
class TestMainExitCodes:
    ...
    def test_warnings_alone_do_not_fail(self, capsys, monkeypatch):
        monkeypatch.setattr(
            doctor, "_gather",
            lambda: _gather_stub(env_warnings=["MANTISHACK_DIR not set"]),
        )
        rc = main([])
        assert rc == 0
...
class TestUsage:
    def test_unknown_flag_returns_two(self, capsys):
        ...
        assert rc == 2
        assert "usage: mantishack doctor" in captured.err
...
class TestOutputShape:
    def test_no_logo_no_quote(self, capsys, monkeypatch):
        ...
        assert "mantishack:~$" not in out  # no quote prompt
    ...
    def test_specific_paths_present_in_env_warnings(
        ...
        monkeypatch.setattr(
            ...
            lambda: _gather_stub(
                env_warnings=[
                    "MANTISHACK_DIR not set in this process; "
                    "expected /home/op/mantishack based on checkout "
                    "location.",
                ],
            ),
        )
        main([])
        out = capsys.readouterr().out
        assert "/home/op/mantishack" in out
