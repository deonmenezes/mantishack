"""MANTISHACK startup — environment checks and session initialisation.
...
def check_tools() -> tuple[list, list, set]:
    ...
    results = []
    available = set()
    for name in sorted(MantishackConfig.TOOL_DEPS):
        found = bool(shutil.which(MantishackConfig.TOOL_DEPS[name]["binary"]))
    ...
    for group_name, group in MantishackConfig.TOOL_GROUPS.items():
        members = sorted(n for n, d in MantishackConfig.TOOL_DEPS.items() if d.get("group") == group_name)
    ...
    for name in sorted(MantishackConfig.TOOL_DEPS):
        dep = MantishackConfig.TOOL_DEPS[name]
...
def check_llm() -> tuple[list, list]:
    ...
    Reads ~/.config/mantishack/models.json directly and tests API keys with
    ...
    try:
        # Read config
        config_path = Path.home() / ".config/mantishack/models.json"
        models = []
...
def _test_key(provider: str, api_key: str, api_base: str = None) -> bool:
    ...
    try:
        if provider == "gemini":
            ...
            # — MANTISHACK owns the URL prefix; ``api_base`` is the
            # operator's own config.
            r = requests.get(  # nosemgrep: sinks.mantishack.web.ssrf.dynamic-url
            ...
            r = requests.get(f"{base}/api/tags", timeout=timeout)  # nosemgrep: sinks.mantishack.web.ssrf.dynamic-url
            return r.status_code == 200
        else:
...
def check_env(unavailable_features: set) -> tuple[list, list]:
    ...
    if sys.version_info < (3, 10):
        ...
        warnings.append(
            f"Python {py_version_str} at {sys.executable} — MANTISHACK "
    ...
    # MANTISHACK_DIR — defensive check for the "operator bypassed the
    # wrapper" path. ``bin/mantishack`` / ``libexec/*`` scripts set this
    # automatically; ``CLAUDE_ENV_FILE`` propagates it into claude-
    # spawned Bash tool calls. Only unset when someone runs
    # ``python3 mantishack.py …`` (or imports core/ modules) from a
    # bare shell. Specific value computed from REPO_ROOT so the
    # operator can copy-paste the right export line.
    mantishack_dir = os.environ.get("MANTISHACK_DIR")
    if not mantishack_dir:
        warnings.append(
            f"MANTISHACK_DIR not set in this process; expected "
            f"{REPO_ROOT} based on checkout location. Affects "
            f"direct ``python3 mantishack.py …`` invocations only — "
            f"bin/mantishack and claude sessions set it automatically."
        )
    else:
        resolved = Path(mantishack_dir).resolve()
        if not resolved.is_dir():
            warnings.append(
                f"MANTISHACK_DIR={mantishack_dir!r} does not resolve to a "
                f"directory"
            )
        else:
            missing = [
            ...
            if missing:
                warnings.append(
                    f"MANTISHACK_DIR={mantishack_dir!r} is missing expected "
                    f"directories: {', '.join(missing)}"
                )

    # No check on .claude/mantishack.env or .claude/settings.json
    # despite both being part of the SessionStart hook chain.
    # Failure modes for either file missing are: operator wiped
    # it (wilful — operator knows), hook script broken (MANTISHACK
    ...
    out_dir = MantishackConfig.get_out_dir()
    ...
    out_dir_env = os.getenv("MANTISHACK_OUT_DIR")
    if out_dir_env:
        parts.append(f"MANTISHACK_OUT_DIR={escape_nonprintable(out_dir_env)}")
    config_env = os.getenv("MANTISHACK_CONFIG")
    if config_env:
...
# `libexec/mantishack-session-init:write_env_file` which is what the
