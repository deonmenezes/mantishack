"""``mantishack doctor`` — on-demand status report.
...
explicitly typed ``mantishack doctor`` because something feels off.
...
Deliberately NOT in scope:
  ...
  * Install advice for missing binaries. MANTISHACK's audience is
...
to ``MantishackConfig.TOOL_DEPS`` lights up in both banner and
...
_USAGE = (
    "usage: mantishack doctor [--strict] [--verbose]\n"
...
def _render(
    ...
    *,
    ...
    # pass, ``MANTISHACK_DIR not set …`` from the new check appears in
    # env_warnings.
    ...
    out: List[str] = ["MANTISHACK doctor", "============="]

    # Defence in depth: although every current producer of these
    # strings is MANTISHACK-internal (check_tools, check_llm, check_env),
