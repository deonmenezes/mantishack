"""Regression tests for MantishackLogger format-string / positional-args support.
...
but 3 were given`. The bifurcation between `get_logger()` (MantishackLogger
...
from core.logging import MantishackLogger
...
class FormatArgsAcrossLevelsTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.mantishack_logger = MantishackLogger()
        self.handler = _CapturingHandler()
        self.mantishack_logger.logger.addHandler(self.handler)

    def tearDown(self) -> None:
        self.mantishack_logger.logger.removeHandler(self.handler)

    def _last_message(self) -> str:
        return self.handler.records[-1].getMessage()

    def _assert_single_arg(self, method_name: str) -> None:
        method = getattr(self.mantishack_logger, method_name)
...
class FormatStyleTest(unittest.TestCase):
    ...
    def test_multi_positional_args(self) -> None:
        self.mantishack_logger.info("git fetch (depth=%d): %s @ %s", 1, "github.com/x/y", "abc123")
        self.assertEqual(
    ...
    def test_mapping_style_format(self) -> None:
        ...
        self.mantishack_logger.warning(
            "%(host)s failed: %(err)s",
    ...
    def test_typeful_args_repr_via_pct_s(self) -> None:
        ...
        self.mantishack_logger.error("read failed for %s: %s", Path("./x"), ValueError("nope"))
        self.assertEqual(
            self.handler.records[-1].getMessage(),
...
class BackwardsCompatTest(unittest.TestCase):
    ...
    def test_no_args_no_formatting(self) -> None:
        ...
        self.mantishack_logger.info("Coverage: 87% of files reviewed")
        self.assertEqual(
            self.handler.records[-1].getMessage(),
    ...
    def test_kwargs_still_route_to_extra(self) -> None:
        ...
        self.mantishack_logger.info("Processed %s files", 5, job_id="abc-123")
