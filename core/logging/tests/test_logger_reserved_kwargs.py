"""Regression tests for MantishackLogger reserved-kwarg handling.

F004: `MantishackLogger.critical` skipped `_split_kwargs` (open-coded only
...
class ReservedKwargAcrossLevelsTest(unittest.TestCase):
    ...
    def test_critical_preserves_exc_info_and_stack_info(self) -> None:
        ...
        try:
            ...
            self.mantishack_logger.critical("with exc", exc_info=True)
        last = self.handler.records[-1]
