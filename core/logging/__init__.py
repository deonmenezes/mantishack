#!/usr/bin/env python3
"""
MANTISHACK Structured Logging System
...
# in LogRecord"`. MantishackLogger filters these and renames colliders
...
class MantishackLogger:
    """
    Centralized logger for MANTISHACK framework.

    Provides both console and file logging with structured JSON output
    for audit trails.
    """

    _instance: Optional["MantishackLogger"] = None
    _initialized: bool = False

    def __new__(cls) -> "MantishackLogger":
    ...
    def __init__(self) -> None:
        ...
        if MantishackLogger._initialized:
            return

        self.logger = logging.getLogger("mantishack")
        ...
        console_formatter = logging.Formatter(MantishackConfig.LOG_FORMAT_CONSOLE)
        ...
        # `mantishack_<unix_seconds>.jsonl` — two MANTISHACK processes
        ...
        log_file = (
            ...
            / f"mantishack_{int(time.time())}_pid{_os.getpid()}_{ns_tail:04d}.jsonl"
        )
        # `delay=True` defers opening the file until the first emit.
        # Pre-fix every `MantishackLogger()` instantiation eagerly created
        # a file in `LOG_DIR`, even for processes that:
        #
        # * Imported `core.logging` but never logged (CLI `--help`,
        #   `mantishack --version`, dry-run / probe modes)
        ...
        #
        # Each created an empty `mantishack_*.jsonl` file that
        ...
        # surface in operator output. Pre-fix, MANTISHACK's handlers
        # were attached only to the "mantishack" namespace; stdlib-named
        ...
        # ``mantishack agentic``) where no other code calls basicConfig.
        #
        ...
        # "mantishack" namespace doesn't double-fire to the root
        # handler. Stdlib-named loggers do propagate, get handled
        # at root, and their format follows the same
        # LOG_FORMAT_CONSOLE shape as mantishack's own messages.
        root_logger = logging.getLogger()
        # Idempotent guard: only attach once even if MantishackLogger
        ...
        # comment above).
        if not any(
            isinstance(h, logging.StreamHandler) and getattr(h, "_mantishack_root_handler", False)
            ...
            root_console._mantishack_root_handler = True  # sentinel for the guard above
            root_logger.addHandler(root_console)
            # Ensure root accepts INFO-level records; default is WARNING.
            if root_logger.level == logging.NOTSET or root_logger.level > logging.INFO:
                root_logger.setLevel(logging.INFO)

        MantishackLogger._initialized = True

        self.debug(f"MANTISHACK logging initialized - audit trail: {log_file}")
...
def get_logger(name: Optional[str] = None) -> "logging.Logger":
    """Get a MANTISHACK logger.

    With no `name` (default): returns the singleton MantishackLogger
    wrapper for the framework's audit-trail behaviour.

    With a `name`: returns a `logging.Logger` child of "mantishack"
    namespaced under that name, e.g. `get_logger("core.sarif")`
    returns `logging.getLogger("mantishack.core.sarif")`. Lets modules
    ...
    base = MantishackLogger()
    if name is None:
        return base
    # Namespace under "mantishack" so child propagation reaches the
    # audit handlers attached to the base "mantishack" logger.
    safe_name = name if name.startswith("mantishack.") else f"mantishack.{name}"
