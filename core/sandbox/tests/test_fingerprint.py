...
def test_machine_id_is_not_naive_published_hash():
    """The first draft used hashlib.sha256(b'mantishack-sandbox-v1') which
    open-source publishes the exact bytes — any attacker can pre-
    compute it and bypass with a literal-string match. Per-install
    seed (MANTISHACK_DIR or this module's path) defeats that."""
    import hashlib as _hashlib
    naive = _hashlib.sha256(b"mantishack-sandbox-v1").hexdigest()[:32]
...
def test_os_release_does_not_mention_mantishack_or_sandbox():
    """Hide-intent invariant: nothing in persona names MANTISHACK, sandbox,
    analysis, malware, or analyst."""
    lower = _OS_RELEASE.lower()
    for tell in ("mantishack", "sandbox", "analysis", "malware", "analyst"):
