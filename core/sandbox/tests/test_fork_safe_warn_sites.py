...
# rather than `os.environ["MANTISHACK_DIR"]`. The env-var form failed when
# MANTISHACK_DIR pointed at a checkout that did not yet have the fail-CLOSED
...
def test_landlock_sys_create_failure_exits_126():
    ...
    assert b"MANTISHACK: landlock: SYS_landlock_create_ruleset" in proc.stderr
...
def test_mount_ns_extra_ro_paths_bind_failure_exits_126():
    ...
    proc = _run_child("""
        ...
        with patch.object(mount_ns, "_mount", fake_mount):
            ...
            import os
            path = "/usr"
            inside = "/tmp/mantishack-test-inside"
            ...
            try:
                ...
                try:
                    os.write(
                        2,
                        b"MANTISHACK: mount_ns: extra_ro_paths bind failed for "
                        + _path_b
                        + b" (errno=%d), exiting\\n" % (exc.errno or 0),
                    )
                except OSError:
    ...
    assert b"MANTISHACK: mount_ns: extra_ro_paths bind failed" in proc.stderr
...
def test_preexec_rlimit_core_failure_exits_99():
    ...
    assert b"MANTISHACK: preexec: RLIMIT_CORE setrlimit failed" in proc.stderr
