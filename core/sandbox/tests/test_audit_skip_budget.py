...
``MANTISHACK_REQUIRE_AUDIT_TESTS=1`` in the environment and any missing
...
Usage in CI:
    ...
    env MANTISHACK_REQUIRE_AUDIT_TESTS=1 pytest core/sandbox/
...
def test_audit_prereqs_present_when_required():
    ...
    """
    if os.environ.get("MANTISHACK_REQUIRE_AUDIT_TESTS") != "1":
        pytest.skip(
            "MANTISHACK_REQUIRE_AUDIT_TESTS not set; audit-test skip "
    ...
    if missing:
        pytest.fail(
            "MANTISHACK_REQUIRE_AUDIT_TESTS=1 was set but the audit "
