"""Tests for libexec/mantishack-r2-sandboxed.
...
WRAPPER = REPO_ROOT / "libexec" / "mantishack-r2-sandboxed"
...
def _trusted_env(**extra) -> dict:
    ...
    env["MANTISHACK_DIR"] = str(REPO_ROOT)
    env.update(extra)
    return env
...
# Wrapper exit codes — must match libexec/mantishack-r2-sandboxed.
...
class TestTrustMarker:
    """Wrapper refuses to run without _MANTISHACK_TRUSTED or CLAUDECODE."""

    def test_no_trust_marker_refuses(self, tmp_path):
        env = os.environ.copy()
        env.pop("_MANTISHACK_TRUSTED", None)
...
class TestAdversarialIsolation:
    ...
    def test_machine_id_is_persona_not_host(self):
        ...
        # The persona's machine-id is deterministic per MANTISHACK install
        # (sha256 of MANTISHACK_DIR). Compute it the same way for the
        # assertion.
