...
class TestE2EShim:
    ...
    def test_shim_exits_zero_and_prints_summary(self, tmp_path):
        ...
        shim = repo_root / "libexec" / "mantishack-sandbox-observe"
        if not shim.exists():
...
class TestTrustMarker:
    """mantishack-sandbox-observe shim refuses to run without
    CLAUDECODE / _MANTISHACK_TRUSTED, mirroring other libexec shims."""
