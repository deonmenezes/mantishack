...
# rather than `os.environ["MANTISHACK_DIR"]`. The env-var form silently
# misbehaved when MANTISHACK_DIR pointed at a different checkout — the
...
def test_warn_post_fork_writes_prefixed_line():
    ...
    try:
        os.dup2(w, 2)
        os.close(w)
        warn_post_fork(b"MANTISHACK: landlock: SYS_create returned -1\n")
    finally:
        os.dup2(saved, 2)
        os.close(saved)
    ...
    out = _read_fd(r)
    os.close(r)
    assert out == b"MANTISHACK: landlock: SYS_create returned -1\n"
...
def test_warn_post_fork_auto_prepends_prefix_when_missing():
    ...
    out = _read_fd(r)
    os.close(r)
    assert out == b"MANTISHACK: bare_event\n"
...
def test_warn_post_fork_no_double_prefix():
    ...
    try:
        os.dup2(w, 2)
        os.close(w)
        warn_post_fork(b"MANTISHACK: already_prefixed\n")
    finally:
        os.dup2(saved, 2)
        os.close(saved)
    ...
    out = _read_fd(r)
    os.close(r)
    assert out.count(b"MANTISHACK: ") == 1
...
def test_warn_post_fork_callable_from_preexec_fn():
    def preexec():
        from core.sandbox._fork_safe_warn import warn_post_fork
        warn_post_fork(b"MANTISHACK: preexec_test: from forked child\n")
