...
Discovered by full agentic E2E against /tmp/vulns: `python mantishack.py
...
class TestAgenticPropagatesSandboxFlags:
    """mantishack_agentic.py must forward --audit and --audit-verbose into
    ...
    def test_agentic_source_builds_passthrough_list(self):
        ...
        for flag, attr in [
            ...
            assert flag in src, f"mantishack_agentic.py missing {flag} forward"
            assert f'getattr(args, "{attr}"' in src, (
                f"mantishack_agentic.py must read args.{attr} to forward "
    ...
    def test_agentic_includes_passthrough_in_semgrep_cmd(self):
        ...
        assert "*sandbox_passthrough" in src, (
            "mantishack_agentic.py must splat *sandbox_passthrough into "
        ...
        assert src.count("*sandbox_passthrough") >= 2, (
            "mantishack_agentic.py spawns at least two sandbox-using "
...
class TestCodeqlAgentRegistersSandboxArgs:
    ...
    def test_codeql_agent_imports_add_cli_args(self):
        ...
        assert "from core.sandbox import add_cli_args" in src, (
            ...
            "mantishack_agentic.py forwarding the flags would error "
