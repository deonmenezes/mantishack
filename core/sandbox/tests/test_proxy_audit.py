...
class TestProxyAuditRefCount:
    ...
    def test_concurrent_non_audit_connect_stays_denied(
        ...
        try:
            ...
            #
            # Acceptable because:
            # 1. MANTISHACK rarely runs concurrent mixed-profile sandboxes
...
# ──────────────────────────────────────────────────────────────────────
# MANTISHACK_PROXY_AUDIT_ENFORCE env-var parse (W36.K.1 / F068)
# ──────────────────────────────────────────────────────────────────────
#
# Before W36.K.1: `bool(env_var)` treated any non-empty string as truthy,
# so MANTISHACK_PROXY_AUDIT_ENFORCE=0 / false / no / off all enabled strict
...
class TestProxyAuditEnforceEnvVarParse:
    ...
    def test_non_truthy_value_disables_enforce(
        ...
        monkeypatch.setenv("MANTISHACK_PROXY_AUDIT_ENFORCE", value)
        p = proxy_mod.get_proxy(["example.com"])
        try:
            assert p._audit_enforce is False, (
                f"MANTISHACK_PROXY_AUDIT_ENFORCE={value!r} must NOT enable "
    ...
    def test_truthy_value_enables_enforce(
        ...
        try:
            assert p._audit_enforce is True, (
                f"MANTISHACK_PROXY_AUDIT_ENFORCE={value!r} must enable "
    ...
    def test_absent_env_var_disables_enforce(self, reset_proxy, monkeypatch):
        ...
        monkeypatch.delenv("MANTISHACK_PROXY_AUDIT_ENFORCE", raising=False)
        p = proxy_mod.get_proxy(["example.com"])
        try:
            assert p._audit_enforce is False
        finally:
