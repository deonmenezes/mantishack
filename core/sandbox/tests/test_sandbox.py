...
class TestSandboxContextManager(unittest.TestCase):
    ...
    def test_env_allowlist_blocks_unknown_vars(self):
        ...
        os.environ["MANTISHACK_TEST_UNKNOWN_INJECTION"] = "should-not-leak"
        try:
            env = MantishackConfig.get_safe_env()
            self.assertNotIn("MANTISHACK_TEST_UNKNOWN_INJECTION", env,
            ...
            for k in ("MANTISHACK_TEST_UNKNOWN_INJECTION", "LC_TIME", "SSH_ASKPASS"):
                os.environ.pop(k, None)
...
class TestLandlockEnforcement(unittest.TestCase):
    ...
    def test_relative_output_path_does_not_break_landlock(self):
        ...
        printing 'MANTISHACK: Landlock writable path could not be opened'
...
class TestForkBombBounded(unittest.TestCase):
    ...
    unrelated MANTISHACK work on the invoking host UID.
    """
...
class TestPidNamespace(unittest.TestCase):
    ...
    def test_sandboxed_process_is_pid_1(self):
        ...
        - pid=3 when wrapped by libexec/mantishack-pid1-shim (shim=pid-1,
...
class TestFdIsolation(unittest.TestCase):
    """File descriptors must NOT be inherited from MANTISHACK into the sandbox
...
class TestSpeculativeToolPathsRetry(unittest.TestCase):
    ...
    def test_mantishack_prefix_lines_dont_block_retry(self):
        """``MANTISHACK:``-prefixed lines (sandbox-internal post-fork
        ...
        from core.sandbox import context as _ctx
        src = Path(_ctx.__file__).read_text()
        self.assertIn('startswith("MANTISHACK:")', src,
                      "speculative-C retry must skip MANTISHACK:-prefixed "
