...
class TestSafeEnvAllowlistHygiene(unittest.TestCase):
    ...
    def test_ssh_auth_sock_not_allowed(self):
        ...
        self.assertNotIn("SSH_AUTH_SOCK", MantishackConfig.SAFE_ENV_ALLOWLIST)
        # And no prefix match either.
        self.assertFalse(
            any("SSH_AUTH_SOCK".startswith(p) for p in MantishackConfig.SAFE_ENV_PREFIXES),
    ...
    def test_credential_vars_not_allowed(self):
        ...
        for name in risky:
            self.assertNotIn(name, MantishackConfig.SAFE_ENV_ALLOWLIST,
...
class TestPidNamespaceDefenses(unittest.TestCase):
    ...
    def test_self_runs_as_init_pid(self):
        ...
        - pid=3 under the subprocess-path pid-1 shim
          (libexec/mantishack-pid1-shim with its double-fork layout)
