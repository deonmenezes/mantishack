...
that check would detect MANTISHACK's sandbox and refuse to execute its
...
class TestAdversarialFingerprint(unittest.TestCase):
    ...
    def test_hostname_not_in_sandbox_blacklist(self):
        script = textwrap.dedent("""
            HN=$(hostname)
            for bad in sandbox cuckoo malware analyst sample mantishack; do
    ...
    def test_machine_id_is_not_naive_published_hash(self):
        ...
        sha256('mantishack-sandbox-v1') and literal-match. Per-install
        seeding defeats that — assert the literal naive value isn't
        what we ship."""
        import hashlib
        naive = hashlib.sha256(b"mantishack-sandbox-v1").hexdigest()[:32]
