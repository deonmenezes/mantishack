...
The module exposes `setup_mount_ns(target, output)` which:
    ...
    2. Creates a fresh tmpfs at /tmp/.mantishack-sbx-<pid> to become the new root.
...
def setup_mount_ns(target: Optional[str], output: Optional[str],
    ...
    # ``/tmp/.mantishack-sbx-{getpid()}`` — was predictable: a same-UID
    ...
    if not root_path:
        raise RuntimeError(
            ...
            "/tmp/.mantishack-sbx-<pid> fallback was a same-UID "
        ...
        for path in extra_ro_paths:
            ...
            try:
                ...
                try:
                    os.write(
                        2,
                        b"MANTISHACK: mount_ns: extra_ro_paths "
                        + _step
                        + b" failed for "
                        + _path_b
                        + b" (errno=%d), exiting\n" % (exc.errno or 0),
