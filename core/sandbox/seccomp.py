...
Blocklist not allowlist: MANTISHACK runs arbitrary target builds, so default-
...
_SECCOMP_BLOCK_ALWAYS = (
    ...
    # future MANTISHACK layouts share a PID namespace across sandboxes.
    "pidfd_getfd",
...
# input buffer. When MANTISHACK is run interactively, stdin is the invoking
    ...
    def _apply_seccomp():
        try:
            if _libc.prctl(_PR_SET_NO_NEW_PRIVS, 1, 0, 0, 0) != 0:
                _os_write(2, b"MANTISHACK: prctl(PR_SET_NO_NEW_PRIVS) failed -- "
            ...
            if not ctx:
                ...
                _os_write(2, b"MANTISHACK: seccomp_init failed -- "
            ...
            try:
                ...
                for name, num in resolved_blocks:
                    ...
                    if ret < 0:
                        _os_write(2, b"MANTISHACK: seccomp add_rule failed\n")
                ...
                if audit_mode:
                    ...
                    for name, num in audit_extra:
                        ...
                        if ret < 0:
                            _os_write(2, b"MANTISHACK: seccomp audit rule failed\n")
                ...
                if socket_num >= 0:
                    for fam in socket_family_blocks:
                        ...
                        if ret < 0:
                            _os_write(2, b"MANTISHACK: seccomp socket family rule failed\n")
                    ...
                    if ret < 0:
                        _os_write(2, b"MANTISHACK: seccomp SOCK_RAW rule failed\n")
                ...
                if block_udp and socket_num < 0:
                    ...
                    _os_write(2, b"MANTISHACK: seccomp block_udp requested but "
                ...
                if block_udp and socket_num >= 0:
                    for fam in (_AF_INET, _AF_INET6):
                        ...
                        if ret < 0:
                            _os_write(2, b"MANTISHACK: seccomp UDP block rule failed\n")
                ...
                if ioctl_num >= 0:
                    for cmd_val in _BLOCKED_IOCTL_CMDS:
                        ...
                        if ret < 0:
                            _os_write(2, b"MANTISHACK: seccomp ioctl rule failed\n")
                ...
                if ret < 0:
                    ...
                    _os_write(2, b"MANTISHACK: seccomp_load failed -- "
                                 b"refusing to exec without filter\n")
                    os._exit(126)
            finally:
            ...
            _os_write(2, b"MANTISHACK: seccomp enforcement failed -- "
                         b"refusing to exec without filter\n")
            os._exit(126)
