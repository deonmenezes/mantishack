...
def check_landlock_available() -> bool:
    ...
    with state._cache_lock:
        ...
        # task — applying it here would irreversibly restrict the MANTISHACK
        # Python process.
...
def _landlock_functional_self_test() -> bool:
    ...
    Why this design:
      - Fork so the parent (MANTISHACK) stays unrestricted.
...
def _run_selftest_in_child(write_fd: int) -> int:
    ...
    # same-user attacker who pre-planted /tmp/.mantishack_landlock_selftest_
    ...
    try:
        fd, test_path = tempfile.mkstemp(
            prefix=".mantishack_landlock_selftest_", dir="/tmp"
        )
    except OSError:
        return 0
    ...
    # and a /tmp/.mantishack_landlock_selftest_* stub.
    try:
        os.write(fd, b"x")
    except OSError:
        try:
    ...
    def _apply_landlock():
        try:
            ...
            if fd < 0:
                ...
                _os_write(2, b"MANTISHACK: landlock: SYS_landlock_create_ruleset failed post-fork\n")
            ...
            try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            ret = libc.syscall(SYS_add_rule, fd, RULE_PATH_BENEATH,
                                               ctypes.byref(rule), 0)
                            if ret < 0:
                                _os_write(2, b"MANTISHACK: Landlock add_rule failed for a writable path\n")
                        finally:
                            _os_close(dir_fd)
                    except OSError:
                        _os_write(2, b"MANTISHACK: Landlock writable path could not be opened\n")
                    ...
                    try:
                        ...
                        try:
                            ...
                            ret = libc.syscall(SYS_add_rule, fd, RULE_PATH_BENEATH,
                                               ctypes.byref(rule), 0)
                            if ret < 0:
                                _os_write(2, b"MANTISHACK: Landlock add_rule failed for a writable device\n")
                        finally:
                            _os_close(dev_fd)
                    except OSError:
                        ...
                        try:
                            ...
                            try:
                                ...
                                ret = libc.syscall(SYS_add_rule, fd, RULE_PATH_BENEATH,
                                                   ctypes.byref(rule), 0)
                                if ret < 0:
                                    _os_write(2, b"MANTISHACK: Landlock add_rule failed for a readable path\n")
                            finally:
                                _os_close(path_fd)
                        except OSError:
                            # Read path may not exist on all hosts (e.g.
                            # /sbin on usrmerge systems) — non-fatal.
                            _os_write(2, b"MANTISHACK: Landlock readable path could not be opened (skipped)\n")
                ...
                if ports is not None and _net_access > 0:
                    for port in ports:
                        ...
                        ret = libc.syscall(SYS_add_rule, fd, RULE_NET_PORT,
                                           ctypes.byref(rule), 0)
                        if ret < 0:
                            _os_write(2, b"MANTISHACK: Landlock TCP port allow-rule failed\n")
                ...
                if prctl_ret < 0:
                    _os_write(2, b"MANTISHACK: prctl(PR_SET_NO_NEW_PRIVS) failed -- aborting sandboxed exec\n")
                    os._exit(126)
                ...
                if result < 0:
                    ...
                    _os_write(2, b"MANTISHACK: Landlock restrict_self failed -- aborting sandboxed exec\n")
                    os._exit(126)
            finally:
            ...
            _os_write(2, b"MANTISHACK: Landlock enforcement failed -- aborting sandboxed exec\n")
            os._exit(126)
