...
Implications of the ⚠ items for the threat model:
  ...
  1. PID visibility: a sandboxed child that compromises a same-UID
     ...
     Mitigation: don't run MANTISHACK alongside other valuable same-UID
     processes; consider a dedicated user account for MANTISHACK on
     shared macOS hosts.
    ...
    if nproc_limit and nproc_limit > 0:
        ...
        def preexec():
            ...
            try:
                ...
                warn_post_fork(b"MANTISHACK: _macos_spawn RLIMIT_NPROC setrlimit failed -- documented soft posture became silent no-op\n")
    else:
