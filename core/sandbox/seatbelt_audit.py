...
appends MANTISHACK-format records to ``<run_dir>/.sandbox-denials.jsonl``
...
# Map SBPL action prefixes to the MANTISHACK sandbox-summary type taxonomy
...
def parse_log_entry(entry: dict, *,
    ...
    """Convert a `log stream` ndjson entry to a MANTISHACK audit record.
