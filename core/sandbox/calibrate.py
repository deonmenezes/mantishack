...
Threat model:
    ...
    ``~/.cache/mantishack/`` is bounded by the attacker's existing
    same-UID access.
...
API:
    ...
    clear_cache(bin_path=None)
        Drop one or all cache entries. ``mantishack sandbox calibrate
        --clear`` calls this.
"""
...
_CACHE_VERSION = 1
_CACHE_DIR = Path.home() / ".cache" / "mantishack" / "sandbox-profiles"
...
def _spawn_probe(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mantishack-calibrate-") as scratch:
        ...
        result = sandbox_run(
            ...
            proxy_hosts=["mantishack-calibrate.invalid"],
