...
# without affecting unrelated MANTISHACK work on the host UID. Skipped
...
_CONFIG_PATH = Path.home() / ".config/mantishack/sandbox.json"
...
# sandbox.json shouldn't have to restart every MANTISHACK process; 60s is
...
def _load_user_limits() -> dict:
    """Load user-configured resource limits from ~/.config/mantishack/sandbox.json.
    ...
    with state._cache_lock:
        ...
        try:
            ...
            # `~/.config/mantishack/sandbox.json` as a FIFO via
            # `mkfifo` would cause every MANTISHACK process to hang
...
def _make_preexec_fn(limits: dict, writable_paths: list = None,
    ...
    would kill unrelated MANTISHACK work. NPROC is applied separately via
    ...
    def _set_limits():
        ...
        try:
            ...
            try:
                os.write(
                    2,
                    b"MANTISHACK: preexec: RLIMIT_CORE setrlimit failed "
                    b"(errno=%d), exiting\n" % _errno,
                )
            except OSError:
                pass
