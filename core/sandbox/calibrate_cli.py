...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mantishack-sandbox-calibrate",
        description=(
...
def _cli_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if not bin_path.exists():
        sys.stderr.write(
            f"mantishack-sandbox-calibrate: {bin_path}: not found\n"
        )
        return _USAGE_EX
    ...
    if args.show:
        ...
        try:
            ...
            sys.stderr.write(
                f"mantishack-sandbox-calibrate: cannot hash {bin_path}: {exc}\n"
            )
            return _SOFTWARE_EX
        ...
        if cached is None:
            sys.stderr.write(
                f"mantishack-sandbox-calibrate: no cached profile for "
                f"{bin_path} (env keys: "
    ...
    try:
        ...
        sys.stderr.write(
            f"mantishack-sandbox-calibrate: {exc}\n"
        )
        return _USAGE_EX
