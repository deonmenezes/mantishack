...
Architecture:
- In-process: runs on a daemon thread inside MANTISHACK's Python process.
...
- asyncio event loop in the daemon thread handles many concurrent
  tunnels without thread-per-connection. MANTISHACK's main code stays
...
- Hostname allowlist is UNION across all callers: if cc_dispatch asks
  ...
  hosts are allowed globally. Trust model: MANTISHACK's own code is the
...
class EgressProxy:
    ...
    def __init__(self, allowed_hosts: Iterable[str],
        ...
        # or set the MANTISHACK_PROXY_AUDIT_ENFORCE env var to one of the
    ...
    async def _serve_tunnel(self, reader: asyncio.StreamReader,
        ...
        if not self.is_host_allowed(host):
            ...
            if _audit_now:
                ...
                # When audit_enforce=True (MANTISHACK_PROXY_AUDIT_ENFORCE=1),
...
def get_proxy(allowed_hosts: Iterable[str]) -> EgressProxy:
    ...
    directly). This lets MANTISHACK work inside corporate networks where
    ...
    with _lock:
        ...
        if _instance is None:
            ...
            # so MANTISHACK_PROXY_AUDIT_ENFORCE=0 / false / no / off all
            ...
            _enforce_raw = _os.environ.get(
                "MANTISHACK_PROXY_AUDIT_ENFORCE", "",
            ).strip().lower()
