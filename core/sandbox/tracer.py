...
Why a separate process rather than a thread inside MANTISHACK:
- ptrace + multi-threaded parents fight for signal delivery: SIGCHLD
  ...
  MANTISHACK signal handling all collide. A dedicated tracer process
  decouples the signal landscape entirely.
- Tracer crash doesn't take down the whole MANTISHACK run.
...
This is acceptable for MANTISHACK's threat model: audit mode is for
operator workflows (gcc/make/python builds, claude sub-agents)
that don't try to detect or evade observation. MANTISHACK is NOT a
...
def _resolve_tracee_path(pid: int, path: str, dirfd: int) -> str:
    ...
    sees it. This is correct for MANTISHACK's standard sandbox layout
...
def _write_record(run_dir: Path, syscall_name: str, syscall_nr: int,
    ...
    try:
        ...
        # cmd's syscall_name and PID come from MANTISHACK-controlled
...
def trace(target_pid: int, run_dir: Path,
    ...
    target keep running. Negligible workflow impact (MANTISHACK users
        ...
        if regs is not None:
            ...
            if should_log:
                ...
                if budget.pop_global_cap_notice():
                    os.write(2, (
                        f"MANTISHACK tracer: audit-record global cap "
