...
# /etc/machine-id: deterministic-pseudo-random per MANTISHACK install.
...
# where a malware author who knows MANTISHACK can pre-compute a literal
...
# "sha256('mantishack-sandbox-v1')" was the prior implementation but
# being open-source it was a single grep away from a one-line bypass.
def _derive_machine_id() -> str:
    # Use MANTISHACK_DIR (operator's install path) as the per-install
    # entropy. Falls back to this module's directory if MANTISHACK_DIR
    # is unset — that path is also install-specific.
    seed = os.environ.get("MANTISHACK_DIR") or os.path.dirname(
        os.path.abspath(__file__)
    )
    return hashlib.sha256(
        b"mantishack-fingerprint-v1\0" + seed.encode("utf-8", errors="replace")
    ).hexdigest()[:32]
...
def build_persona(tmpdir: Path, cpu_count: int) -> Persona:
    ...
    #
    # Fake uptime is deterministic per MANTISHACK install (same seed as
...
def _derive_uptime_and_processes() -> tuple[int, int]:
    ...
    MANTISHACK install. Uptime in [3 days, 30 days], processes in
    ...
    h = hashlib.sha256(
        b"mantishack-fingerprint-uptime-v1\0" + seed.encode("utf-8", errors="replace")
    ).digest()
...
def is_supported() -> bool:
    ...
    untrusted-binary analysis on macOS: run MANTISHACK in a Linux VM
