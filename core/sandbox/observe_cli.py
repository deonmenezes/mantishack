"""CLI entry point for ``mantishack-sandbox-observe``.

Wraps ``sandbox(observe=True)`` so an operator can quickly answer
"what does this binary touch?" without writing Python:

  mantishack-sandbox-observe -- /usr/bin/true
  mantishack-sandbox-observe --json -- claude --version
  mantishack-sandbox-observe --out /tmp/probe -- ./scan-target
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mantishack-sandbox-observe",
        description=(
...
def _resolve_run_dir(args: argparse.Namespace,
    ...
    tmp = tempfile.mkdtemp(prefix="mantishack-observe-")
    if not args.keep:
...
def _cli_main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    with contextlib.ExitStack() as stack:
        ...
        try:
            ...
            sys.stderr.write(f"mantishack-sandbox-observe: {exc}\n")
            return _USAGE_EX
        ...
        if not observe_log.exists():
            sys.stderr.write(
                "mantishack-sandbox-observe: observe log not produced — "
