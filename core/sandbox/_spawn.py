...
def _set_rlimits(limits: dict) -> None:
    ...
    if mem > 0:
        try:
            ...
            warn_post_fork(b"MANTISHACK: _set_rlimits RLIMIT_AS setrlimit failed -- memory cap not applied\n")
    if file_mb > 0:
        try:
            resource.setrlimit(resource.RLIMIT_FSIZE, (file_bytes, file_bytes))
        except (ValueError, OSError):
            warn_post_fork(b"MANTISHACK: _set_rlimits RLIMIT_FSIZE setrlimit failed -- file-size cap not applied\n")
    if cpu > 0:
        try:
            resource.setrlimit(resource.RLIMIT_CPU, (cpu, cpu + 1))
        except (ValueError, OSError):
            warn_post_fork(b"MANTISHACK: _set_rlimits RLIMIT_CPU setrlimit failed -- cpu cap not applied\n")
    ...
    try:
        ...
        warn_post_fork(b"MANTISHACK: _set_rlimits RLIMIT_CORE setrlimit failed -- coredump suppression relies on kernel core_pattern\n")
...
def _sweep_stale_audit_configs() -> None:
    """Remove stale mantishack-audit-cfg-* tempfiles in /tmp owned by
    ...
    for path in glob.glob(f"{tmp_root}/mantishack-audit-cfg-*.json"):
        try:
            st = os.lstat(path)
            if st.st_uid != my_uid:
                continue
...
def run_sandboxed(
    ...
    *,
    ...
    # TOCTOU substitution.
    import tempfile as _tempfile
    _root_dir = _tempfile.mkdtemp(prefix=".mantishack-sbx-")
    ...
    if audit_mode:
        if audit_run_dir is None:
            ...
            # Pre-fix this raise leaked a `.mantishack-sbx-*` directory on
        ...
        if not seccomp_profile:
            ...
            # (``MantishackConfig.get_safe_env()`` strips the
            ...
            # decide to pin via ``MantishackConfig`` if the threat
            # model tightens.
            import tempfile as _tempfile
            ...
            _cfd, _audit_config_path = _tf.mkstemp(
                prefix="mantishack-audit-cfg-", suffix=".json",
            )
    ...
    try:
        ...
        # the audit-config file, and a .mantishack-sbx-* dir on every call.
        _close_leftover()
    ...
    if child_pid == 0:
        ...
        if _use_devnull:
            if stdin == subprocess.PIPE:
                try:
                    os.write(2, b"MANTISHACK sandbox: stdin=subprocess.PIPE "
        ...
        try:
            ...
            if cwd:
                try:
                    ...
                    try:
                        os.write(2,
                            f"MANTISHACK sandbox: cwd={cwd!r} unusable inside "
            ...
            if grand == 0:
                ...
                if env is not None:
                    ...
                    if strict_env:
                        from core.config import MantishackConfig
                        _dangerous = set(MantishackConfig.DANGEROUS_ENV_VARS)
                        exec_env = {
                ...
                if nproc_limit and nproc_limit > 0:
                    ...
                    try:
                        ...
                        warn_post_fork(b"MANTISHACK: _spawn grandchild RLIMIT_NPROC setrlimit failed -- fork-bomb bound not applied\n")
                try:
                    # nosemgrep: python.lang.security.audit.dangerous-os-exec-tainted-env-args.dangerous-os-exec-tainted-env-args
                    # exec_env is from a MANTISHACK caller — either an
                    ...
                    # path. Either way the caller is trusted (MANTISHACK
            ...
            try:
                os.write(2, f"MANTISHACK sandbox child failure:\n{traceback.format_exc()}\n".encode())
            except Exception:
                pass
            os._exit(126)
    ...
    try:
        ...
        if _audit_engaged:
            ...
            if tracer_pid == 0:
                ...
                # running MANTISHACK processes that had bumped their
                ...
                # as mantishack-pid1-shim.
                try:
                    mantishack_dir = os.environ.get("MANTISHACK_DIR")
                    if mantishack_dir is None:
                    ...
                    if os.WIFEXITED(tracer_status):
                        ...
                        if ec == 127:
                            ...
                            cause = ("tracer subprocess failed to "
                                     ...
                                     "see MANTISHACK debug logs for the "
                                     "execvpe stack trace")
                        elif ec == 1:
