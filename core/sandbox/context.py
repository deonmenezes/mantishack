...
def sandbox(block_network: bool = False, target: str = None, output: str = None,
    ...
    Args:
        ...
        restrict_reads: If True, flip Landlock's default "read everywhere"
                 ...
                 can't read ~/.ssh, ~/.aws/credentials, ~/.config/mantishack/
    ...
    if effectively_disabled and not state._cli_sandbox_disabled:
        ...
        if state.warn_once("_sandbox_landlock_only_warned"):
            logger.warning(
                "MANTISHACK: sandbox running in Landlock-only mode — "
    ...
    if target or output or allowed_tcp_ports or extra_writable_paths:
        ...
        if output:
            ...
            # "MANTISHACK: Landlock writable path could not be opened"
    ...
    if restrict_reads:
        ...
        # single file keeps the rest of libexec/ (other MANTISHACK
        ...
        _shim = _Path(__file__).resolve().parents[2] / "libexec" / "mantishack-pid1-shim"
        if _shim.is_file():
    ...
    if sanitise_host_fingerprint:
        ...
        if not is_supported():
            _fp_unsupported_reason = (
                ...
                "bind-mount + UTS-namespace primitives — run MANTISHACK "
        ...
        if _fp_unsupported_reason:
            ...
            _persona_tmpdir = _tf.mkdtemp(prefix="mantishack-persona-")
    ...
    def run(cmd: List[str], **kwargs) -> subprocess.CompletedProcess:
        ...
        # LD_PRELOAD from whatever shell invoked MANTISHACK.
        #
        ...
        if kwargs.get("env") is None:
            ...
            kwargs["env"] = MantishackConfig.get_safe_env()
        else:
        ...
        # but we reject explicit overrides — inheriting FDs from MANTISHACK
        ...
        # for any same-UID host process — including the parent MANTISHACK
        ...
        if need_unshare:
            ...
            # observability. Interpose libexec/mantishack-pid1-shim so
            ...
            from pathlib import Path as _Path
            shim_path = str(
                _Path(__file__).resolve().parents[2] / "libexec" / "mantishack-pid1-shim"
            )
            ...
            if spawn_eligible and use_seatbelt:
                ...
                try:
                    ...
                    if _spawn_mod.mount_ns_available():
                        ...
                        # Strip ``MANTISHACK:``-prefixed lines before the
                        ...
                        _stderr_text = "\n".join(
                            ...
                            if not ln.lstrip().startswith("MANTISHACK:")
                        )
                        if (tool_paths
        ...
        if use_egress_proxy and _engaging_audit:
            try:
                ...
                logger.warning(
                    ...
                    "sandboxes in this process. Restart MANTISHACK if "
                    "this recurs.",
                    exc_info=True,
                )
...
def run_trusted(cmd: List[str], **kwargs) -> subprocess.CompletedProcess:
    """Run a command whose input was chosen by MANTISHACK itself (not the target).

    Example uses: readelf/nm/strings on a MANTISHACK-picked binary path,
...
def run_untrusted(cmd: List[str], *, target: str = None, output: str = None,
    ...
    exfiltrate ~/.ssh, ~/.aws/credentials, ~/.config/mantishack/models.json,
    ...
    # is the operator's TTY (common for interactive MANTISHACK use) and the
    ...
    # tool running under an interactive MANTISHACK invocation would return
