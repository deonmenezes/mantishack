...
Egress proxy (use_egress_proxy=True + proxy_hosts=[...]):
  A per-MANTISHACK-process asyncio daemon-thread HTTPS-CONNECT proxy.
...
Sanitised host fingerprint (sanitise_host_fingerprint=True):
  ...
  All hide-intent values — no "sandbox" / "mantishack" / "Generic CPU"
...
Log-output hygiene:
  MANTISHACK logs attacker-influenced strings in several places (argv
...
- Cross-process /proc/<host_pid>/environ reads (restrict_reads=True):
  ...
  / AWS env exfil from the parent MANTISHACK process in Landlock-only
  mode.
...
- Fork bombs — RLIMIT_NPROC=1024 applied via `prlimit --nproc` wrapper
  ...
  work). Configurable via `~/.config/mantishack/sandbox.json`.
...
- Parent-env code injection — `bin/mantishack` strips `LD_PRELOAD`,
...
- Controlling-tty keystroke-sniff — `run_untrusted()` defaults
  ...
  MANTISHACK invocation can't open `/dev/tty` to passively read
...
- Landlock self-test TOCTOU — the forked self-test uses
  ...
  that pre-plants `/tmp/.mantishack_landlock_selftest_<pid>` as a
...
behaviour change. Operators wanting a hardened posture opt in.
  ...
  - ``run_untrusted(strict_env=True)`` — default ON for this helper
    ...
    ``MantishackConfig.DANGEROUS_ENV_VARS`` (LD_PRELOAD, AWS_*, GH_TOKEN,
  ...
  - ``EgressProxy(audit_enforce=True)`` / env var
    ``MANTISHACK_PROXY_AUDIT_ENFORCE`` set to ``"1"`` / ``"true"`` /
...
- Binary-analysis tools on MANTISHACK-chosen paths (readelf, nm, strings,
...
Two entrypoints for 90% of callers:
    ...
    # Trusted tool on a MANTISHACK-chosen binary — no ns, no Landlock
...
- log_sanitisation.py  — escape_nonprintable() / has_nonprintable().
                         ...
                         MANTISHACK components (SARIF parsers, report
