...
def _make_understand_dir(parent, name="understand-20260401-120000",
    ...
    # .mantishack-run.json so infer_command_type works
    _write_json(d / ".mantishack-run.json", {"version": 1, "command": "understand",
                                          "status": "completed"})
    if checklist:
...
class TestFindUnderstandOutput:
    ...
    def test_tier3_global_out_by_target_path(self, tmp_path, monkeypatch):
        ...
        # Monkeypatch MantishackConfig to use our tmp out/
        monkeypatch.setattr("core.config.MantishackConfig.get_out_dir",
    ...
    def test_ignores_dirs_without_context_map(self, tmp_path):
        ...
        empty.mkdir()
        _write_json(empty / ".mantishack-run.json", {"version": 1, "command": "understand"})
    ...
    def test_ignores_non_understand_dirs(self, tmp_path):
        ...
        _write_json(scan / ".mantishack-run.json", {"version": 1, "command": "scan"})
...
class TestNormalizeContextMap:
    ...
    def test_log_output_escapes_terminal_escapes_in_file_path(self, caplog):
        ...
        # operator's terminal via mantishack's own warning output. Defended
...
# ---------------------------------------------------------------------------
# mantishack-build-checklist script
# ---------------------------------------------------------------------------

@pytest.mark.integration
class TestBuildChecklistScript:
    """Spawns the libexec/mantishack-build-checklist binary as a real
    subprocess — marker keeps the class out of default fast-tier runs."""
    def test_creates_checklist(self, tmp_path):
        """mantishack-build-checklist creates checklist.json."""
        import subprocess
        target = tmp_path / "src"
        target.mkdir()
        ...
        result = subprocess.run(
            ["libexec/mantishack-build-checklist", str(target), str(out_dir)],
