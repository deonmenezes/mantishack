...
``libexec/mantishack-run-lifecycle`` and ``libexec/mantishack-build-checklist``
...
def _make_agentic_out(tmp: Path, target: Path) -> Path:
    ...
    (out_dir / ".mantishack-run.json").write_text(json.dumps({
        "command": "agentic",
        "target": str(target),
        "status": "running",
    }))
    ...
    def test_creates_proper_understand_run_discoverable_by_bridge(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            env_patch = patch.dict(os.environ, {
                "HOME": str(tmp / "home"),
                "MANTISHACK_OUT_DIR": str(tmp / "out"),
            })
            with env_patch, \
            ...
            self.assertTrue((understand_dir / ".mantishack-run.json").exists())
            run_meta = json.loads((understand_dir / ".mantishack-run.json").read_text())
            ...
            # mantishack-build-checklist call needed).
...
class PostpassIntegrationTests(unittest.TestCase):
    ...
    def test_creates_proper_validate_run_as_sibling_of_understand(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            run_meta = json.loads((validate_dir / ".mantishack-run.json").read_text())
