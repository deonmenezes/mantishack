...
def _make_lifecycle_dispatcher(start_dir=None, claude_writes=None,
    ...
    def dispatcher(cmd, *args, **kwargs):
        ...
        if program == "mantishack-run-lifecycle":
        ...
        if program == "mantishack-build-checklist":
...
class UnderstandPrepassTests(unittest.TestCase):
    ...
    def test_happy_path_runs_lifecycle_builds_checklist_invokes_claude(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("mantishack-run-lifecycle", programs)
            self.assertIn("mantishack-build-checklist", programs)
    ...
    def test_happy_path_invokes_claude_with_expected_args(self):
        with TemporaryDirectory() as tmp:
            ...
            # MANTISHACK_DIR must be in --add-dir so the subprocess can read
            # .claude/skills/code-understanding/SKILL.md.
            from core.orchestration.agentic_passes import _MANTISHACK_DIR
            add_dirs = {cmd[i + 1] for i, a in enumerate(cmd) if a == "--add-dir"}
            self.assertIn(str(_MANTISHACK_DIR), add_dirs)
    ...
    def test_claude_dispatch_uses_sandbox_with_egress_proxy(self):
        with TemporaryDirectory() as tmp:
            ...
            # and MANTISHACK_DIR (for libexec scripts the LLM invokes).
            ...
            self.assertTrue(any("mantishack" in p.lower() for p in paths),
                            f"missing MANTISHACK_DIR in readable_paths: {paths!r}")
    ...
    def test_reuses_agentic_checklist_when_present(self):
        ...
        # parse pass via mantishack-build-checklist.
        with TemporaryDirectory() as tmp:
            tmp = Path(tmp)
            agentic_out = tmp / "agentic"
            agentic_out.mkdir()
            ...
            # Assert mantishack-build-checklist was NOT invoked (checklist was copied).
            programs = [Path(c.args[0][0]).name for c in mock_run.call_args_list]
            self.assertNotIn("mantishack-build-checklist", programs)
...
class ValidatePostpassTests(unittest.TestCase):
    ...
    def test_validate_dispatch_uses_sandbox_with_egress_proxy(self):
        with TemporaryDirectory() as tmp:
            ...
            # readable_paths must include ~/.claude + MANTISHACK_DIR + the
...
class PromptBuildTests(unittest.TestCase):
    ...
    def test_understand_prompt_includes_paths(self):
        ...
        self.assertIn("Do not call libexec/mantishack-run-lifecycle", prompt)
    ...
    def test_validate_prompt_references_selection_file(self):
        ...
        self.assertIn("libexec/mantishack-run-lifecycle", prompt)
...
class AdversarialBugsTests(unittest.TestCase):
    ...
    def test_keyboard_interrupt_during_claude_marks_lifecycle_failed(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            fail_called = any(
                Path(c[0]).name == "mantishack-run-lifecycle" and "fail" in c
                for c in calls
            )
            self.assertTrue(
                fail_called,
...
class LifecycleHelperResilienceTests(unittest.TestCase):
    ...
    def test_complete_lifecycle_failure_does_not_raise(self):
        # If mantishack-run-lifecycle complete itself errors after the work
...
class TimeoutTests(unittest.TestCase):
    ...
    def test_prepass_timeout_marks_lifecycle_failed(self):
        with TemporaryDirectory() as tmp:
            ...
            fail_called = any(
                c[0] == "mantishack-run-lifecycle" and "fail" in c
                for c in calls
            )
