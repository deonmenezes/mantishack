...
``mantishack_agentic.main``) and available to any future consumer that
needs to bucket a list of per-finding LLM dispatch results.

Lives here rather than inline in ``mantishack_agentic.py`` so the
classification can be unit-tested without pulling in the
``mantishack_agentic`` module's transitive imports (every ``core.*`` and
...
def bucket_orchestration_results(results: list[dict]) -> dict[str, Any]:
    ...
    * any other value, most commonly ``None`` from a q<0.5 empty
    ...
    Pre-fix the inline loop in ``mantishack_agentic`` treated everything
