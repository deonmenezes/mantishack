...
# of a flow trace.  Mirrors the names accepted by ``mantishack-smt-validate-path``
...
def _collect_candidates(
    ...
    if target_path:
        ...
        out_root = MantishackConfig.get_out_dir()
...
def _validate_path_conditions(
    ...
    ``mantishack-smt-validate-path --stdin`` accepts).  Returns ``None`` —
...
def _validate_path_profile(profile: Any, source: str) -> Optional[str]:
    ...
    ``mantishack-smt-validate-path``.  Drops the field with a warning on
    anything else.
    """
    if profile is None:
        return None
