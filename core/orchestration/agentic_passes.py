...
passes are first-class run dirs created via libexec/mantishack-run-lifecycle,
...
_MANTISHACK_DIR = Path(__file__).resolve().parents[2]
_LIFECYCLE = _MANTISHACK_DIR / "libexec" / "mantishack-run-lifecycle"
_BUILD_CHECKLIST = _MANTISHACK_DIR / "libexec" / "mantishack-build-checklist"
    ...
    try:
        ...
        try:
            ...
            prepass_config = CCDispatchConfig(
                ...
                add_dirs=(str(_MANTISHACK_DIR), str(target), str(understand_dir)),
            ...
            # str(_MANTISHACK_DIR) on top of the calibrated/default
            # readable_paths so the LLM-directed Bash tool can invoke
            # libexec/mantishack-normalize-context-map (MAP-5) and
            # libexec/mantishack-coverage-summary --mark (MAP-6) — those
            # scripts live under MANTISHACK_DIR. target+understand_dir
            ...
            proc = run_untrusted_networked(
                ...
                readable_paths=(
                    [str(_MANTISHACK_DIR)] + _readable_paths_for_cc_dispatch(claude_bin)
                ),
    ...
    try:
        ...
        try:
            ...
            postpass_config = CCDispatchConfig(
                ...
                add_dirs=(str(_MANTISHACK_DIR), str(target), str(agentic_out_dir), str(validate_dir)),
            ...
            # all of which run from MANTISHACK_DIR/libexec; agentic_out_dir
            ...
            # layout; site-specific extras (MANTISHACK_DIR, agentic_out_dir)
            # are prepended.
            proc = run_untrusted_networked(
                ...
                readable_paths=(
                    [str(_MANTISHACK_DIR), str(agentic_out_dir)]
...
# ---------------------------------------------------------------------------
# Lifecycle helpers — wrap libexec/mantishack-run-lifecycle and mantishack-build-checklist.
# ---------------------------------------------------------------------------
...
def _start_lifecycle(command: str, target: Path) -> Optional[Path]:
    ...
    target — operator points MANTISHACK at a freshly cloned
    ...
    subprocesses (which themselves invoke mantishack-managed
    bash + python) widens the trust boundary unnecessarily.

    Use `MantishackConfig.get_safe_env()` (strips the
    ...
    safe_env = MantishackConfig.get_safe_env()
    try:
        proc = subprocess.run(
...
def _build_checklist(target: Path, output_dir: Path) -> bool:
    """Run libexec/mantishack-build-checklist. Returns True on success.
...
def _provision_understand_checklist(target: Path, agentic_out_dir: Path,
    ...
    Falls back to running mantishack-build-checklist when no agentic checklist
...
def _build_understand_prompt(target: Path, understand_dir: Path) -> str:
    ...
    safe_mantishack = escape_nonprintable(str(_MANTISHACK_DIR))
...
1. Load .claude/skills/code-understanding/SKILL.md and
   .claude/skills/code-understanding/map.md from {safe_mantishack}.
...
4. Do not call libexec/mantishack-run-lifecycle — the launcher manages the
...
1. Load .claude/skills/exploitability-validation/SKILL.md from {_MANTISHACK_DIR}
...
2. Use {validate_dir} as the validate output directory. The launcher has
   ...
   libexec/mantishack-run-lifecycle.
